...
function prerequisitesAvailable():
  ...
  if (!fs.existsSync(BUNDLE))
    return {
      ok: false,
      reason: `diagram-render bundle missing (${BUNDLE}). Run bun run build:diagram-render.`,
    };
...
function countSaturatedPixels(ppmPath: string, delta: number): number {
  ...
  for (let p = 0; p < w * h; p++) {
    const o = i + p * 3;
    if (
      Math.max(b[o], b[o + 1], b[o + 2]) - Math.min(b[o], b[o + 1], b[o + 2]) >
      delta
    )
      sat++;
  }
...
describe("diagram render gate", () => {
  ...
  test.skipIf(!avail.ok)(
    "mermaid fences render as vector diagrams; images and diagnostics behave",
    () => {
      if (!avail.ok) return;
      ...
      try {
        ...
        const saturated = countSaturatedPixels(
          `${ppmPrefix}.ppm`,
          SATURATION_DELTA,
        );
  ...
  test.skipIf(!avail.ok)(
    "--strict fails on a missing image with a non-zero exit",
    () => {
      if (!avail.ok) return;
  ...
  if (!avail.ok) {
    test("diagram gate prerequisites are present (hard-required in CI)", () => {
      if (process.env.CI) {
        throw new Error(
          `diagram gate prerequisites missing in CI: ${avail.reason}`,
        );
      }
