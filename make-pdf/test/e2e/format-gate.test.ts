...
function prerequisitesAvailable():
  ...
  if (!Bun.which("unzip"))
    return {
      ok: false,
      reason: "unzip not found (needed for docx zip checks).",
    };
  return { ok: true };
}
...
function generate(to: string, outputPath: string): void {
  execFileSync(
    PDF_BIN,
    ["generate", FIXTURE, outputPath, "--quiet", "--to", to],
    {
      encoding: "utf8",
...
describe("output format gate", () => {
  ...
  test.skipIf(!avail.ok)(
    "--to html: single self-contained file, zero network refs",
    () => {
      if (!avail.ok) return;
  ...
  test.skipIf(!avail.ok)(
    "--to docx: valid OOXML with content + PNG diagram media",
    () => {
      if (!avail.ok) return;
      ...
      try {
        ...
        const listing = execFileSync("unzip", ["-l", out], {
          encoding: "utf8",
          timeout: CHILD_TIMEOUT_MS,
        });
        ...
        expect(
          (listing.match(/word\/media\/image[^\s]*\.png/g) ?? []).length,
  ...
  test.skipIf(!avail.ok)(
    "--to rejects unknown formats with a --format disambiguation hint",
    () => {
      if (!avail.ok) return;
      let stderr = "";
      try {
  ...
  if (!avail.ok) {
    test("format gate prerequisites are present (hard-required in CI)", () => {
      if (process.env.CI) {
        throw new Error(
          `format gate prerequisites missing in CI: ${avail.reason}`,
        );
      }
