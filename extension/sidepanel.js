...
// ─── Copy Cookies ───────────────────────────────────────────────
document
  .getElementById("chat-cookies-btn")
  .addEventListener("click", async () => {
    ...
    try {
      await fetch(`${serverUrl}/command`, {
        ...
        body: JSON.stringify({
          command: "goto",
          args: [`${serverUrl}/cookie-picker`],
        }),
      });
    } catch (err) {
      console.error(
        "[gstack sidebar] Failed to open cookie picker:",
        err.message,
      );
    }
  });
...
function showPrimaryPane() {
  ...
  document
    .querySelectorAll(".debug-tabs .tab")
...
document
  .querySelectorAll(".debug-tabs .tab:not(.close-debug)")
  .forEach((tab) => {
...
function addEntry(entry) {
  ...
  if (entry.url)
    document.getElementById("footer-url")?.textContent &&
      (document.getElementById("footer-url").textContent = new URL(
        entry.url,
      ).hostname);
...
async function ensureSseSessionCookie() {
  ...
  try {
    ...
    console.warn(
      "[gstack sidebar] Failed to mint SSE session cookie:",
      err && err.message,
    );
    return false;
  }
...
async function connectSSE() {
  ...
  eventSource.addEventListener("activity", (e) => {
    try {
      ...
      console.error(
        "[gstack sidebar] Failed to parse activity event:",
        err.message,
      );
    }
  });
...
function renderMemFooter(snapshot) {
  ...
  if (bunRss > 8 * 1024 * 1024 * 1024 || tabCount > 200)
    el.classList.add("bad");
  else if (bunRss > 2 * 1024 * 1024 * 1024 || tabCount > 50)
...
async function pollMemoryOnce() {
  ...
  try {
    ...
    try {
      evaluateMemToast(snapshot);
    } catch (err) {
      console.debug(
        "[gstack sidebar] mem-toast evaluation failed:",
        err && err.message,
      );
    }
    ...
    if (elapsed > MEM_POLL_SLOW_THRESHOLD_MS) {
      console.debug(
        "[gstack sidebar] /memory poll slow/failed:",
        elapsed,
        "ms",
        err && err.message,
      );
...
async function loadSnoozeState() {
  ...
  try {
    ...
    console.debug(
      "[gstack sidebar] mem-toast snooze load failed:",
      err && err.message,
    );
  }
}
...
async function saveSnoozeState() {
  ...
  try {
    ...
    console.debug(
      "[gstack sidebar] mem-toast snooze save failed:",
      err && err.message,
    );
  }
}
  ...
  closeBtn.onclick = async () => {
    ...
    for (const id of ids) {
      try {
        ...
        console.warn(
          "[gstack sidebar] mem-toast closetab failed:",
          id,
          err && err.message,
        );
      }
...
function evaluateMemToast(snapshot) {
  ...
  if (tabs.length >= 200 && tabs.length > memToastSnooze.tabsAbove) {
    const top5 = [...tabs]
      .sort((a, b) => tabRamScore(b) - tabRamScore(a))
      .slice(0, 5);
    showMemToast(`${tabs.length} tabs open — close some?`, "", top5);
    return;
  }
    ...
    snooze.addEventListener("click", async () => {
      ...
      try {
        ...
        if (resp.ok) {
          ...
          const maxHeap = tabs.reduce(
            (m, t) => Math.max(m, t.jsHeapUsed || 0),
            0,
          );
        ...
        console.debug(
          "[gstack sidebar] mem-toast snooze fetch failed:",
          err && err.message,
        );
      }
      dismissMemToast();
...
function inspectorShowData(data) {
  ...
  const cls =
    data.classes && data.classes.length > 0 ? "." + data.classes.join(".") : "";
...
function renderBoxModel(data) {
  ...
  if (!box) {
    inspectorBoxmodel.innerHTML =
      '<span class="inspector-no-data">No box model data</span>';
    return;
  }
...
function renderMatchedRules(data) {
  ...
  if (rules.length === 0) {
    inspectorRules.innerHTML =
      '<div class="inspector-no-data">No matched rules</div>';
    return;
  }
...
function renderRule(rule, isUA) {
  ...
  for (const prop of props) {
    ...
    const truncatedVal =
      valText.length > 30 ? valText.slice(0, 30) + "..." : valText;
    const priority =
      prop.priority === "important"
        ? ' <span class="inspector-important">!important</span>'
        : "";
...
function renderComputedStyles(data) {
  ...
  if (keys.length === 0) {
    inspectorComputed.innerHTML =
      '<div class="inspector-no-data">No computed styles</div>';
    return;
  }
...
function renderQuickEdit(data) {
  ...
  // Bind click-to-edit
  inspectorQuickedit
    .querySelectorAll(".inspector-quickedit-value")
    .forEach((el) => {
...
function startQuickEdit(valueEl) {
  ...
  const currentVal =
    valueEl.textContent === "(none)" ? "" : valueEl.textContent;
...
inspectorSendBtn.addEventListener("click", async () => {
  ...
  if (inspectorModifications.length > 0) {
    ...
    const diffs = inspectorModifications
      .map((m) => `  ${m.property}: ${m.value} (selector: ${m.selector})`)
      .join("\n");
    message = `CSS Inspector modifications:\n\nSelector: ${inspectorData.selector}\n\nChanges:\n${diffs}`;

    // Include source file info if available
    const rules =
      inspectorData.matchedRules || inspectorData.basicData?.matchedRules || [];
    const sources = rules
      .filter((r) => r.source && r.source !== "inline")
      .map((r) => r.source);
    if (sources.length > 0) {
  ...
  const verdict = await window.gstackScanForPTYInject?.(
    message + "\n",
    "inspector-send",
  );
  if (verdict?.verdict === "BLOCK") {
    console.warn(
      "[gstack sidebar] Inspector send BLOCKED by /pty-inject-scan:",
      verdict.reasons,
    );
    return;
  }
  ...
  if (!ok) {
    console.warn(
      "[gstack sidebar] Inspector send needs an active Terminal session.",
    );
  }
});
...
async function runCleanup(...buttons) {
  ...
  if (verdict?.verdict === "BLOCK") {
    console.warn(
      "[gstack sidebar] Cleanup BLOCKED by /pty-inject-scan:",
      verdict.reasons,
    );
    setTimeout(
...
const inspectorScreenshotBtn = document.getElementById(
  "inspector-screenshot-btn",
);
...
if (inspectorCleanupBtn)
  inspectorCleanupBtn.addEventListener("click", () =>
    runCleanup(inspectorCleanupBtn, chatCleanupBtn),
  );
if (inspectorScreenshotBtn)
  inspectorScreenshotBtn.addEventListener("click", () =>
    runScreenshot(inspectorScreenshotBtn, chatScreenshotBtn),
  );
if (chatCleanupBtn)
  chatCleanupBtn.addEventListener("click", () =>
    runCleanup(chatCleanupBtn, inspectorCleanupBtn),
  );
if (chatScreenshotBtn)
  chatScreenshotBtn.addEventListener("click", () =>
    runScreenshot(chatScreenshotBtn, inspectorScreenshotBtn),
  );
  ...
  try {
    ...
    inspectorSSE.addEventListener("inspectResult", (e) => {
      try {
        ...
        console.error(
          "[gstack sidebar] Failed to parse inspectResult:",
          err.message,
        );
      }
    });
...
function setActionButtonsEnabled(enabled) {
  const btns = document.querySelectorAll(
    ".quick-action-btn, .inspector-action-btn",
  );
  btns.forEach((btn) => {
...
function updateConnection(url, token) {
  ...
  if (url) {
    try {
      window.gstackServerPort = parseInt(new URL(url).port, 10);
    } catch {}
...
document.getElementById("conn-copy").addEventListener("click", () => {
  navigator.clipboard.writeText("/open-gstack-browser").then(() => {
    ...
    setTimeout(() => {
      btn.textContent = "/open-gstack-browser";
    }, 2000);
  });
});
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === "browserTabState") {
    document.dispatchEvent(
      new CustomEvent("gstack:tab-state", {
