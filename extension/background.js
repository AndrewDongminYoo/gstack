...
function setConnected(healthData) {
  ...
  chrome.runtime
    .sendMessage({ type: "health", data: healthData })
    .catch((err) => {
      console.debug(
        "[gstack bg] No listener for health broadcast:",
        err.message,
      );
    });
...
function setDisconnected() {
  ...
  chrome.runtime.sendMessage({ type: "health", data: null }).catch((err) => {
    console.debug(
      "[gstack bg] No listener for disconnect broadcast:",
      err.message,
    );
  });
...
async function notifyContentScripts(type) {
  try {
    ...
    console.error(
      "[gstack bg] Failed to query tabs for notification:",
      err.message,
    );
  }
}
...
async function injectInspector(tabId) {
  ...
  try {
    ...
    try {
      ...
      console.debug(
        "[gstack bg] Inspector CSS injection failed (non-fatal):",
        err.message,
      );
    }
    ...
    try {
      ...
      console.error(
        "[gstack bg] Inspector injection failed completely:",
        err.message,
        "| Basic fallback:",
        err2.message,
      );
...
async function stopInspector(tabId) {
  try {
    ...
    console.debug(
      "[gstack bg] Failed to stop picker on tab",
      tabId,
      ":",
      err.message,
    );
...
async function postInspectorPick(selector, frameInfo, basicData, activeTabUrl) {
  ...
  try {
    ...
    console.debug(
      "[gstack bg] Inspector pick server unavailable, using basic mode:",
      err.message,
    );
    return { mode: "basic", selector, basicData, frameInfo };
  }
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === "getTabState") {
    snapshotTabs().then((snap) =>
      sendResponse(snap || { active: null, tabs: [] }),
    );
  ...
  if (msg.type === "elementPicked") {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      const frameInfo = msg.frameSrc
        ? { frameSrc: msg.frameSrc, frameName: msg.frameName }
        : null;
      postInspectorPick(
        msg.selector,
        frameInfo,
        ...
        chrome.runtime
          ...
          .catch((err) => {
            console.warn(
              "[gstack bg] Failed to forward inspectResult to sidepanel:",
              err.message,
            );
          });
        sendResponse({ ok: true });
  ...
  if (msg.type === "pickerCancelled") {
    chrome.runtime.sendMessage({ type: "pickerCancelled" }).catch((err) => {
      console.debug(
        "[gstack bg] No listener for pickerCancelled:",
        err.message,
      );
    });
    return;
  ...
  if (msg.type === "command") {
    executeCommand(msg.command, msg.args).then((result) =>
      sendResponse(result),
    );
    return true;
  }
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      fetch(`${base}/sidebar-command`, {
        ...
        .then((r) => {
          if (!r.ok) {
            console.error(
              `[gstack bg] sidebar-command failed: ${r.status} ${r.statusText}`,
            );
            return r
              .json()
              .catch(() => ({ error: `Server returned ${r.status}` }));
          }
          return r.json();
        })
...
async function autoOpenSidePanel() {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      console.log(
        `[gstack] Side panel open attempt ${attempt + 1} failed:`,
        e.message,
      );
    }
    // Backoff: 500ms, 1000ms, 2000ms, 3000ms, 5000ms
    await new Promise((r) =>
      setTimeout(r, [500, 1000, 2000, 3000, 5000][attempt]),
    );
  }
...
async function snapshotTabs() {
  try {
    ...
    return {
      active: active
        ? { tabId: active.id, url: active.url || "", title: active.title || "" }
        : null,
      tabs: slim,
    };
  } catch {
  ...
  loadPort().then(() => {
    ...
    const startupCheck = setInterval(async () => {
      ...
      if (isConnected || startupAttempts >= 15) {
        ...
        if (!isConnected) {
          console.log(
            "[gstack] Startup health checks failed after 15 attempts, falling back to 10s polling",
          );
        }
      }
    }, 1000);
