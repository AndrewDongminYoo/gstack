    ...
    fetch(req) {
      ...
      if (req.method === "POST" && url.pathname === "/api/feedback") {
        return (async () => {
          ...
          const feedbackFile = isSubmit
            ? "feedback.json"
            : "feedback-pending.json";
          fs.writeFileSync(
            path.join(tmpDir, feedbackFile),
...
describe("Submit: browser click → feedback.json on disk", () => {
  test("clicking Submit writes feedback.json that the agent can poll for", async () => {
    ...
    const httpDetected = await handleReadCommand(
      "js",
  ...
  test("post-submit: inputs disabled, success message shown", async () => {
    ...
    const submitBtnExists = await handleReadCommand(
      "js",
    ...
    const successVisible = await handleReadCommand(
      "js",
    ...
    const successText = await handleReadCommand(
      "js",
...
describe("Regenerate: browser click → feedback-pending.json on disk", () => {
  ...
  test("board shows spinner after regenerate (user stays on same tab)", async () => {
    ...
    const bodyText = await handleReadCommand(
      "js",
