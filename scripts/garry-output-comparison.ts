...
const EXCLUDED_REPOS: Record<string, string> = {
  "tax-app":
    "demo app for an upcoming YC channel video, not production shipping work",
};
...
type PerYearResult = {
  ...
  per_day_rate: {
    // per calendar day (incl. non-active days)
    logical: number;
    raw: number;
    commits: number;
  };
  annualized_projection: {
    // per_day_rate × 365 — what the year looks like if pace holds
    logical: number;
    raw: number;
    commits: number;
  };
  ...
  return {
    ...
    caveats:
      commits.length === 0
        ...
        : isPartial
          ? [
              `Year ${year} is partial (day ${days} of 365). Run-rate multiple extrapolates current pace.`,
            ]
          : [],
  };
}
...
function main() {
  ...
  const repoRoot =
    repoRootIdx >= 0 && args[repoRootIdx + 1]
      ? args[repoRootIdx + 1]
      : process.cwd();
  ...
  if (!sccAvailable) {
    printSccHint();
    process.stderr.write(
      "Continuing with regex-based logical-line classification (an approximation).\n\n",
    );
  }
  ...
  const now = new Date();
  const years = TARGET_YEARS.map((y) =>
    analyzeRepo(repoRoot, y, authorEmails, sccAvailable, now),
  );
  ...
  const toDate = {
    logical_lines_added:
      y2013?.active && y2013.logical_lines_added > 0 && y2026?.active
        ? +(y2026.logical_lines_added / y2013.logical_lines_added).toFixed(1)
        : null,
    raw_lines_added:
      y2013?.active && y2013.raw_lines_added > 0 && y2026?.active
        ? +(y2026.raw_lines_added / y2013.raw_lines_added).toFixed(1)
        : null,
    commits:
      y2013?.active && y2013.commits > 0 && y2026?.active
        ? +(y2026.commits / y2013.commits).toFixed(1)
        : null,
    files_touched:
      y2013?.active && y2013.files_touched > 0 && y2026?.active
  ...
  if (toDate.logical_lines_added !== null) {
    process.stderr.write(
      `TO-DATE multiple (raw volume):  ${toDate.logical_lines_added}× logical, ${toDate.raw_lines_added}× raw\n`,
    );
  }
  ...
  if (toDate.logical_lines_added === null && runRate.logical_per_day === null) {
    process.stderr.write(
      `No multiple computable (one or both years inactive in this repo).\n`,
    );
  }
}
