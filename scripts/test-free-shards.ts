...
const WINDOWS_FRAGILE_PATTERNS: Array<{ pattern: RegExp; reason: string }> = [
  ...
  {
    pattern: /['"`]\/bin\/(?:ba)?sh/,
    reason: "hardcoded /bin/sh or /bin/bash",
  },
  {
    pattern: /spawnSync\(['"]sh['"],|spawn\(['"]sh['"],|exec\(['"]sh /,
    reason: 'spawn("sh", ...)',
  },
  ...
  {
    pattern: /\.mode\s*&\s*0o[0-7]+/,
    reason:
      "POSIX file mode bitmask (mode & 0o600 etc — Windows fakes mode bits)",
  },
  {
    pattern: /\.endsWith\(['"]\//,
    reason: "hardcoded forward-slash path assertion (Windows uses \\\\)",
  },
  {
    pattern: /['"]\.\/[a-zA-Z][^"']*['"]\)\s*\.\s*toBe\(true\)/,
    reason: "forward-slash path comparison",
  },
  ...
  //   - path.join(ROOT, 'bin')                       — bare BIN constant (brain-sync)
  {
    ...
    reason:
      "spawns bin/ shebang script (Windows CreateProcess does not parse shebangs)",
  },
  ...
  {
    pattern: /await\s+\w+\.launch\(/,
    reason:
      "launches Playwright browser (Chromium not installed in windows-free CI)",
  },
  ...
  {
    pattern: /BROWSE_HEADLESS_SKIP|spawn\(\[['"]bun['"],\s*['"]run['"]/,
    reason:
      "spawns the browse server subprocess (Bun-driven path is Windows-broken)",
  },
  ...
  {
    pattern: /sidebar-agent\.ts/,
    reason:
      "reads deleted browse/src/sidebar-agent.ts (pre-existing breakage from v1.14.0.0 sidebar refactor)",
  },
];
...
const KNOWN_WINDOWS_INCOMPATIBLE: Array<{ file: string; reason: string }> = [
  {
    ...
    reason:
      'asserts "claude" binary on PATH (only true when running inside Claude Code, not on bare CI runner)',
  },
  {
    file: "browse/test/findport.test.ts",
    reason:
      "asserts Bun.serve.stop() is fire-and-forget — Bun behavior differs on Windows for this polyfill",
  },
];
 ...
 */
export function detectWindowsFragility(
  absolutePath: string,
): { reason: string } | null {
  let content: string;
  try {
...
export function curateWindowsSafe(
  files: string[],
  rootDir = ROOT,
): CurationResult {
  const safe: string[] = [];
  ...
  const knownBad = new Map(
    KNOWN_WINDOWS_INCOMPATIBLE.map((e) => [e.file, e.reason]),
  );
...
function main(): number {
  ...
  if (options.windowsOnly) {
    ...
    console.log(
      `[test:free] curated ${files.length} Windows-safe tests (${curationReport.excluded.length} excluded)`,
    );
  ...
  if (options.shardIndex !== null) {
    if (
      ...
      options.shardIndex > shards.length
    ) {
      throw new Error(
