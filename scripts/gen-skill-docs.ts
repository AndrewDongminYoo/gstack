...
function effectiveSuppressedResolvers(hostConfig: HostConfig): Set<string> {
  ...
  if (GBRAIN_OVERRIDE.detected) {
    list = list.filter(
      (r) => r !== "GBRAIN_CONTEXT_LOAD" && r !== "GBRAIN_SAVE_RESULTS",
    );
  }
  return new Set(list);
}
...
const MODEL_ARG_VAL: Model = (() => {
  ...
  const val = MODEL_ARG.includes("=")
    ? MODEL_ARG.split("=")[1]
    : process.argv[process.argv.indexOf(MODEL_ARG) + 1];
...
const CATALOG_MODE_ARG = process.argv.find((a) =>
  a.startsWith("--catalog-mode"),
);
...
const EXPLAIN_LEVEL_ARG = process.argv.find((a) =>
  a.startsWith("--explain-level"),
);
...
export function splitCatalogDescription(description: string): CatalogParts {
  ...
  let working = voiceLine
    ? description.replace(voiceLine, "").trim()
    : description.trim();
  ...
  const sentenceLead = sentenceMatch
    ? sentenceMatch[1].trim()
    : collapsed.split(/\s/).slice(0, 20).join(" ");
  ...
  if (collapsedLeadIdx >= 0) {
    ...
    for (
      let i = 0;
      i < working.length && consumed < collapsedLeadIdx + sentenceLead.length;
      i++
    ) {
      if (/\s/.test(working[i])) {
 ...
 */
export function applyCatalogTrim(
  content: string,
  skillName: string,
): { content: string; parts: CatalogParts } | null {
  ...
  let descText: string;
  if (
    descMatch[0].startsWith("description: |") ||
    /^description:\s*\|/.test(descMatch[0])
  ) {
...
function buildContext(
  ...
  const skillName =
    skillNameOverride || extractedName || path.basename(path.dirname(tmplPath));
  ...
  const interactiveMatch = tmplContent.match(
    /^interactive:\s*(true|false)\s*$/m,
  );
  const interactive = interactiveMatch
    ? interactiveMatch[1] === "true"
    : undefined;
  return {
    skillName,
    tmplPath,
...
function processTemplate(
  ...
  content: string;
  symlinkLoop?: boolean;
  catalogParts?: CatalogParts | null;
} {
  ...
  if (OUT_DIR && host === "claude") {
    outputPath = path.join(
      OUT_DIR,
      skillDir,
      path.basename(tmplPath).replace(/\.tmpl$/, ""),
    );
  }
  ...
  let content = resolvePlaceholders(
    tmplContent,
    ctx,
    currentHostConfig,
    relTmplPath,
...
function processSectionTemplate(
  ...
  const parentContent = fs.existsSync(parentTmplPath)
    ? fs.readFileSync(parentTmplPath, "utf-8")
    : "";
  const parentName =
    (parentContent && extractNameAndDescription(parentContent).name) ||
    skillDir;
  const ctx = buildContext(
    parentContent || tmplContent,
    parentTmplPath,
    host,
    parentName,
  );
  ...
  content =
    GENERATED_HEADER.replace("{{SOURCE}}", path.basename(sectionTmplPath)) +
    content;
...
for (const currentHost of hostsToRun) {
  ...
  try {
    ...
    const proactiveAggregate: Record<
      string,
      {
        lead: string;
        routing: string;
    ...
    for (const tmplPath of findTemplates()) {
      ...
      const { outputPath, content, symlinkLoop, catalogParts } =
        processTemplate(tmplPath, currentHost);
      if (catalogParts) {
      ...
      if (content.length > TOKEN_CEILING_BYTES) {
        console.warn(
          `⚠️  TOKEN CEILING: ${relOutput} is ${content.length} bytes (~${tokens} tokens), exceeds ${TOKEN_CEILING_BYTES} byte ceiling (~40K tokens)`,
        );
      }
    }
    ...
    for (const sec of currentHost === "claude"
      ? discoverSectionTemplates(ROOT)
      : []) {
      if (
        currentHostConfig.generation.includeSkills?.length &&
        !currentHostConfig.generation.includeSkills.includes(sec.skillDir)
      )
        continue;
      if (
        currentHostConfig.generation.skipSkills?.length &&
        currentHostConfig.generation.skipSkills.includes(sec.skillDir)
      )
        continue;

      const { outputPath, content } = processSectionTemplate(
        path.join(ROOT, sec.tmpl),
        sec.skillDir,
        currentHost,
      );
      const relOutput = path.relative(OUT_DIR || ROOT, outputPath);
...
`;
    ...
    if (
      currentHost === "claude" &&
      CATALOG_MODE === "trim" &&
      Object.keys(proactiveAggregate).length > 0 &&
      !DRY_RUN &&
      !OUT_DIR
    ) {
      const proactivePath = path.join(
        ROOT,
        "scripts",
      ...
      let existing = "";
      try {
        existing = fs.readFileSync(proactivePath, "utf-8");
      } catch {
        /* first run */
      }
