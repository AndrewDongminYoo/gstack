 ...
 */
async function readDesignCommands(): Promise<
  Record<string, { category: string; description: string; usage?: string }>
> {
...
export async function generateLlmsTxt(
  ...
  for (const t of templates) {
    ...
    if (!entry) {
      warnings.push(
        `skill ${t.tmpl}: missing name or description in frontmatter`,
      );
      if (opts.strict) {
        throw new Error(
  ...
  if (designCommands.length > 0) {
    ...
    lines.push("");
    lines.push(
      "Run with `design <command> [args]`. Full reference: `design/SKILL.md`.",
    );
    lines.push("");
...
export async function writeLlmsTxt(
  opts: GenerateOptions & { outputPath?: string } = {},
): Promise<GenerateResult> {
  ...
  void (async () => {
    ...
    if (dryRun) {
      ...
      if (existing !== result.content) {
        console.error(
          "[gen-llms-txt] OUT OF DATE — run `bun run gen:skill-docs` to regenerate gstack/llms.txt",
        );
        process.exit(1);
      }
      ...
      console.log(
        `[gen-llms-txt]   skills=${result.skills.length} browse=${result.browseCommands.length} design=${result.designCommands.length}`,
      );
    }
  })();
}
