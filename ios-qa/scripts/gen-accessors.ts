...
function parseFields(body: string): AccessorField[] {
  ...
  while ((m = fieldPattern.exec(normalized)) !== null) {
    ...
    const afterMatch = normalized
      .slice(afterMatchIdx, afterMatchIdx + 4)
      .trim();
...
export function computeCacheKey(inputs: {
  ...
  h.update(
    `swift=${inputs.swiftVersion}|tool=${inputs.toolGitRev}|platform=${inputs.platformTriple}|`,
  );
...
function detectSwiftVersion(): string {
  ...
  try {
    const out = execSync("swift --version", {
...
export function defaultCacheRoot(): string {
  return (
    process.env.GSTACK_IOS_CACHE_ROOT ??
    join(homedir(), ".gstack", "cache", "gen-accessors")
  );
}
...
if (import.meta.main) {
  ...
  if (inputIdx === -1) {
    process.stderr.write(
      "usage: gen-accessors --input <dir> [--output <dir>]\n",
    );
    process.exit(2);
  }
