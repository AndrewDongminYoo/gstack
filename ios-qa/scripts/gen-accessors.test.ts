...
`;
    ...
    expect(specs[0]!.fields.map((f) => f.name)).toEqual([
      "isLoggedIn",
      "username",
    ]);
    expect(
      specs[0]!.fields.find((f) => f.name === "isLoggedIn")!.typeText,
    ).toBe("Bool");
  });
...
`,
    ...
    expect(existsSync(join(cacheRoot, r.cacheKey, "StateAccessor.swift"))).toBe(
      true,
    );
  });
  ...
  test("modifying source invalidates the cache", () => {
    ...
    writeFileSync(
      file,
      '@Observable class A { @Snapshotable var y: String = "" }',
    );
    const r2 = generate({
      inputDir,
      cacheRoot,
