...
describe("daemon — loopback listener", () => {
  ...
  test("healthz returns 200 with mode=loopback", async () => {
    const r = await fetchWith(
      "GET",
      `http://127.0.0.1:${daemon.loopbackPort}/healthz`,
    );
  ...
  test("proxies /screenshot to stub StateServer with the rotated bearer", async () => {
    const r = await fetchWith(
      "GET",
      `http://127.0.0.1:${daemon.loopbackPort}/screenshot`,
    );
    expect(r.status).toBe(200);
    ...
    expect(lastReq?.headers["authorization"]).toBe(
      `Bearer ${STATE_SERVER_TOKEN}`,
    );
  });
  ...
  test("proxies X-Session-Id passthrough on /tap", async () => {
    const r = await fetchWith(
      "POST",
      `http://127.0.0.1:${daemon.loopbackPort}/tap`,
      {
        headers: {
          "x-session-id": "sess-loopback-1",
  ...
  test("returns 503 when no device tunnel is provided", async () => {
    ...
    try {
      const r = await fetchWith(
        "GET",
        `http://127.0.0.1:${d2.loopbackPort}/screenshot`,
      );
...
describe("daemon — tailnet listener (mocked tailscaled)", () => {
  ...
  test("tailnet listener refuses to open when probe fails", async () => {
    ...
    try {
      ...
      const r = await fetchWith(
        "GET",
        `http://127.0.0.1:${d.loopbackPort}/healthz`,
      );
      expect(r.status).toBe(200);
    } finally {
      await d.close();
  ...
  test("non-allowlisted endpoint returns 404 on tailnet", async () => {
    const r = await fetchWith(
      "GET",
      `http://127.0.0.1:${daemon.tailnetPort}/auth/sessions`,
    );
    expect(r.status).toBe(404);
    expect(JSON.parse(r.bodyText).error).toBe(
      "endpoint_not_in_tailnet_allowlist",
    );
  });
  ...
  test("/auth/mint rejects unknown identity (mocked WhoIs)", async () => {
    const r = await fetchWith(
      "POST",
      `http://127.0.0.1:${daemon.tailnetPort}/auth/mint`,
      {
  ...
  test("/auth/mint succeeds for allowlisted identity, then proxies are bearer-gated", async () => {
    ...
    const tapR = await fetchWith(
      "POST",
      `http://127.0.0.1:${daemon.tailnetPort}/tap`,
      {
        headers: {
    ...
    const tapNoAuth = await fetchWith(
      "POST",
      `http://127.0.0.1:${daemon.tailnetPort}/tap`,
      {
        headers: { "content-type": "application/json" },
  ...
  test("boot token never appears in tailnet responses", async () => {
    ...
    const { session_token } = JSON.parse(mintR.bodyText);
    const screenshotR = await fetchWith(
      "GET",
      `http://127.0.0.1:${daemon.tailnetPort}/screenshot`,
      {
