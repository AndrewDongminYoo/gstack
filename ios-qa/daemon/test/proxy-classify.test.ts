...
describe("classifyRoute", () => {
  test("healthz, screenshot, elements, snapshot are observe-tier", () => {
    ...
    expect(classifyRoute("GET", "/screenshot").requiredCapability).toBe(
      "observe",
    );
    expect(classifyRoute("GET", "/elements").requiredCapability).toBe(
      "observe",
    );
    expect(classifyRoute("GET", "/state/snapshot").requiredCapability).toBe(
      "observe",
    );
    expect(classifyRoute("GET", "/state/anyKey").requiredCapability).toBe(
      "observe",
    );
  });
  ...
  test("tap, swipe, type, session ops are interact-tier", () => {
    ...
    expect(classifyRoute("POST", "/session/acquire").requiredCapability).toBe(
      "interact",
    );
    expect(classifyRoute("POST", "/session/release").requiredCapability).toBe(
      "interact",
    );
    expect(classifyRoute("POST", "/session/heartbeat").requiredCapability).toBe(
      "interact",
    );
  });
  ...
  test("arbitrary state writes are mutate-tier", () => {
    expect(
      classifyRoute("POST", "/state/userIsLoggedIn").requiredCapability,
    ).toBe("mutate");
    expect(classifyRoute("POST", "/state/anyField").requiredCapability).toBe(
      "mutate",
    );
  });

  test("state/restore is restore-tier (highest)", () => {
    expect(classifyRoute("POST", "/state/restore").requiredCapability).toBe(
      "restore",
    );
  });

  test("mint endpoint is observe-tier (minimum bar to attempt mint)", () => {
    expect(classifyRoute("POST", "/auth/mint").requiredCapability).toBe(
      "observe",
    );
  });
