...
describe("SessionTokenStore", () => {
  ...
  test("validate rejects null/empty/unknown tokens", () => {
    const store = new SessionTokenStore();
    expect(store.validate(null, "observe")).toEqual({
      ok: false,
      reason: "no_token",
    });
    expect(store.validate("", "observe")).toEqual({
      ok: false,
      reason: "no_token",
    });
    expect(store.validate("bogus-token", "observe")).toEqual({
      ok: false,
  ...
  test("validate rejects expired tokens", () => {
    ...
    expect(store.validate(result.token, "observe")).toEqual({
      ok: false,
  ...
  test("validate rejects tokens with insufficient capability", () => {
    ...
    expect(store.validate(r.token, "interact")).toEqual({
      ok: false,
      reason: "capability_insufficient",
    });
    expect(store.validate(r.token, "mutate")).toEqual({
      ok: false,
      reason: "capability_insufficient",
    });
    expect(store.validate(r.token, "restore")).toEqual({
      ok: false,
      reason: "capability_insufficient",
    });
  });
  ...
  test("revoke removes a token", () => {
    ...
    expect(store.validate(r.token, "observe")).toEqual({
      ok: false,
      reason: "invalid_token",
    });
  });
  ...
  test("revokeByIdentity removes all tokens for one identity", () => {
    ...
    if ("error" in a1 || "error" in a2 || "error" in b1)
