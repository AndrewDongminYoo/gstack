  ...
  return (cmd: string, args: string[]) => {
    ...
    if (!script.argsMatch.test(joined)) {
      return makeReturn(
        1,
        "",
        `unexpected call shape: ${joined} (expected ${script.argsMatch})`,
      );
    }
    idx++;
  ...
  test("returns device_locked when launchApp errors due to lock", async () => {
    const spawn = makeSpawn([
      ...
      },
      {
        ...
        stderr:
          'Locked ("Unable to launch com.test because the device was not, or could not be, unlocked").',
        exitCode: 1,
      },
    ]);
    const r = await bootstrapTunnel({ bundleId: "com.test", spawnImpl: spawn });
        ...
        jsonOutput: {
          result: {
            runningProcesses: [
              {
                executable:
                  "file:///private/var/containers/Bundle/Application/.../com.test.app/com.test",
  ...
  test("happy path: returns DeviceTunnel with rotated token", async () => {
    const spawn = makeSpawn([
        ...
        jsonOutput: {
          result: {
            runningProcesses: [
              {
                executable:
                  "file:///var/containers/Bundle/Application/X/com.test.app/com.test",
      ...
      },
      {
        ...
        argsMatch: /devicectl device info details/,
        jsonOutput: {
          result: { connectionProperties: { tunnelIPAddress: "fd99::beef" } },
        },
      },
      {
  ...
  test("respects explicit udid when set", async () => {
    const spawn = makeSpawn([
      ...
      },
      {
        ...
        jsonOutput: {
          result: { connectionProperties: { tunnelIPAddress: "fd00::b" } },
        },
      },
      {
...
describe("getDeviceTunnelIPv6FromDevicectl", () => {
    ...
    const spawn = makeSpawn([
      {
        ...
        jsonOutput: {
          result: {
            connectionProperties: { tunnelIPAddress: "fde4:2827:528e::1" },
          },
        },
      },
    ]);
    expect(getDeviceTunnelIPv6FromDevicectl("TEST-UDID", spawn)).toBe(
      "fde4:2827:528e::1",
    );
  });
  ...
  test("falls back to result.tunnel.ipAddress when connectionProperties absent", () => {
    ...
    expect(getDeviceTunnelIPv6FromDevicectl("UDID", spawn)).toBe(
      "fd00::dead:beef",
    );
  });
    ...
    const spawn = makeSpawn([
      {
        argsMatch: /devicectl device info details/,
        jsonOutput: {
          result: { connectionProperties: { tunnelIPAddress: "fd11::1" } },
        },
      },
    ]);
