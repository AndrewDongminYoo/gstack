...
describe("Allowlist", () => {
  ...
  test("findEntry matches exact identity", async () => {
    const list = {
      ...
      entries: [
        {
          identity: "user@example.com",
          capabilities: ["mutate" as const],
          expires_at: null,
        },
      ],
    };
    expect(findEntry(list, "user@example.com")?.identity).toBe(
      "user@example.com",
    );
  ...
  test("hasCapability is tier-aware", async () => {
    const list = {
      ...
      entries: [
        {
          identity: "restore-user",
          capabilities: ["restore" as const],
          expires_at: null,
        },
        {
