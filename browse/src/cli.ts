...
export function buildRestartEnv(
  ...
  if (globalFlags?.headed || oldState?.mode === "headed")
    env.BROWSE_HEADED = "1";
...
async function startServer(
  ...
  const parentPid =
    parseInt(process.env.BROWSE_PARENT_PID || "", 10) === 0
      ? "0"
      : String(process.pid);
...
async function ensureServer(flags?: GlobalFlags): Promise<ServerState> {
  ...
  if (state && (await isServerHealthy(state.port))) {
    ...
    if (
      desiredHash &&
      !state.configHash &&
      (flags?.proxyUrl || flags?.headed)
    ) {
      console.error(
        `[browse] existing daemon was started without --proxy/--headed.`,
      );
      console.error(
        `[browse] run 'browse disconnect' first to apply new flags.`,
      );
      process.exit(1);
    }
  ...
  if (process.env.BROWSE_NO_AUTOSTART === "1") {
    console.error(
      "[browse] Server not available and BROWSE_NO_AUTOSTART is set.",
    );
    console.error(
      "[browse] The headed browser may have been closed. Run /open-gstack-browser to restart.",
    );
    process.exit(1);
  }
 ...
 */
export function extractTabId(args: string[]): {
...
async function sendCommand(
  ...
  try {
    ...
    if (
      ...
      if (
        oldState?.pid &&
        isProcessAlive(oldState.pid) &&
        (await probeHealthWithBackoff(oldState.port))
      ) {
        if (retries >= 1)
          throw new Error(
        ...
        console.error(
          "[browse] Server was briefly unresponsive (busy); retrying command...",
        );
      ...
      const newState = await startServer(
        Object.keys(restartEnv).length ? restartEnv : undefined,
      );
...
export function extractGlobalFlags(
  ...
  if (proxyUrl) {
    ...
    rebuilt.password = parsed.password
...
async function handlePairAgent(
  ...
  if (pairData.tunnel_url) {
    ...
    try {
      ...
      if (cliProbe.ok) {
        ...
        console.warn(
          `[browse] Tunnel returned HTTP ${cliProbe.status}, attempting restart...`,
        );
        pairData.tunnel_url = null; // fall through to restart logic
      }
    } catch {
      console.warn(
        "[browse] Tunnel unreachable from CLI, attempting restart...",
      );
      pairData.tunnel_url = null; // fall through to restart logic
    }
  }
    ...
    if (ngrokAvailable) {
      ...
      try {
        const tunnelResp = await fetch(
          `http://127.0.0.1:${state.port}/tunnel/start`,
          {
            method: "POST",
        ...
        if (tunnelResp.ok && tunnelData.url) {
          ...
          console.warn(
            `[browse] Tunnel failed: ${tunnelData.error || "unknown error"}`,
          );
      ...
      console.warn(
        "[browse] No tunnel active and ngrok is not installed/configured.",
      );
      console.warn(
        "[browse] Instructions will use localhost (same-machine only).",
      );
      console.warn(
  ...
  if (localHost) {
    try {
      ...
      console.error(
        `[browse] Failed to write config for ${localHost}: ${err.message}`,
      );
      process.exit(1);
    }
    return;
...
Refs:           After 'snapshot', use @e1, @e2... as selectors:
  ...
  if (command === "connect") {
    ...
    if (
      ...
      try {
        const resp = await fetch(
          `http://127.0.0.1:${existingState.port}/health`,
          {
            signal: AbortSignal.timeout(2000),
          },
        );
    ...
    try {
      ...
      const serverEnv: Record<string, string> = {
        ...
        ...(globalFlags.proxyUrl
          ? { BROWSE_PROXY_URL: globalFlags.proxyUrl }
          : {}),
        ...(globalFlags.configHash
          ? { BROWSE_CONFIG_HASH: globalFlags.configHash }
          : {}),
      };
      ...
      if (process.platform === "darwin") {
        console.log(
          "(If you still don't see it, check Mission Control / other Spaces.)",
        );
      }
      ...
      try {
        ...
        console.error(
          `[browse] Terminal agent failed to start: ${err.message}`,
        );
      }
    } catch (err: any) {
    ...
    const superviseRequested =
      commandArgs.includes("--supervise") ||
    ...
    const SUPERVISOR_BACKOFF_MS = (
      process.env.GSTACK_SUPERVISOR_BACKOFF || "1000,2000,4000,8000,30000"
    )
      .split(",")
    ...
    while (!supervisorExiting) {
      ...
      while (
        respawns.length &&
        now - respawns[0] > SUPERVISOR_GUARD_WINDOW_MS
      ) {
        respawns.shift();
      }
      ...
      const backoff =
        SUPERVISOR_BACKOFF_MS[
          Math.min(attempt, SUPERVISOR_BACKOFF_MS.length - 1)
        ] ?? 30_000;
      console.warn(
      ...
      try {
        ...
        console.log(
          `[browse] Supervisor: server respawned (PID ${respawned.pid}, port ${respawned.port}).`,
        );
        ...
        try {
          ...
          console.warn(
            `[browse] Supervisor: terminal-agent respawn failed: ${err?.message || err}`,
          );
        }
      } catch (err: any) {
        console.error(
          `[browse] Supervisor: server respawn failed: ${err?.message || err}`,
        );
  ...
  if (command === "disconnect") {
    ...
    const hasCustomConfig =
      existingState &&
      (existingState.mode === "headed" || existingState.configHash);
    ...
    if (existingState.mode === "headed") {
      try {
        const resp = await fetch(
          `http://127.0.0.1:${existingState.port}/command`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  if (command === "pair-agent") {
    ...
    if (state.mode !== "headed" && !hasFlag(commandArgs, "--headless")) {
      console.log(
        "[browse] Opening GStack Browser so you can see what the remote agent does...",
      );
      ...
      if (newState && (await isServerHealthy(newState.port))) {
        ...
        console.warn(
          "[browse] Could not switch to headed mode. Continuing headless.",
        );
      }
    }
