...
export const CDP_ALLOWLIST: ReadonlyArray<CdpAllowEntry> = Object.freeze([
  // ─── Accessibility (read-only) ─────────────────────────────
  {
    ...
    output: "untrusted",
    justification:
      "Read-only AX tree extraction. Output is third-party page content; wrap in UNTRUSTED.",
  },
  {
    domain: "Accessibility",
    ...
    output: "untrusted",
    justification:
      "Read-only AX tree subtree by node. Output is third-party page content.",
  },
  {
    domain: "Accessibility",
  ...
  },
  {
    ...
    output: "trusted",
    justification:
      "Pure geometric data (box dimensions). No page content leaks; safe trusted.",
  },
  {
    domain: "DOM",
  ...
  // ─── CSS (read-only) ───────────────────────────────────────
  {
    ...
    output: "untrusted",
    justification:
      "Read computed cascade for a node; output may contain attacker-controlled selectors.",
  },
  {
    domain: "CSS",
    ...
    output: "trusted",
    justification:
      "Computed style values are bounded (CSS keywords/numbers); safe trusted.",
  },
  {
    domain: "CSS",
    ...
    output: "untrusted",
    justification:
      "Inline style content may contain attacker-controlled custom-property values.",
  },
  // ─── Performance metrics ───────────────────────────────────
  {
    domain: "Performance",
  ...
  },
  {
    ...
    output: "trusted",
    justification:
      "Domain enable; no content; required prerequisite for getMetrics.",
  },
  {
    domain: "Performance",
    method: "disable",
  ...
  // We mark it browser-scoped to acquire the global lock when in use.
  {
    ...
    output: "trusted",
    justification:
      "Trace category capture. Browser-scoped to serialize against other CDP ops.",
  },
  {
    domain: "Tracing",
    method: "end",
  ...
  },
  {
    ...
    output: "trusted",
    justification:
      "UA override on the active tab. NOTE: changes affect future requests; fine for tests.",
  },
  // ─── Page capture (output, not navigation) ─────────────────
  {
    ...
    output: "untrusted",
    justification:
      "Screenshot bytes; output is bounded image data (no marker injection vector).",
  },
  {
    domain: "Page",
    method: "printToPDF",
  ...
  // ─── Network metadata (NOT bodies/cookies — those exfil data) ──
  {
    ...
    output: "trusted",
    justification:
      "Domain enable; required prerequisite. Does not return data.",
  },
  {
    domain: "Network",
    method: "disable",
    ...
    output: "trusted",
    justification:
      "Domain disable; mirrors Network.enable for cleanup symmetry.",
  },
  ...
  // Only a tiny safe subset:
  {
    ...
    output: "untrusted",
    justification:
      "Inspect properties of an existing remote object. Read-only; output may contain page data.",
  },
]);
