 ...
 */
export function defaultTierPaths(
  ...
  return {
    project: projectRoot
      ? path.join(projectRoot, ".gstack", "browser-skills")
      : null,
...
export function parseSkillFile(
  ...
  const frontmatter: SkillFrontmatter = {
    ...
    source:
      fm.source === "agent" || fm.source === "human" ? fm.source : undefined,
  };
...
function collectArgsList(
  lines: string[],
  from: number,
): { items: SkillArg[]; consumed: number } {
  ...
  for (const { tier, root } of order) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const { frontmatter, bodyMd } = parseSkillFile(content, {
          skillName: entry,
        });
 ...
 * in any tier.
 */
export function readBrowserSkill(
  name: string,
  tiers?: TierPaths,
): BrowserSkill | null {
  const t = tiers ?? defaultTierPaths();
 ...
 */
export function tombstoneBrowserSkill(
  name: string,
  tier: "project" | "global",
  tiers?: TierPaths,
): string {
  ...
  if (!root) {
    throw new Error(
      `tombstoneBrowserSkill: tier "${tier}" has no resolved path`,
    );
  }
