...
export async function handleCookiePickerRoute(
  ...
  try {
    ...
    if (pathname === "/cookie-picker" && req.method === "GET") {
      ...
      if (code) {
        ...
        if (!expiry || Date.now() > expiry) {
          pendingCodes.delete(code);
          return new Response(
            "Invalid or expired code. Re-run cookie-import-browser.",
            {
              status: 403,
      ...
      return new Response(
        "Access denied. Open the cookie picker from gstack.",
        {
          status: 403,
          headers: { "Content-Type": "text/plain" },
        },
    ...
    const hasBearer =
      !!authToken && !!authHeader && authHeader === `Bearer ${authToken}`;
    ...
    if (pathname === "/cookie-picker/import" && req.method === "POST") {
      ...
      if (
        ...
        result.failed > 0 &&
        hasV20Cookies(browser, selectedProfile)
      ) {
        console.log(
          `[cookie-picker] v20 App-Bound Encryption detected, trying CDP extraction...`,
        );
        try {
