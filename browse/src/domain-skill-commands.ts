...
function formatSkillListing(list: {
  project: DomainSkillRow[];
  global: DomainSkillRow[];
}): string {
...
async function handleSave(args: string[], bm: BrowserManager): Promise<string> {
  ...
  if (filterResult.blocked) {
    logTelemetry({
      event: "domain_skill_save_blocked",
      host,
...
async function handleEdit(args: string[]): Promise<string> {
  ...
  const list = await listSkills(slug);
  const current = [...list.project, ...list.global].find(
    (r) => r.host === host,
  );
  if (!current) {
    throw new Error(
  ...
  const tmpFile = path.join(
    os.tmpdir(),
    `gstack-domain-skill-${process.pid}-${Date.now()}.md`,
  );
...
export async function handleDomainSkillCommand(
  args: string[],
  bm: BrowserManager,
): Promise<string> {
  const sub = args[0];
