...
function assertJsOriginAllowed(bm: BrowserManager, pageUrl: string): void {
  ...
  const allowed = [...importedDomains].some((domain) => {
    ...
    return (
      hostname === domain.replace(/^\./, "") || hostname.endsWith(normalized)
    );
  });
  ...
  switch (command) {
      ...
      if (selector) {
        ...
        if ("locator" in resolved) {
          return stripLoneSurrogates(
            await resolved.locator.innerHTML({ timeout: 5000 }),
          );
        }
        return stripLoneSurrogates(
          await target.locator(resolved.selector).innerHTML({ timeout: 5000 }),
        );
      }
    ...
    case "cookies": {
      ...
      const redacted = cookies.map((c) => {
        if (
          SENSITIVE_COOKIE_NAME.test(c.name) ||
          SENSITIVE_COOKIE_VALUE.test(c.value)
        ) {
        ...
        ({ wantJsonLd, wantOg, wantTwitter, wantMeta }) => {
          ...
          if (wantTwitter) {
            ...
            document.querySelectorAll('meta[name^="twitter:"]').forEach((m) => {
              const name =
                m.getAttribute("name")?.replace("twitter:", "") || "";
