...
export async function handleSkillCommand(
  ...
  switch (sub) {
    ...
    default:
      throw new Error(
        `Unknown skill subcommand: "${sub}". Try: list, show, run, test, rm.`,
      );
  }
}
...
function handleRm(args: string[], ctx: SkillCommandContext): string {
  ...
  const effectiveTier: "project" | "global" =
    tier === "project" && !tiers.project ? "global" : tier;
 ...
 */
export async function spawnSkill(
  ...
  try {
    ...
    const proc = Bun.spawn(
      ["bun", "run", scriptPath, "--", ...opts.skillArgs],
      {
        cwd: opts.skill.dir,
        env,
        stdout: "pipe",
