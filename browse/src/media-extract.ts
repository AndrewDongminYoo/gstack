...
export async function extractMedia(
  ...
  const result = await target.evaluate(
    ({ scopeSelector, filter }) => {
      ...
      if (!filter || filter === "videos") {
        ...
        vids.forEach((vid, i) => {
          const sources = Array.from(vid.querySelectorAll("source")).map(
            (s) => ({
              src: s.src || "",
              type: s.type || "",
            }),
          );
          const isHLS = sources.some(
            (s) => s.type.includes("mpegURL") || s.src.includes(".m3u8"),
          );
          const isDASH = sources.some(
            (s) => s.type.includes("dash") || s.src.includes(".mpd"),
          );
          videos.push({
            index: i,
            src: vid.src || "",
    ...
    {
      scopeSelector: options?.selector || null,
  ...
  return {
    ...
    total:
      ...
      result.audio.length +
      result.backgroundImages.length,
  };
}
