...
export async function handleWriteCommand(
  ...
  switch (command) {
    ...
    case "load-html": {
      ...
      if (fromFilePayload) {
        const MAX_BYTES =
          parseInt(process.env.GSTACK_BROWSE_MAX_HTML_BYTES || "", 10) ||
          50 * 1024 * 1024;
        ...
        await session.setTabContent(fromFilePayload.html, {
      ...
      if (stat.isDirectory()) {
        throw new Error(
          `load-html: ${absolutePath} is a directory, not a file. Pass a .html file.`,
        );
      }
      if (!stat.isFile()) {
    ...
    case "scroll": {
      ...
      const waitMs =
        waitIdx >= 0 ? parseInt(args[waitIdx + 1], 10) || 1000 : 1000;
      const selector = args.find(
        (a) =>
          !a.startsWith("--") &&
          args.indexOf(a) !== timesIdx + 1 &&
          args.indexOf(a) !== waitIdx + 1,
      );
    ...
    case "wait": {
      ...
      if (selector === "--networkidle") {
        ...
        const timeout = Math.min(
          Math.max(
            args[1] ? parseInt(args[1], 10) || MIN_WAIT_MS : 15000,
            MIN_WAIT_MS,
          ),
          MAX_WAIT_MS,
        );
      ...
      for (let i = 0; i < args.length; i++) {
        if (args[i] === "--scale") {
          ...
          if (val === undefined || val === "") {
            throw new Error(
              "viewport --scale: missing value. Usage: viewport [WxH] --scale <n>",
            );
          }
          const parsed = Number(val);
          if (!Number.isFinite(parsed)) {
            throw new Error(
              `viewport --scale: value '${val}' is not a finite number.`,
            );
          }
          scaleArg = parsed;
        } else if (args[i].startsWith("--")) {
          ...
          throw new Error(
            `Unexpected positional arg: ${args[i]}. Usage: viewport [WxH] [--scale <n>]`,
          );
        }
      }
    ...
    case "cookie-import": {
      ...
      for (const c of cookies) {
        ...
        if (!c.domain) {
          ...
          if (
            ...
            !defaultDomain.endsWith("." + cookieDomain)
          ) {
            throw new Error(
      ...
      const importedDomains = [
        ...new Set(cookies.map((c: any) => c.domain).filter(Boolean)),
      ];
    ...
    case "cookie-import-browser": {
      ...
      if (domainIdx !== -1 && domainIdx + 1 < args.length) {
        ...
        const normalizedDomain = domain.startsWith(".")
          ...
          : domain;
        if (
          normalizedDomain !== pageHostname &&
          !pageHostname.endsWith("." + normalizedDomain)
        ) {
          throw new Error(
      ...
      if (hasAll) {
        ...
        msg.push(
          "(used --all: all browser cookies imported, consider --domain for tighter scoping)",
        );
        if (result.failed > 0) msg.push(`(${result.failed} failed to decrypt)`);
        return msg.join(" ");
      }
    ...
    case "style": {
      ...
      if (args[0] === "--undo") {
        ...
        return idx !== undefined
          ? `Reverted modification #${idx}`
          : "Reverted last modification";
      }
      ...
      const DANGEROUS_CSS =
        /url\s*\(|expression\s*\(|@import|javascript:|data:/i;
    ...
    case "cleanup": {
      ...
      const scrollFixed = await page.evaluate(() => {
        ...
        const truncated = document.querySelectorAll(
          '[class*="truncat"], [class*="preview"], [class*="teaser"]',
        );
    ...
    case "prettyscreenshot": {
      ...
      if (scrollTo) {
        ...
        if (!scrolled) {
          ...
          throw new Error(
            `Could not find element or text to scroll to: ${scrollTo}`,
          );
        }
    ...
    case "download": {
      ...
      if (url.startsWith("@")) {
        ...
        if (tagName === "img") {
          ...
          url = await locator.evaluate(
            (el) =>
              (el as HTMLAudioElement).currentSrc ||
      ...
      if (url.startsWith("blob:")) {
        ...
        if (dataUrl.startsWith("ERROR:EXPIRED"))
          throw new Error(
            `Blob URL expired or inaccessible: ${dataUrl.slice("ERROR:EXPIRED:".length)}`,
          );
      ...
      if (isBase64) {
        if (buffer.length > 10 * 1024 * 1024) {
          throw new Error(
            "File too large for --base64 (>10MB). Use disk download + GET /file instead.",
          );
        }
      ...
      const destPath =
        outputPath ||
        path.join(TEMP_DIR, `browse-download-${Date.now()}${ext}`);
    ...
    case "scrape": {
      if (args.length === 0)
        throw new Error(
          "Usage: scrape <images|videos|media> [--selector sel] [--dir path] [--limit N]",
        );
      ...
      const dir =
        dirIdx >= 0
          ? args[dirIdx + 1]
          : path.join(TEMP_DIR, `browse-scrape-${Date.now()}`);
      const limitIdx = args.indexOf("--limit");
      const limit = Math.min(
        limitIdx >= 0 ? parseInt(args[limitIdx + 1], 10) || 50 : 50,
        200,
      );
      ...
      for (let i = 0; i < toDownload.length; i++) {
        ...
        try {
          ...
          manifest.succeeded++;
          lines.push(
            `  [${i + 1}/${toDownload.length}] ${filename} (${Math.round(body.length / 1024)}KB)`,
          );
        } catch (err: any) {
          manifest.files.push({
            path: null,
          ...
          manifest.failed++;
          lines.push(
            `  [${i + 1}/${toDownload.length}] FAILED: ${err.message}`,
          );
        }
    ...
    case "archive": {
      const page = bm.getPage();
      const outputPath =
        args[0] || path.join(TEMP_DIR, `browse-archive-${Date.now()}.mhtml`);
      validateOutputPath(outputPath);
