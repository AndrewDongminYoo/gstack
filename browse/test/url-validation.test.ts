...
describe("validateNavigationUrl", () => {
  it("allows http URLs", async () => {
    await expect(validateNavigationUrl("http://example.com")).resolves.toBe(
      "http://example.com",
    );
  });
  ...
  it("allows localhost", async () => {
    await expect(validateNavigationUrl("http://localhost:3000")).resolves.toBe(
      "http://localhost:3000",
    );
  });

  it("allows 127.0.0.1", async () => {
    await expect(validateNavigationUrl("http://127.0.0.1:8080")).resolves.toBe(
      "http://127.0.0.1:8080",
    );
  });

  it("allows private IPs", async () => {
    await expect(validateNavigationUrl("http://192.168.1.1")).resolves.toBe(
      "http://192.168.1.1",
    );
  });
  ...
  it("rejects unsupported file URL host (UNC/network paths)", async () => {
    await expect(
      validateNavigationUrl("file://host.example.com/foo.html"),
    ).rejects.toThrow(/Unsupported file URL host/i);
  });
  ...
  it("blocks metadata IP in hex form", async () => {
    await expect(validateNavigationUrl("http://0xA9FEA9FE/")).rejects.toThrow(
      /cloud metadata/i,
    );
  });

  it("blocks metadata IP in decimal form", async () => {
    await expect(validateNavigationUrl("http://2852039166/")).rejects.toThrow(
      /cloud metadata/i,
    );
  });
  ...
  it("blocks metadata IP in octal form", async () => {
    await expect(
      validateNavigationUrl("http://0251.0376.0251.0376/"),
    ).rejects.toThrow(/cloud metadata/i);
  });

  it("blocks IPv6 metadata with brackets (fd00::)", async () => {
    await expect(validateNavigationUrl("http://[fd00::]/")).rejects.toThrow(
      /cloud metadata/i,
    );
  });

  it("blocks IPv6 ULA fd00::1 (not just fd00::)", async () => {
    await expect(validateNavigationUrl("http://[fd00::1]/")).rejects.toThrow(
      /cloud metadata/i,
    );
  });
  ...
  it("blocks IPv6 ULA fd12:3456::1", async () => {
    await expect(
      validateNavigationUrl("http://[fd12:3456::1]/"),
    ).rejects.toThrow(/cloud metadata/i);
  });

  it("blocks IPv6 ULA fc00:: (full fc00::/7 range)", async () => {
    await expect(validateNavigationUrl("http://[fc00::]/")).rejects.toThrow(
      /cloud metadata/i,
    );
  });

  it("blocks direct IPv6 link-local addresses", async () => {
    await expect(validateNavigationUrl("http://[fe80::2]/")).rejects.toThrow(
      /cloud metadata/i,
    );
  });
  ...
  it("does not block hostnames starting with fd (e.g. fd.example.com)", async () => {
    await expect(
      validateNavigationUrl("https://fd.example.com/"),
    ).resolves.toBe("https://fd.example.com/");
  });

  it("does not block hostnames starting with fc (e.g. fcustomer.com)", async () => {
    await expect(validateNavigationUrl("https://fcustomer.com/")).resolves.toBe(
      "https://fcustomer.com/",
    );
  });
...
describe("validateNavigationUrl — restoreState coverage", () => {
  ...
  it("blocks chrome:// URLs that could appear in saved state", async () => {
    await expect(validateNavigationUrl("chrome://settings")).rejects.toThrow(
      /scheme.*not allowed/i,
    );
  });
  ...
  it("allows normal https URLs from saved state", async () => {
    await expect(
      validateNavigationUrl("https://example.com/page"),
    ).resolves.toBe("https://example.com/page");
  });

  it("allows localhost URLs from saved state", async () => {
    await expect(
      validateNavigationUrl("http://localhost:3000/app"),
...
describe("normalizeFileUrl", () => {
  ...
  it("passes through absolute file:/// URLs unchanged", () => {
    expect(normalizeFileUrl("file:///tmp/page.html")).toBe(
      "file:///tmp/page.html",
    );
  });
  ...
  it("passes through file://localhost/<abs> unchanged", () => {
    expect(normalizeFileUrl("file://localhost/tmp/page.html")).toBe(
      "file://localhost/tmp/page.html",
    );
  });
  ...
  it("rejects dotted host-like segment file://docs.v1/page.html", () => {
    expect(() => normalizeFileUrl("file://docs.v1/page.html")).toThrow(
      /Unsupported file URL host/i,
    );
  });

  it("rejects IP-like host file://127.0.0.1/foo", () => {
    expect(() => normalizeFileUrl("file://127.0.0.1/tmp/x")).toThrow(
      /Unsupported file URL host/i,
    );
  });

  it("rejects IPv6 host file://[::1]/foo", () => {
    expect(() => normalizeFileUrl("file://[::1]/tmp/x")).toThrow(
      /Unsupported file URL host/i,
    );
  });

  it("rejects Windows drive letter file://C:/Users/x", () => {
    expect(() => normalizeFileUrl("file://C:/Users/x")).toThrow(
      /Unsupported file URL host/i,
    );
  });
