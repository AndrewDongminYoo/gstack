...
const WRITE_SRC = fs.readFileSync(
  path.join(import.meta.dir, "../src/write-commands.ts"),
  "utf-8",
);
...
const PATH_SECURITY_SRC = fs.readFileSync(
  path.join(import.meta.dir, "../src/path-security.ts"),
  "utf-8",
);
...
const CDP_SRC = fs.readFileSync(
  path.join(import.meta.dir, "../src/cdp-inspector.ts"),
  "utf-8",
);
  ...
  describe("behavioral checks", () => {
    ...
    it("realpathSync on symlink-to-/etc resolves to /etc (out of safe dirs)", () => {
      ...
      const isSafe = safeDirs.some(
        (d) => passwdReal === d || passwdReal.startsWith(d + path.sep),
      );
    ...
    it("meta-commands validateOutputPath rejects paths outside safe dirs", async () => {
      const mod = await import("../src/meta-commands.ts");
      expect(() => mod.validateOutputPath("/home/user/secret.png")).toThrow(
        /Path must be within/,
      );
      expect(() => mod.validateOutputPath("/var/log/access.log")).toThrow(
        /Path must be within/,
      );
    });
  });
...
describe("Round-2 finding 2: snapshot.ts annotated path uses realpathSync", () => {
  it("snapshot.ts annotated screenshot section contains realpathSync", () => {
    ...
    const annotateBlock = SNAPSHOT_SRC.slice(
      annotateStart,
      annotateStart + 2000,
    );
...
describe("Task 11: state load cookie validation", () => {
  it("state load block filters cookies by domain and type", () => {
    const block = sliceBetween(
      META_SRC,
      "action === 'load'",
      "throw new Error('Usage: state save|load",
    );
...
describe("Task 13: inbox output wrapped as untrusted content", () => {
  ...
  it("inbox handler applies wrapUntrustedContent to userMessage", () => {
    ...
    expect(block).toMatch(
      /wrapUntrustedContent.*userMessage|userMessage.*wrapUntrustedContent/,
    );
  });
  ...
  it("inbox handler applies wrapUntrustedContent to url", () => {
    ...
    expect(block).toMatch(
      /wrapUntrustedContent.*msg\.url|msg\.url.*wrapUntrustedContent/,
    );
  });
