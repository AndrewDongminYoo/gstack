...
describe("Tab Isolation", () => {
  ...
  describe("checkTabAccess", () => {
    ...
    it("own-only scoped agent CANNOT read an unowned tab", () => {
      expect(
        bm.checkTabAccess(1, "agent-1", { isWrite: false, ownOnly: true }),
      ).toBe(false);
    });

    it("own-only scoped agent CANNOT write to an unowned tab", () => {
      expect(
        bm.checkTabAccess(1, "agent-1", { isWrite: true, ownOnly: true }),
      ).toBe(false);
    });
    ...
    it("own-only scoped agent can read its own tab", () => {
      ...
      expect(
        bm.checkTabAccess(1, "someone-else", { isWrite: false, ownOnly: true }),
      ).toBe(false);
    });

    it("own-only scoped agent CANNOT write to another agent tab", () => {
      expect(
        bm.checkTabAccess(1, "agent-2", { isWrite: true, ownOnly: true }),
      ).toBe(false);
    });
  });
