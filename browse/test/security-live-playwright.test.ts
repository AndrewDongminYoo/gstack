...
describe("defense-in-depth — live Playwright fixture", () => {
  ...
  test("L2 — content-security.ts hidden-element stripper detects the .sneaky div", async () => {
    ...
    const sneakyMatch = stripped.some(
      (s) =>
        s.toLowerCase().includes("opacity") ||
        s.toLowerCase().includes("off-screen"),
    );
  ...
  test("L2b — content-security.ts ARIA regex catches the injected aria-label", async () => {
    ...
    const stripped = await markHiddenElements(page);
    const ariaHits = stripped.filter((s) =>
      s.toLowerCase().includes("aria injection"),
    );
  ...
  test("L3 — content-security.ts URL blocklist fires on multiple exfil domains", async () => {
    ...
    const result = urlBlocklistFilter(
      html,
      `${baseUrl}/injection-combined.html`,
      "html",
    );
    ...
    const distinctDomains = new Set(
      result.warnings
        .map(
          (w) =>
            (w.match(/(webhook\.site|pipedream\.com|requestbin\.com)/) ??
              [])[1],
        )
        .filter(Boolean),
    );
  ...
  test("defense-in-depth — all deterministic layers (L1-L3) fire on the combined fixture", async () => {
    ...
    const stripped = await markHiddenElements(page);
    const html = await page.content();
    const urlResult = urlBlocklistFilter(
      html,
      `${baseUrl}/injection-combined.html`,
      "html",
    );
    ...
    // L2b: ARIA regex
    const ariaCount = stripped.filter((s) =>
      s.toLowerCase().includes("aria injection"),
    ).length;
  ...
  test.skipIf(!ML_AVAILABLE)(
    ...
    async () => {
      ...
      const { loadTestsavant, scanPageContent } =
        await import("../src/security-classifier");
  ...
  test.skipIf(!ML_AVAILABLE)(
    ...
    async () => {
      const benign =
        "Premium Widget. $29.99. High-quality widget with premium features. Add to Cart.";
      const { loadTestsavant, scanPageContent } =
        await import("../src/security-classifier");
      await loadTestsavant();
