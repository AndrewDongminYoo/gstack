...
async function waitForReady(
  ...
  throw new Error(
    `Daemon did not become ready within ${timeoutMs}ms at ${baseUrl}`,
  );
}
...
async function waitForTunnelPort(
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (typeof state.tunnelLocalPort === "number")
        return state.tunnelLocalPort;
    } catch {
  ...
  throw new Error(
    `Tunnel local port did not appear in ${stateFile} within ${timeoutMs}ms`,
  );
}
...
describe("pair-agent over tunnel surface — gate fires on the right surface only", () => {
  ...
  test("newtab on tunnel surface passes the allowlist gate (not 403 disallowed_command)", async () => {
    const { status, bodyText } = await postCommand(
      daemon.tunnelUrl,
      daemon.scopedToken,
      { command: "newtab" },
    );
  ...
  test("catch-22 regression: newtab + goto on the just-created tab passes ownership check", async () => {
    ...
    const newtabResp = await postCommand(daemon.tunnelUrl, daemon.scopedToken, {
      command: "newtab",
    });
    ...
    expect(gotoResp.bodyText).not.toContain(
      "is not allowed over the tunnel surface",
    );
  });
});
