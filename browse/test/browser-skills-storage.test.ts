...
describe("listBrowserSkills", () => {
  ...
  it("skips entries without SKILL.md", () => {
    ...
    fs.writeFileSync(
      path.join(tiers.bundled, "no-skill-md", "README"),
      "nothing here",
    );
    expect(listBrowserSkills(tiers)).toEqual([]);
  });
  ...
  it("skips dotfiles and .tombstones", () => {
    ...
    fs.mkdirSync(path.join(tiers.global, ".tombstones", "old-skill"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tiers.global, ".tombstones", "old-skill", "SKILL.md"),
      "---\nname: x\nhost: y\n---\n",
    );
    expect(listBrowserSkills(tiers)).toEqual([]);
  });
  ...
  it("skips malformed SKILL.md silently (best-effort listing)", () => {
    ...
    fs.writeFileSync(
      path.join(tiers.bundled, "broken", "SKILL.md"),
      "no frontmatter",
    );
...
describe("tombstoneBrowserSkill", () => {
  ...
  it("throws when skill not found in target tier", () => {
    expect(() => tombstoneBrowserSkill("nope", "global", tiers)).toThrow(
      /not found/,
    );
  });
