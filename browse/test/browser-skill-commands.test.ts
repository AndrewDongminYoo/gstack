...
describe("handleSkillCommand: show", () => {
  it("prints SKILL.md", async () => {
    ...
    const result = await handleSkillCommand(["show", "foo"], {
      port: 9999,
      tiers,
    });
  ...
  it("throws when skill missing", async () => {
    await expect(
      handleSkillCommand(["show", "nope"], { port: 9999, tiers }),
    ).rejects.toThrow(/not found/);
  });

  it("throws when name omitted", async () => {
    await expect(
      handleSkillCommand(["show"], { port: 9999, tiers }),
...
describe("handleSkillCommand: rm", () => {
  it("tombstones global skill by default", async () => {
    ...
    const result = await handleSkillCommand(["rm", "gone", "--global"], {
      port: 9999,
      tiers,
    });
  ...
  it("tombstones project skill", async () => {
    ...
    const result = await handleSkillCommand(["rm", "gone"], {
      port: 9999,
      tiers,
    });
    expect(result).toContain("Tombstoned");
...
describe("handleSkillCommand: help / unknown", () => {
  ...
  it("throws on unknown subcommand", async () => {
    await expect(
      handleSkillCommand(["frobnicate"], { port: 9999, tiers }),
...
describe.skipIf(SKIP_SPAWN)("spawnSkill: lifecycle", () => {
  it("happy path: returns stdout, exit 0, token revoked", async () => {
    ...
    expect(
      listTokens().filter((t) => t.clientId.startsWith("skill:echo-skill:")),
    ).toEqual([]);
  });
  ...
  it("timeout fires, exit code 124, token revoked", async () => {
    ...
    expect(
      listTokens().filter((t) => t.clientId.startsWith("skill:sleeper:")),
    ).toEqual([]);
  }, 10_000);
