 ...
 */
async function installStubsBeforeLoad(
  page: Page,
  scenario: {
    healthSecurity?: {
      ...
      if (url.endsWith("/health")) {
        return new Response(
          JSON.stringify({
            ...
            session: null,
            security: scenarioRef.healthSecurity ?? {
              status: "degraded",
              layers: {},
              lastUpdated: "",
            },
...
describe("sidepanel security DOM", () => {
  test.skipIf(!CHROMIUM_AVAILABLE)(
    ...
    async () => {
      ...
      await page.waitForFunction(
        () =>
          document
            .getElementById("security-shield")
            ?.getAttribute("data-status") === "protected",
        { timeout: 5000 },
      );
      const status = await page.$eval("#security-shield", (el) =>
        el.getAttribute("data-status"),
      );
      expect(status).toBe("protected");
      // aria-label carries human-readable state
      const aria = await page.$eval("#security-shield", (el) =>
    ...
    async () => {
      ...
      await page.waitForFunction(
        () =>
          document
            .getElementById("security-shield")
            ?.getAttribute("data-status") === "degraded",
        { timeout: 5000 },
      );
      const status = await page.$eval("#security-shield", (el) =>
        el.getAttribute("data-status"),
  ...
  test.skipIf(!CHROMIUM_AVAILABLE)(
    ...
    async () => {
      ...
      const initialHidden = await page.$eval(
      ...
      const expandedHidden = await page.$eval(
        "#security-banner-details",
        (el) => (el as HTMLElement).hidden,
      );
