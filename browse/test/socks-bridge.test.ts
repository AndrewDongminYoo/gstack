...
describe("startSocksBridge", () => {
  ...
  test("rejects connection when upstream auth fails", async () => {
    ...
    try {
      await expect(
        socks5NoAuthConnect(bridge.port, "127.0.0.1", 80),
      ).rejects.toThrow();
    } finally {
      await bridge.close();
      await upstream.close();
  ...
  test("handles SOCKS5 handshake split across multiple TCP packets (codex finding)", async () => {
    ...
    try {
      ...
      const readAtLeast = async (
        ...
        throw new Error(
          `timeout waiting for ${n} bytes (have ${inbox.reduce((s, b) => s + b.length, 0)})`,
        );
      };
  ...
  test("exhausts retries and throws on bad creds", async () => {
    ...
    try {
      await expect(
        ...
        /SOCKS5 upstream rejected or unreachable after 3 attempts/,
      );
    } finally {
      await upstream.close();
    }
