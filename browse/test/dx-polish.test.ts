...
describe("buildUnknownCommandError", () => {
  ...
  it("appends upgrade hint when command appears in NEW_IN_VERSION", () => {
    ...
    const noLoadHtml = new Set(
      [...ALL_COMMANDS].filter((c) => c !== "load-html"),
    );
    const msg = buildUnknownCommandError(
      "load-html",
      noLoadHtml,
      COMMAND_ALIASES,
      NEW_IN_VERSION,
...
describe("Alias + SCOPE_WRITE integration invariant", () => {
  ...
  it("setcontent is NOT directly in any scope set (must canonicalize first)", async () => {
    const { SCOPE_WRITE, SCOPE_READ, SCOPE_ADMIN, SCOPE_CONTROL } =
      await import("../src/token-registry");
