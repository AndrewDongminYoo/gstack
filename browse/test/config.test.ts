...
describe("config", () => {
  ...
  describe("resolveConfig", () => {
    test("uses git root by default", () => {
      ...
      expect(config.stateFile).toBe(
        path.join(gitRoot!, ".gstack", "browse.json"),
      );
    });
    ...
    test("log paths are in stateDir", () => {
      const config = resolveConfig({});
      expect(config.consoleLog).toBe(
        path.join(config.stateDir, "browse-console.log"),
      );
      expect(config.networkLog).toBe(
        path.join(config.stateDir, "browse-network.log"),
      );
      expect(config.dialogLog).toBe(
        path.join(config.stateDir, "browse-dialog.log"),
      );
    });
  });
  ...
  describe("ensureStateDir", () => {
    ...
    test("is a no-op if directory already exists", () => {
      ...
      const config = resolveConfig({
        BROWSE_STATE_FILE: path.join(stateDir, "browse.json"),
      });
    ...
    test("adds .gstack/ to .gitignore if not present", () => {
      const tmpDir = path.join(
        os.tmpdir(),
        `browse-gitignore-test-${Date.now()}`,
      );
...
describe("resolveServerScript", () => {
  ...
  test("uses BROWSE_SERVER_SCRIPT env when set", () => {
    const result = resolveServerScript(
  ...
  test("throws when server.ts cannot be found", () => {
    expect(() =>
      resolveServerScript({}, "/nonexistent/$bunfs", "/nonexistent/browse"),
...
describe("startup error log", () => {
  test("write and read error log", () => {
    const tmpDir = path.join(
      os.tmpdir(),
      `browse-error-log-test-${Date.now()}`,
    );
    fs.mkdirSync(tmpDir, { recursive: true });
...
describe("resolveChromiumProfile", () => {
  test("explicit arg wins over env and default", () => {
    ...
    try {
      expect(resolveChromiumProfile("/tmp/explicit-profile")).toBe(
  ...
  test("falls back to resolveGstackHome()/chromium-profile when nothing set", () => {
    ...
    try {
      expect(resolveChromiumProfile()).toBe(
        "/tmp/fallback-gstack/chromium-profile",
      );
    } finally {
