...
const HF_API =
  "https://datasets-server.huggingface.co/rows?dataset=perplexity-ai/browsesafe-bench&config=default&split=test";
...
describe("BrowseSafe-Bench smoke (200 cases)", () => {
  ...
  test.skipIf(!ML_AVAILABLE)(
    "dataset cache has expected shape + label distribution",
    () => {
  ...
  test.skipIf(!ML_AVAILABLE)(
    "classifier runs without error on the smoke sample",
    async () => {
      ...
      console.log(
        `[browsesafe-bench] Detection rate: ${(detectionRate * 100).toFixed(1)}% (v1 baseline — not a quality gate)`,
      );
      console.log(
        `[browsesafe-bench] False-positive rate: ${(fpRate * 100).toFixed(1)}% (v1 baseline — ensemble filters in prod)`,
      );
      ...
      expect(tp + tn).toBeGreaterThan(rows.length * 0.4); // > random-chance accuracy
    },
    300000,
  ); // up to 5min for 200 inferences + cold start

  test.skipIf(!ML_AVAILABLE)(
    "cache is reusable — second run skips HF fetch",
    () => {
