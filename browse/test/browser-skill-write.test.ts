...
describe("stageSkill", () => {
  it("writes all files into the staged dir and returns the path", () => {
    ...
    expect(stagedDir).toBe(
      path.join(stagingTmpRoot, "skillify-aaaa1111-test", "test-skill"),
    );
    expect(fs.existsSync(path.join(stagedDir, "SKILL.md"))).toBe(true);
    expect(fs.existsSync(path.join(stagedDir, "script.ts"))).toBe(true);
    expect(
      fs.existsSync(path.join(stagedDir, "_lib", "browse-client.ts")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(stagedDir, "fixtures", "example-com-2026-04-27.html"),
      ),
    ).toBe(true);
    expect(
      fs.readFileSync(path.join(stagedDir, "script.ts"), "utf-8"),
    ).toContain("hi");
  });
...
describe("commitSkill", () => {
  ...
  it("refuses to clobber an existing skill at the same path", () => {
    ...
    fs.writeFileSync(
      path.join(tiers.global, "collide-skill", "marker.txt"),
      "existing\n",
    );
  ...
  it("throws when project tier is unresolved", () => {
    ...
    const tiersNoProject: TierPaths = {
      project: null,
      global: tiers.global,
      bundled: tiers.bundled,
    };
    expect(() =>
      commitSkill({
        name: "test-skill",
...
describe("D3 contract: simulated test failure leaves no on-disk artifact", () => {
  ...
  it("stage → user rejects in approval gate → discard → no skill at final path", () => {
    ...
    expect(fs.existsSync(path.join(tiers.global, "rejected-skill"))).toBe(
      false,
    );
  });
