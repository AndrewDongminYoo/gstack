...
const TMP_HOME = path.join(
  os.tmpdir(),
  `gstack-domain-e2e-${process.pid}-${Date.now()}`,
);
...
async function fakeBodyPipe(body: string): Promise<string> {
  ...
  const tmpFile = path.join(
    os.tmpdir(),
    `e2e-body-${process.pid}-${Date.now()}.md`,
  );
...
beforeAll(async () => {
  ...
  await fs.mkdir(path.join(TMP_HOME, "projects", "e2e-test-slug"), {
    recursive: true,
  });
...
describe("$B domain-skill (E2E gate tier)", () => {
  test("save: derives host from active tab, writes quarantined row, list surfaces it", async () => {
    const { handleDomainSkillCommand } =
      await import("../src/domain-skill-commands");
    // Navigate to a test page (host: 127.0.0.1 in this fixture server)
    await bm.getPage().goto(baseUrl + "/basic.html");

    const bodyFile = await fakeBodyPipe(
      "# Test skill\n\nThis page is the basic fixture.",
    );
    const out = await handleDomainSkillCommand(
      ["save", "--from-file", bodyFile],
      bm,
    );
    ...
    const lines = jsonl
      ...
      .split("\n")
      .map((l) => JSON.parse(l));
    const skill = lines.find(
      (r: any) => r.type === "domain" && r.host === "127.0.0.1",
    );
  ...
  test("readSkill returns null while quarantined; classifier_score=0 blocks auto-promote (#1369)", async () => {
    ...
    const lines = (await fs.readFile(jsonlPath, "utf8"))
      ...
      .map((l) => JSON.parse(l));
    const latest = lines
      .filter((r: any) => r.type === "domain" && r.host === "127.0.0.1")
      .pop();
  ...
  test("save without an active page errors with structured guidance", async () => {
    ...
    await expect(
      handleDomainSkillCommand(["save", "--from-file", bodyFile], bm),
    ).rejects.toThrow(/no top-level URL/);
    await fs.unlink(bodyFile).catch(() => {});
  });
});
