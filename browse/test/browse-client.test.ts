...
describe("browse-client", () => {
  ...
  describe("resolveBrowseAuth", () => {
    ...
    it("rejects GSTACK_PORT env values with trailing characters", () => {
      ...
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "browse-client-test-"),
      );
      try {
        expect(() =>
          resolveBrowseAuth({ stateFile: path.join(tmpDir, "missing.json") }),
    ...
    it("falls back to state file when env vars missing", () => {
      ...
      fs.writeFileSync(
        stateFile,
        JSON.stringify({ pid: 1, port: server.port, token: "root-token" }),
      );
      try {
    ...
    it("throws a clear error when neither env nor state file resolves", () => {
      ...
      try {
        expect(() =>
          resolveBrowseAuth({
            stateFile: path.join(tmpDir, "nonexistent.json"),
          }),
        ).toThrow("browse-client: cannot find daemon port + token");
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
  ...
  describe("convenience methods", () => {
    ...
    it("links/forms/accessibility take no args", async () => {
      ...
      expect(server.requests.map((r) => r.body.command)).toEqual([
        "links",
        "forms",
        "accessibility",
      ]);
