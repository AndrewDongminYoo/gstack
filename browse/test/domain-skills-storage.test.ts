...
const TMP_HOME = path.join(
  os.tmpdir(),
  `gstack-test-${process.pid}-${Date.now()}`,
);
...
beforeEach(async () => {
  ...
  await fs.mkdir(path.join(TMP_HOME, "projects", "test-slug"), {
    recursive: true,
  });
});
...
describe("domain-skills: hostname normalization (T3)", () => {
  ...
  it("strips protocol, path, query, fragment, and port", async () => {
    const m = await freshImport();
    expect(m.normalizeHost("https://docs.github.com:443/issues?x=1#hash")).toBe(
      "docs.github.com",
    );
  });
  ...
  it("preserves subdomain (subdomain-exact match)", async () => {
    ...
    expect(m.normalizeHost("docs.github.com")).not.toBe(
      m.normalizeHost("github.com"),
    );
  });
});
...
describe("domain-skills: scope shadowing (T4)", () => {
  ...
  it("global skill fires for project that has no override", async () => {
    const m = await freshImport();
    await fs.mkdir(path.join(TMP_HOME, "projects", "other-slug"), {
      recursive: true,
    });
    ...
    for (let i = 0; i < 3; i++)
      await m.recordSkillUse("stripe.com", "test-slug", false);
...
describe("domain-skills: rollback by version log", () => {
  ...
  it("rollback throws if only one version exists", async () => {
    ...
    await expect(
      m.rollbackSkill("a.com", "test-slug", "project"),
    ).rejects.toThrow(/fewer than 2 versions/);
  });
});
...
describe("domain-skills: deletion (tombstone)", () => {
  it("delete tombstones the skill; read returns null", async () => {
    ...
    for (let i = 0; i < 3; i++)
      await m.recordSkillUse("doomed.com", "test-slug", false);
    expect((await m.readSkill("doomed.com", "test-slug"))?.row.host).toBe(
      "doomed.com",
    );
