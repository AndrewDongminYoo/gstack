...
import {
  handleReadCommand as _handleReadCommand,
  parseOutArgs,
  hasOutArg,
  resultToString,
...
describe("parseOutArgs / hasOutArg", () => {
  test("--out <path> splits the flag from the positional", () => {
    expect(parseOutArgs(["expr", "--out", "/tmp/x"])).toEqual({
      outPath: "/tmp/x",
      raw: false,
      rest: ["expr"],
    });
  ...
  test("flag ordering does not matter", () => {
    expect(parseOutArgs(["--out", "/tmp/x", "expr"])).toEqual({
      outPath: "/tmp/x",
      raw: false,
      rest: ["expr"],
    });
  ...
  test("--raw and --raw=true|false", () => {
    ...
    expect(parseOutArgs(["e", "--out", "/tmp/x", "--raw=false"]).raw).toBe(
      false,
    );
  });

  test("repeated --out throws", () => {
    expect(() => parseOutArgs(["e", "--out", "/a", "--out", "/b"])).toThrow(
      /more than once/,
    );
  });
  ...
  test("--out with a missing value throws", () => {
    ...
    expect(() => parseOutArgs(["e", "--out", "--raw"])).toThrow(
  ...
  test("bad --raw value throws", () => {
    expect(() => parseOutArgs(["e", "--out", "/a", "--raw=maybe"])).toThrow(
...
describe("Inspection", () => {
  ...
  test("js --out decodes a base64 PNG data URL to real bytes", async () => {
    ...
    try {
      const result = await handleReadCommand(
        "js",
        [`'data:image/png;base64,' + '${b64}'`, "--out", out],
        bm,
      );
      const buf = fs.readFileSync(out);
      // PNG magic bytes: 89 50 4E 47
      expect([buf[0], buf[1], buf[2], buf[3]]).toEqual([
        0x89, 0x50, 0x4e, 0x47,
      ]);
  ...
  test("js --out throws on a malformed base64 data URL instead of writing corrupt bytes", async () => {
    ...
    try {
      await expect(
        handleReadCommand(
          "js",
          ["'data:image/png;base64,!!!not-base64!!!'", "--out", out],
          bm,
        ),
  ...
  test("js --out rejects a path outside the safe directories", async () => {
    await expect(
      handleReadCommand(
        "js",
        ["1 + 1", "--out", "/etc/browse-should-not-write.txt"],
        bm,
      ),
    ).rejects.toThrow();
  });
...
describe("Cookies and storage", () => {
  ...
  test("storage read redacts sensitive keys", async () => {
    ...
    await handleReadCommand(
      "storage",
      ["set", "auth_token", "my-secret-token"],
      bm,
    );
    await handleReadCommand("storage", ["set", "api_key", "key-12345"], bm);
    await handleReadCommand(
      "storage",
      ["set", "displayName", "normalValue"],
      bm,
    );
    const result = await handleReadCommand("storage", [], bm);
    const storage = JSON.parse(result);
  ...
  test("storage read redacts sensitive values by prefix", async () => {
    ...
    await handleReadCommand(
      "storage",
      ["set", "userData", "eyJhbGciOiJIUzI1NiJ9.payload.sig"],
      bm,
    );
    // GitHub PAT under innocuous key name
    await handleReadCommand(
      "storage",
      ["set", "repoAccess", "ghp_abc123def456"],
      bm,
    );
    const result = await handleReadCommand("storage", [], bm);
    const storage = JSON.parse(result);
...
describe("Cookie import", () => {
  ...
  test("cookie-import preserves explicit domain", async () => {
    ...
    const cookies = [
      { name: "explicit", value: "domain", domain: "127.0.0.1", path: "/foo" },
    ];
    fs.writeFileSync(tempFile, JSON.stringify(cookies));
...
describe("Frame", () => {
  test("frame switch to iframe and back", async () => {
    ...
    // Switch back to main
    const mainResult = await handleMetaCommand(
      "frame",
      ["main"],
      bm,
      async () => {},
...
describe("load-html", () => {
  ...
  const fixturePath = path.join(
    tmpDir,
    `browse-test-loadhtml-${Date.now()}.html`,
  );
  const fragmentPath = path.join(
    tmpDir,
    `browse-test-fragment-${Date.now()}.html`,
  );

  beforeAll(() => {
    fs.writeFileSync(
      fixturePath,
      '<html><body><h1 id="loaded">loaded by load-html</h1></body></html>',
    );
    fs.writeFileSync(
      fragmentPath,
      '<div class="fragment" style="width:100px;height:50px">fragment</div>',
    );
  });
  ...
  test("load-html rejects missing file with actionable error", async () => {
    try {
      await handleWriteCommand(
        "load-html",
        [path.join(tmpDir, "does-not-exist.html")],
        bm,
      );
      expect(true).toBe(false);
    } catch (err: any) {
  ...
  test("load-html rejects directory target", async () => {
    try {
      await handleWriteCommand(
        "load-html",
        [path.join(tmpDir, "browse-test-notafile.html") + "/"],
        bm,
      );
      expect(true).toBe(false);
    } catch (err: any) {
      ...
      expect(err.message).toMatch(
        /not found|directory|not a regular file|security policy/,
      );
    }
  });
  ...
  test("load-html strips UTF-8 BOM before magic-byte check", async () => {
    ...
    fs.writeFileSync(
      ...
      Buffer.concat([
        bomBytes,
        Buffer.from("<html><body>bom ok</body></html>"),
      ]),
    );
    try {
  ...
  test("load-html --wait-until networkidle exercises non-default branch", async () => {
    const result = await handleWriteCommand(
      "load-html",
      [fixturePath, "--wait-until", "networkidle"],
      bm,
    );
    expect(result).toContain("Loaded HTML:");
  });
...
describe("screenshot --selector", () => {
  ...
  test("--selector conflicts with --clip", async () => {
    ...
    try {
      await handleMetaCommand(
        "screenshot",
        ["--selector", "#title", "--clip", "0,0,100,100"],
        bm,
        async () => {},
      );
      expect(true).toBe(false);
...
describe("viewport --scale", () => {
  test("viewport WxH --scale 2 produces 2x dimension screenshot", async () => {
    ...
    fs.writeFileSync(
      tmpFix,
      '<div id="box" style="width:100px;height:50px;background:#f00"></div>',
    );
    try {
...
describe("Command aliases", () => {
  ...
  test("setcontent alias routes to load-html via chain", async () => {
    ...
    const result = await handleMetaCommand(
      "chain",
      [JSON.stringify([["setcontent", aliasFix]])],
      bm,
      async () => {},
    );
    expect(result).toContain("Loaded HTML:");
  ...
  test("set-content (hyphenated) alias also routes", async () => {
    const result = await handleMetaCommand(
      "chain",
      [JSON.stringify([["set-content", aliasFix]])],
      bm,
      async () => {},
    );
    expect(result).toContain("Loaded HTML:");
