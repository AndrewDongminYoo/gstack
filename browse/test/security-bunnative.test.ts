...
const TOKENIZER_AVAILABLE = fs.existsSync(
  path.join(MODEL_DIR, "tokenizer.json"),
);

describe("bun-native tokenizer", () => {
  test.skipIf(!TOKENIZER_AVAILABLE)(
    "loads HF tokenizer.json into a WordPiece state",
    async () => {
  ...
  test.skipIf(!TOKENIZER_AVAILABLE)(
    "encodes simple English into [CLS] ... [SEP] frame",
    async () => {
      const { loadHFTokenizer, encodeWordPiece } =
        await import("../src/security-bunnative");
      const tok = loadHFTokenizer(MODEL_DIR);
  ...
  test.skipIf(!TOKENIZER_AVAILABLE)(
    "matches transformers.js for a regression set",
    async () => {
      ...
      for (const text of fixtures) {
        ...
        const refIds = Array.from(refIdsTensor?.data ?? []).map((x: any) =>
          Number(x),
        );
...
describe("bun-native benchmark harness", () => {
  test.skipIf(!TOKENIZER_AVAILABLE)(
    "benchClassify returns well-shaped latency report",
    async () => {
