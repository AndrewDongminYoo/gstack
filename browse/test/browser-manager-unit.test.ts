...
describe("shouldEnableChromiumSandbox", () => {
  ...
  it("darwin, no CI/CONTAINER/root → true", async () => {
    ...
    const { shouldEnableChromiumSandbox } =
      await import("../src/browser-manager");
...
describe("stealth injected on every context-creation path", () => {
  it("every context-creation path calls applyStealth (>= 4 call sites)", async () => {
    ...
    const src = readFileSync(
      join(import.meta.dir, "..", "src", "browser-manager.ts"),
      "utf-8",
    );
    ...
    const resumeAnchor = src.indexOf(
      "Resume AI control after user handoff",
      handoffStart,
    );
    const handoffBody = src.slice(
      handoffStart,
      resumeAnchor > 0 ? resumeAnchor : handoffStart + 4000,
    );
    ...
    const recreateBody = src.slice(
      recreateDef,
      setUaAnchor > 0 ? setUaAnchor : recreateDef + 6000,
    );
  ...
  it("STEALTH_IGNORE_DEFAULT_ARGS is wired into both persistent-context paths", async () => {
    ...
    const sites =
      src.match(/ignoreDefaultArgs:\s*STEALTH_IGNORE_DEFAULT_ARGS/g) || [];
