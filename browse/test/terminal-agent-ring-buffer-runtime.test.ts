...
describe("buildReplayPayload runtime", () => {
  ...
  test("re-enters alt-screen when session was in alt-screen at detach", () => {
    ...
    expect(payload.indexOf("\x1b[!p")).toBeLessThan(
      payload.indexOf("\x1b[?1049h"),
    );
    expect(payload.indexOf("\x1b[?1049h")).toBeLessThan(
      payload.indexOf("tool output"),
    );
  });
...
describe("lease lifecycle interplay (via pty-session-lease)", () => {
  ...
  test("lease registry is independent of ring buffer state", async () => {
    const { mintLease, validateLease, __resetLeases } =
      await import("../src/pty-session-lease");
    __resetLeases();
    const a = mintLease();
    const b = mintLease();
