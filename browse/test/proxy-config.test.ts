...
describe("parseProxyConfig", () => {
  ...
  test("rejects malformed URL", () => {
    expect(() => parseProxyConfig({ proxyUrl: "not-a-url" })).toThrow(
      ProxyConfigError,
    );
  });

  test("rejects unsupported scheme", () => {
    expect(() => parseProxyConfig({ proxyUrl: "ftp://host:21" })).toThrow(
...
describe("computeConfigHash", () => {
  ...
  test("strips creds before hashing (cred-stable hash)", () => {
    ...
    const a = computeConfigHash({
      proxyUrl: "socks5://alice:pass1@host:1080",
      headed: false,
    });
    const b = computeConfigHash({
      proxyUrl: "socks5://alice:pass2@host:1080",
      headed: false,
    });
    expect(a).toBe(b);
  });
...
describe("extractGlobalFlags", () => {
  ...
  test("--proxy without value → throws", () => {
    expect(() =>
      extractGlobalFlags(["goto", "https://x", "--proxy"], ENV_EMPTY),
    ).toThrow(ProxyConfigError);
  });
