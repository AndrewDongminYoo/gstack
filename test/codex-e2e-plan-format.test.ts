...
const CODEX_FORMAT_TOUCHFILES: Record<string, string[]> = {
  "codex-plan-ceo-format-mode": [
    ...
    "model-overlays/gpt.md",
    "model-overlays/gpt-5.4.md",
  ],
...
function setupCodexSkillDir(
...
): { skillDir: string; planDir: string; outFile: string } {
  ...
  const codexSkillSource = path.join(
    ROOT,
    ".agents",
    "skills",
    `gstack-${skillName}`,
...
describeCodex("Codex Plan Format — CEO Mode Selection", () => {
  ...
  beforeAll(() => {
    ({ skillDir, planDir, outFile } = setupCodexSkillDir(
      "codex-e2e-plan-format-ceo-mode-",
      "plan-ceo-review",
    ));
  });
  ...
  testIfSelected(
    ...
    async () => {
      ...
      console.log(
        `codex-plan-ceo-format-mode: ${result.tokens}t, ${Math.round(result.durationMs / 1000)}s, exit=${result.exitCode}`,
      );

      // Codex may timeout — accept as non-fatal (same pattern as existing codex-e2e tests)
      if (result.exitCode === 124 || result.exitCode === 137) {
        console.warn(
          `codex timed out (exit ${result.exitCode}) — skipping assertions`,
        );
        return;
      }
...
describeCodex("Codex Plan Format — CEO Approach Menu", () => {
  ...
  beforeAll(() => {
    ({ skillDir, planDir, outFile } = setupCodexSkillDir(
      "codex-e2e-plan-format-ceo-approach-",
      "plan-ceo-review",
    ));
  });
  ...
  testIfSelected(
    ...
    async () => {
      ...
      console.log(
        `codex-plan-ceo-format-approach: ${result.tokens}t, ${Math.round(result.durationMs / 1000)}s, exit=${result.exitCode}`,
      );
...
describeCodex("Codex Plan Format — Eng Coverage Issue", () => {
  ...
  testIfSelected(
    ...
    async () => {
      ...
      console.log(
        `codex-plan-eng-format-coverage: ${result.tokens}t, ${Math.round(result.durationMs / 1000)}s, exit=${result.exitCode}`,
      );
...
describeCodex("Codex Plan Format — Eng Kind Issue", () => {
  ...
  beforeAll(() => {
    ({ skillDir, planDir, outFile } = setupCodexSkillDir(
      "codex-e2e-plan-format-eng-kind-",
      "plan-eng-review",
    ));
  });
  ...
  testIfSelected(
    ...
    async () => {
      ...
      console.log(
        `codex-plan-eng-format-kind: ${result.tokens}t, ${Math.round(result.durationMs / 1000)}s, exit=${result.exitCode}`,
      );
