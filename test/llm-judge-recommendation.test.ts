...
describeIfSelected(
  "judgeRecommendation rubric sanity",
  ...
  () => {
    testIfSelected(
      ...
      async () => {
        ...
        const good5 = await judgeRecommendation(
          buildAUQ(
        ...
        const good4 = await judgeRecommendation(
          buildAUQ(
        ...
        const bad1 = await judgeRecommendation(
          buildAUQ("Recommendation: Choose B because it is better."),
        );
        ...
        // SUBSTANCE ~3: generic.
        const bad3 = await judgeRecommendation(
          buildAUQ("Recommendation: Choose B because it is faster."),
        );
        ...
        const noBecause = await judgeRecommendation(
          buildAUQ("Recommendation: Choose B (it has the best tradeoffs)."),
        );
        ...
        const noRec =
          await judgeRecommendation(`D1 — Where should the smarts live?
ELI10: ...
Pros / cons:
A) Server-side
B) Client-side
Net: ...`);
        ...
        for (const [label, text, shouldPass] of crossModelCases) {
          ...
          expect(
            score.present,
            `[cross-model:${label}] present should be true`,
          ).toBe(true);
          expect(
            score.has_because,
            `[cross-model:${label}] has_because should be true`,
          ).toBe(true);
          if (shouldPass) {
            expect(
        ...
        const hedgeForms = [
          ...
          [
            "depends on traffic",
            "Recommendation: A depends on traffic — pick B if read-heavy.",
          ],
          [
            "depending on the team",
            "Recommendation: depending on the team, A or B is fine.",
          ],
          [
            "if X then Y",
            "Recommendation: if low-traffic then A, otherwise B because both work.",
          ],
          ["or maybe", "Recommendation: A or maybe B because both ship in V1."],
          [
            "whichever fits",
            "Recommendation: whichever fits the team — A or B both work.",
          ],
        ];
        for (const [label, text] of hedgeForms) {
          const score = await judgeRecommendation(buildAUQ(text));
          expect(score.present, `[hedge:${label}] present should be true`).toBe(
            true,
          );
          expect(
            score.commits,
