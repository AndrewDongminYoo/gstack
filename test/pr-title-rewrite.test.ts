...
describe("gstack-pr-title-rewrite", () => {
  ...
  test("different version prefix: replaces it", () => {
    expect(rewrite("1.2.3.5", "v1.2.3.4 feat: foo").stdout).toBe(
      "v1.2.3.5 feat: foo",
    );
  });

  test("different prefix length (3-part vs 4-part): replaces it", () => {
    expect(rewrite("1.2.3.4", "v1.2.3 feat: foo").stdout).toBe(
      "v1.2.3.4 feat: foo",
    );
  });
  ...
  test("does not mistake plain words for a prefix", () => {
    expect(rewrite("1.2.3.4", "version 5 feature").stdout).toBe(
      "v1.2.3.4 version 5 feature",
    );
  });

  test("does not strip a single-segment prefix like v1", () => {
    expect(rewrite("1.2.3.4", "v1 feat: foo").stdout).toBe(
      "v1.2.3.4 v1 feat: foo",
    );
  });
