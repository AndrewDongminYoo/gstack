...
function makeFakeGbrain(
  opts: {
    hasOldSource?: boolean;
    addSucceeds?: boolean;
    removeSucceeds?: boolean;
  } = {},
) {
...
describe("v1.27.0.0 migration — nothing to migrate", () => {
  test("no legacy state → exits 0, writes done touchfile, no journal", () => {
    ...
    expect(
      fs.existsSync(path.join(tmpHome, ".gstack/.migrations/v1.27.0.0.done")),
    ).toBe(true);
    expect(
      fs.existsSync(
  ...
  test("done touchfile present → exits 0 silently (no re-prompt)", () => {
    fs.mkdirSync(path.join(tmpHome, ".gstack/.migrations"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpHome, ".gstack/.migrations/v1.27.0.0.done"),
      "",
    );
    const r = run();
    expect(r.code).toBe(0);
  ...
  test("skipped-by-user touchfile → exits 0 silently", () => {
    ...
    fs.writeFileSync(
      path.join(tmpHome, ".gstack/.migrations/v1.27.0.0.skipped-by-user"),
      "",
    );
    fs.writeFileSync(
      path.join(tmpHome, ".gstack-brain-remote.txt"),
      "https://github.com/x/gstack-brain-testuser",
    );
    const r = run();
    expect(r.code).toBe(0);
    expect(r.stderr).toBe("");
...
describe("v1.27.0.0 migration — GitHub host (non-interactive)", () => {
  ...
  test("renames repo, mvs remote.txt, rewrites config key, writes done", () => {
    ...
    const ghLog = fs.readFileSync(
      path.join(fakeBinDir, "gh-calls.log"),
      "utf-8",
    );
    expect(ghLog).toMatch(/gh repo (rename|edit)/);
    // Old remote.txt is gone, new one exists with rewritten URL.
    expect(fs.existsSync(path.join(tmpHome, ".gstack-brain-remote.txt"))).toBe(
      false,
    );
    const newUrl = fs
      .readFileSync(path.join(tmpHome, ".gstack-artifacts-remote.txt"), "utf-8")
      .trim();
    expect(newUrl).toBe(
...
describe("v1.27.0.0 migration — interruption resume", () => {
  ...
  test("partial journal: skips already-done steps", () => {
    ...
    fs.writeFileSync(
      path.join(migDir, "v1.27.0.0.journal"),
      "gh_repo_renamed\nremote_txt_renamed\n",
    );
  ...
  test("with mcpServers.gbrain.type=url → step 5 prints commands, doesn't call gbrain", () => {
    ...
    if (fs.existsSync(path.join(fakeBinDir, "gbrain-calls.log"))) {
      const log = fs.readFileSync(
        path.join(fakeBinDir, "gbrain-calls.log"),
        "utf-8",
      );
...
describe("v1.27.0.0 migration — local CLI sources swap (codex Finding #6 ordering)", () => {
  test("add-new before remove-old (verify by call order in log)", () => {
    ...
    const removeIdx = log.indexOf(
      "gbrain sources remove gstack-brain-testuser",
    );
