  ...
  test("always-ask preference → defer", () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: "<gstack-qid:test-q> Yes?",
            options: ["A) Yes (recommended)", "B) No"],
          },
        ],
      },
...
describe("enforces never-ask preferences", () => {
  test("marker + never-ask + two-way + clean recommendation → deny", () => {
    ...
    expect(r.parsed?.hookSpecificOutput?.permissionDecisionReason).toContain(
      "plan-tune auto-decide",
    );
    expect(r.parsed?.hookSpecificOutput?.permissionDecisionReason).toContain(
      "Fix now",
    );
...
describe("Conductor prose redirect", () => {
  ...
  test("two-way, no preference → deny with [conductor] prose directive", () => {
    ...
    expect(r.parsed?.hookSpecificOutput?.permissionDecisionReason).toMatch(
      /do not call askuserquestion/i,
    );
    expect(r.parsed?.hookSpecificOutput?.permissionDecisionReason).toMatch(
      /reply with a letter/i,
    );
  });
    ...
    const r = runHook(
      {
        ...
        tool_input: {
          questions: [
            {
              question: "No marker — an ad-hoc question",
              options: ["A) Yes (recommended)", "B) No"],
            },
          ],
        },
    ...
    const r = runHook(
      {
        ...
        tool_input: {
          questions: [
            {
              question: "<gstack-qid:test-q> Pick?",
              options: ["A) X (recommended)", "B) Y"],
            },
          ],
        },
      },
      undefined,
      { CONDUCTOR_WORKSPACE_PATH: "/Users/x/conductor/ws" },
    );
    expect(r.parsed?.hookSpecificOutput?.permissionDecision).toBe("deny");
    expect(r.parsed?.hookSpecificOutput?.permissionDecisionReason).toContain(
      "[conductor]",
