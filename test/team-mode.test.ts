...
describe("gstack-settings-hook", () => {
  ...
  test("add preserves existing settings", () => {
    fs.writeFileSync(
      settingsFile,
      JSON.stringify(
        { effortLevel: "high", permissions: { defaultMode: "auto" } },
        null,
        2,
      ),
    );
  ...
  test("remove removes the hook", () => {
    ...
    const result = run(
      `${SETTINGS_HOOK} remove /path/to/gstack-session-update`,
      {
        env: { GSTACK_SETTINGS_FILE: settingsFile },
      },
    );
            ...
            SessionStart: [
              {
                hooks: [
                  {
                    type: "command",
                    command: "/path/to/gstack-session-update",
                  },
                ],
              },
...
describe("gstack-session-update", () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(
      path.join(binDir, "gstack-config"),
      '#!/bin/bash\necho "true"',
    );
  ...
  test("exits 0 when auto_upgrade is not true", () => {
    ...
    fs.writeFileSync(
      path.join(gstackDir, "bin", "gstack-config"),
      '#!/bin/bash\necho "false"',
    );
    const result = run(SESSION_UPDATE, {
      env: { GSTACK_DIR: gstackDir, GSTACK_STATE_DIR: stateDir },
...
describe("gstack-team-init", () => {
  ...
  test("skips when .claude/skills/gstack is a symlink", () => {
    ...
    expect(fs.lstatSync(path.join(skillsDir, "gstack")).isSymbolicLink()).toBe(
      true,
    );
...
describe("setup --team / --no-team / -q", () => {
  ...
  test("setup --local prints deprecation warning", () => {
    ...
    const result = run(
      `bash -c '${path.join(ROOT, "setup")} --local -q 2>&1'`,
      { cwd: ROOT },
    );
