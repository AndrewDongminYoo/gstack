...
describeIfSelected("Autoplan dual-voice E2E", ["autoplan-dual-voice"], () => {
  ...
  beforeAll(() => {
    ...
    copyDirSync(
      path.join(ROOT, "plan-ceo-review"),
      path.join(workDir, "plan-ceo-review"),
    );
    copyDirSync(
      path.join(ROOT, "plan-eng-review"),
      path.join(workDir, "plan-eng-review"),
    );
    copyDirSync(
      path.join(ROOT, "plan-design-review"),
      path.join(workDir, "plan-design-review"),
    );
    copyDirSync(
      path.join(ROOT, "plan-devex-review"),
      path.join(workDir, "plan-devex-review"),
    );
...
`,
  ...
  test.skipIf(!evalsEnabled)(
    ...
    async () => {
      ...
      const claudeVoiceFired =
        /"name":\s*"Agent"|"subagent_type":\s*"[^"]/.test(out) ||
        /Claude\s+(CEO|subagent)\s+(review|complete|finished)|claude-subagent\s/i.test(
          out,
        );
      // Codex voice: require evidence of codex CLI invocation (command string in
      // a Bash tool_use), not prompt-text mentions.
      const codexVoiceFired =
        /"command":\s*"[^"]*codex\s+(exec|review)/.test(out) ||
        /CODEX SAYS\s*\(/i.test(out);
      // Unavailable markers: explicit probe-failure strings emitted by the skill.
      const codexUnavailable =
        /\[codex-unavailable\]|AUTH_FAILED\b|CODEX_NOT_AVAILABLE\b|codex_cli_missing|Codex CLI not found/i.test(
          out,
        );
      ...
      const reachedPhase1 =
        /Phase\s+1\s+(complete|done|finished)|CEO\s+Review\s+(complete|done|approved)|Strategy\s*&\s*Scope\s+(complete|done)|Phase\s+2\s+(started|begin)/i.test(
          out,
        );
      ...
      recordE2E(
        evalCollector,
        "autoplan-dual-voice",
        "Autoplan dual-voice E2E",
        result,
        {
          passed:
            claudeVoiceFired &&
            (codexVoiceFired || codexUnavailable) &&
            reachedPhase1,
        },
      );
    },
