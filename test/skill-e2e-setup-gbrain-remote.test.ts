...
describeE2E("/setup-gbrain Path 4 (Remote MCP) — happy path", () => {
  test("verifies, registers HTTP MCP, never writes token to CLAUDE.md", async () => {
    ...
    const gstackHome = fs.mkdtempSync(
      path.join(os.tmpdir(), "setup-gbrain-remote-"),
    );
    const fakeBinDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "setup-gbrain-remote-bin-"),
    );
    ...
    try {
      ...
      const result = await runAgentSdkTest({
        ...
        canUseTool: async (toolName, input) => {
          if (toolName === "AskUserQuestion") {
            ...
            const decline =
              q.options.find((o) =>
                /skip|decline|no thanks|local/i.test(o.label),
      ...
      const wroteBefore = /## GSTACK REVIEW REPORT|critical_gaps/i.test(
        modelTextOutput,
      );
