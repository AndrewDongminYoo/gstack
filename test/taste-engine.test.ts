...
interface TasteProfile {
  ...
  dimensions: Record<
    ...
    { approved: Preference[]; rejected: Preference[] }
  >;
  sessions: Array<{
    ts: string;
...
describe("taste-engine: first-write lifecycle", () => {
  test("approved creates profile with correct v1 schema", () => {
    const r = run([
      ...
      "variant-A",
      "--reason",
      "fonts: Geist Sans; colors: emerald",
    ]);
...
describe("taste-engine: dimension extraction", () => {
  test("parses multiple dimensions from one reason string", () => {
    run([
      ...
      "v1",
      "--reason",
      "fonts: Geist, IBM Plex; colors: emerald; layouts: grid-12; aesthetics: brutalist",
    ]);
    const p = readProfile();
    expect(p.dimensions.fonts.approved.map((x) => x.value).sort()).toEqual([
      "Geist",
      "IBM Plex",
    ]);
    expect(p.dimensions.colors.approved[0].value).toBe("emerald");
