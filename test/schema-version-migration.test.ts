...
describe("schema-version cache migration (D4 A4)", () => {
  test(
    "cache file with mismatched schema_version triggers wipe-and-rebuild attempt",
    ...
    async () => {
      ...
      const newMeta = JSON.parse(
        readFileSync(join(cacheDir, "_meta.json"), "utf-8"),
      );
  ...
  test(
    "matching schema_version + fresh TTL is warm hit (no rebuild)",
    { timeout: SLOW_TIMEOUT },
    async () => {
      const mod = await importCache();
      const cacheDir = join(TMP_HOME, "projects", "helsinki", "brain-cache");
  ...
  test(
    ...
    async () => {
      ...
      writeFileSync(
        join(cacheDir, "developer-persona.md"),
        "# stale persona\n",
      );
      writeFileSync(
        join(cacheDir, "_meta.json"),
