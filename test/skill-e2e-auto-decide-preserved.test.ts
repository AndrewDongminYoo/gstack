...
describeE2E(
  "AUTO_DECIDE opt-in preserved under Conductor flags (periodic)",
  () => {
    test("user-opted-in question still auto-decides when AskUserQuestion is --disallowedTools", async () => {
      const tmpHome = fs.mkdtempSync(
        path.join(os.tmpdir(), "gstack-auto-decide-"),
      );
      try {
        ...
        if (setRes.status !== 0) {
          throw new Error(
            `gstack-config set failed: ${setRes.stderr || setRes.stdout}`,
          );
        }
        ...
        const slug = (
          slugRes.stdout.match(/SLUG=([^\s;]+)/)?.[1] ?? "unknown"
        ).replace(/['"]/g, "");
        ...
        if (writeRes.status !== 0) {
          throw new Error(
            `gstack-question-preference --write failed: ${writeRes.stderr || writeRes.stdout}`,
          );
        }

        // Sanity: the preference file landed where we expect.
        const prefFile = path.join(
          tmpHome,
          "projects",
          slug,
        ...
        if (!fs.existsSync(prefFile)) {
          throw new Error(
            `expected preference file at ${prefFile}; not found. slug=${slug}`,
          );
        }
