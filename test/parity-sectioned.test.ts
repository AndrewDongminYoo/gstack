...
import {
  checkSkillParity,
  readSkillForParity,
  type ParityInvariant,
} from "./helpers/parity-harness";
...
fs.writeFileSync(
  path.join(root, "ship", "sections", "changelog.md"),
...
fs.writeFileSync(
  path.join(root, "mono", "SKILL.md"),
  "## Preamble\nVERSION CHANGELOG review all inline here.\n",
);

const skeletonBytes = Buffer.byteLength(
  fs.readFileSync(path.join(root, "ship", "SKILL.md"), "utf-8"),
  "utf-8",
);
const unionBytes = readSkillForParity(root, "ship", true).unionBytes;
const baseline: SkillBaselineEntry = {
  skillMdBytes: unionBytes,
} as SkillBaselineEntry;
...
describe("checkSkillParity (sectioned)", () => {
  test("finds phrases that moved into sections (union content check)", () => {
    ...
    const res = checkSkillParity(
      inv,
      { skillMdBytes: skeletonBytes } as SkillBaselineEntry,
      baseline,
      root,
    );
  ...
  test("flags a phrase that truly went missing", () => {
    const inv: ParityInvariant = {
      skill: "ship",
      sectioned: true,
      mustContain: ["this-phrase-is-not-anywhere"],
    };
    const res = checkSkillParity(
      inv,
      { skillMdBytes: skeletonBytes } as SkillBaselineEntry,
