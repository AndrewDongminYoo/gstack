  ...
  test(
    ...
    async () => {
      ...
      for (let i = 0; i < N_RUNS; i++) {
        ...
        let text = "";
        try {
          text = await captureModeSelectionAuq({
            planDir: dir,
            testName: `auq-consistency-${i}`,
            runId,
          });
        } finally {
        ...
        const present = new Set(
          AUQ_FORMAT_ELEMENTS.filter((e) => e.re.test(text)).map(
            (e) => e.field,
          ),
        );
        let substance = 0;
        ...
        console.log(
          `[AUQ-consistency run ${i + 1}/${N_RUNS}] present=${present.size}/${AUQ_FORMAT_ELEMENTS.length} ` +
            `missing=[${AUQ_FORMAT_ELEMENTS.filter((e) => !present.has(e.field))
              .map((e) => e.field)
              .join(",")}] ` +
      ...
      if (anyEmpty.length > 0)
        problems.push(`run(s) produced no AUQ at all: ${anyEmpty.join(",")}`);
      ...
      for (const f of everPresent) {
        const runsMissing = runs
          .filter((r) => !r.present.has(f))
          .map((r) => r.i + 1);
        if (runsMissing.length > 0)
          problems.push(
            `format element "${f}" missing in run(s) ${runsMissing.join(",")}`,
          );
      }
      ...
      if (minSub < 3)
        problems.push(`a run cratered: min substance ${minSub} < 3`);
      if (maxSub - minSub > 2)
        problems.push(
          `substance unstable: spread ${maxSub - minSub} > 2 (${subs.join(",")})`,
        );
        ...
        throw new Error(
          `AUQ inconsistency across ${N_RUNS} runs:\n` +
            ...
            runs
              .map(
                (r) =>
                  `[${r.i + 1}] fmt=${r.present.size}/${AUQ_FORMAT_ELEMENTS.length} sub=${r.substance}`,
              )
              .join(" "),
        );
      }
