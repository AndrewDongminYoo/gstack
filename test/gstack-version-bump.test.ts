...
describe("classifyState (idempotency)", () => {
  ...
  test("ALREADY_BUMPED when VERSION moved past base and pkg agrees (re-run)", () => {
    expect(classifyState("1.2.0.0", "1.1.0.0", true, "1.2.0.0")).toBe(
      "ALREADY_BUMPED",
    );
  });
  test("ALREADY_BUMPED when VERSION moved past base, no package.json", () => {
    expect(classifyState("1.2.0.0", "1.1.0.0", false, "")).toBe(
      "ALREADY_BUMPED",
    );
  });
  test("DRIFT_STALE_PKG when VERSION bumped but pkg lagging", () => {
    expect(classifyState("1.2.0.0", "1.1.0.0", true, "1.1.0.0")).toBe(
      "DRIFT_STALE_PKG",
    );
  });
  test("DRIFT_UNEXPECTED when VERSION matches base but pkg diverges (manual edit)", () => {
    expect(classifyState("1.1.0.0", "1.1.0.0", true, "1.2.0.0")).toBe(
      "DRIFT_UNEXPECTED",
    );
  });
});
...
describe("write (FRESH bump)", () => {
  ...
  test("writes VERSION + package.json.version, preserving other pkg fields", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        { name: "x", version: "1.0.0.0", scripts: { t: "y" } },
        null,
        2,
      ) + "\n",
    );
    const out = execFileSync("bun", [BIN, "write", "--version", "1.1.0.0"], {
      cwd: dir,
    }).toString();
    expect(JSON.parse(out)).toEqual({ wrote: "1.1.0.0", packageJson: true });
    expect(fs.readFileSync(path.join(dir, "VERSION"), "utf-8").trim()).toBe(
      "1.1.0.0",
    );
  ...
  test("rejects a malformed version with exit 2", () => {
    let code = 0;
    try {
      execFileSync("bun", [BIN, "write", "--version", "1.2.3"], {
        cwd: dir,
        stdio: "pipe",
      });
    } catch (e: any) {
  ...
  test("VERSION-only repo (no package.json) writes just VERSION", () => {
    ...
    const out = execFileSync("bun", [BIN, "write", "--version", "0.2.0.0"], {
      cwd: d2,
    }).toString();
    expect(JSON.parse(out)).toEqual({ wrote: "0.2.0.0", packageJson: false });
    expect(fs.readFileSync(path.join(d2, "VERSION"), "utf-8").trim()).toBe(
      "0.2.0.0",
    );
...
describe("repair (DRIFT_STALE_PKG)", () => {
  ...
  test("syncs package.json.version up to VERSION, no re-bump", () => {
    ...
    fs.writeFileSync(
      path.join(dir, "package.json"),
      JSON.stringify({ name: "x", version: "1.9.0.0" }, null, 2) + "\n",
    );
...
describe("classify (idempotency over a real git base)", () => {
  ...
  fs.writeFileSync(
    path.join(dir, "package.json"),
    JSON.stringify({ name: "x", version: "1.0.0.0" }, null, 2) + "\n",
  );
  git("init", "-q", "-b", "main");
  ...
  test("reports FRESH before any bump", () => {
    const out = execFileSync("bun", [BIN, "classify", "--base", "main"], {
      cwd: dir,
    }).toString();
  ...
  test("reports ALREADY_BUMPED after VERSION+pkg move together", () => {
    ...
    fs.writeFileSync(
      path.join(dir, "package.json"),
      JSON.stringify({ name: "x", version: "1.1.0.0" }, null, 2) + "\n",
    );
    const out = execFileSync("bun", [BIN, "classify", "--base", "main"], {
      cwd: dir,
    }).toString();
