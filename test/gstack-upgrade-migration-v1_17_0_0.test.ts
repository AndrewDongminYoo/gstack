...
exit 0
`;
    fs.writeFileSync(path.join(skillsBin, "gstack-config"), cfg, {
      mode: 0o755,
    });
  }
...
`;
    fs.writeFileSync(path.join(skillsBin, "gstack-gbrain-source-wireup"), wu, {
      mode: 0o755,
    });
  }
}
...
describe("migrations/v1.17.0.0.sh", () => {
  ...
  test("gbrain_sync_mode = off: exit 0 silently (no helper invoked)", () => {
    ...
    expect(calls.some((c) => c.startsWith("gstack-gbrain-source-wireup"))).toBe(
      false,
    );
  });
  ...
  test("happy path: invokes the helper", () => {
    ...
    const helperCall = calls.find((c) =>
      c.startsWith("gstack-gbrain-source-wireup"),
    );
