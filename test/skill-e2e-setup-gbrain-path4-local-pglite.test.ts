...
describeE2E(
  "/setup-gbrain Path 4 + Step 4.5 Yes → local PGLite for code",
  () => {
    test("opt-in flow invokes install + gbrain init + remote MCP register", async () => {
      ...
      const sandboxHome = fs.mkdtempSync(
        path.join(os.tmpdir(), "path4-pglite-"),
      );
      const fakeBinDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "path4-pglite-bin-"),
      );
          ...
          canUseTool: async (toolName, input) => {
            if (toolName === "AskUserQuestion") {
              ...
              for (const q of qs) {
                ...
                const yes =
                  q.options.find((o) =>
                    /yes.*local|local.*pglite|code search|opt in/i.test(
                      o.label,
                    ),
                  ) ??
