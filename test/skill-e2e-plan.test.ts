...
describeIfSelected(
  "Plan CEO Review Expansion Energy E2E",
  ...
  () => {
    ...
    beforeAll(() => {
      planDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "skill-e2e-plan-ceo-exp-"),
      );
      const run = (cmd: string, args: string[]) =>
        spawnSync(cmd, args, { cwd: planDir, stdio: "pipe", timeout: 5000 });
...
Write your expansion proposals to ${planDir}/proposals.md with ONLY the proposal text — no conversational wrapper, no review summary, no mode analysis. Each proposal separated by "---".`,
        ...
        if (
          result.exitReason === "error_api" &&
          result.costEstimate?.turnsUsed === 0
        ) {
          console.warn(
            "[transient] /plan-ceo-review-expansion-energy: error_api with 0 turns — treating as inconclusive",
          );
          return;
        }
        expect(["success", "error_max_turns"]).toContain(result.exitReason);
        ...
        if (!fs.existsSync(proposalsPath)) {
          throw new Error(
            "Agent did not emit proposals.md — expansion energy eval requires proposal output",
          );
        }
...
This review report at the bottom of the plan is the MOST IMPORTANT deliverable of this test.`,
    ...
    if (
      ...
      console.warn(
        "[transient] /plan-review-report: error_api with 0 turns — treating as inconclusive (likely Anthropic API hiccup, see CLAUDE.md eval-blame protocol)",
      );
      return;
    }
    expect(["success", "error_max_turns"]).toContain(result.exitReason);
    ...
    const isUnresolvedBullet =
      /^[-*]\s+/.test(lastLine) &&
      !/VERDICT/i.test(lastLine) &&
      afterReport.includes("UNRESOLVED DECISIONS:");
    expect(
    ...
    console.log(
      "Plan review report found at bottom of plan.md (ends with unresolved status)",
    );
  }, 420_000);
});
  ...
  () => {
    ...
    beforeAll(() => {
      ...
      for (const skill of [
        ...
        if (fs.existsSync(_sec))
          fs.cpSync(_sec, path.join(testDir, skill, "sections"), {
            recursive: true,
          });
      }
    });
