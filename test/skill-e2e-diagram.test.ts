...
function skillExtract(): string {
  ...
  if (start < 0)
    throw new Error(
      'diagram/SKILL.md missing "# /diagram" section — regenerate skill docs',
    );
...
describeIfSelected(
  "/diagram skill E2E",
  ["diagram-triplet", "diagram-authoring-quality"],
  () => {
    testConcurrentIfSelected(
      "diagram-triplet",
      async () => {
        ...
        try {
          ...
          const mmd = fs.readFileSync(
            path.join(dir, "out", "flow.mmd"),
            "utf-8",
          );
          expect(mmd).toMatch(/graph\s+(LR|TD)/);

          const scene = JSON.parse(
            fs.readFileSync(path.join(dir, "out", "flow.excalidraw"), "utf-8"),
          );
...
Respond with JSON: {"score": N, "reasoning": "..."}`,
          ...
          console.log(
            `[diagram-quality] score=${verdict.score} — ${verdict.reasoning}`,
          );
