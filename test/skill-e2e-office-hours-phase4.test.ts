...
function setupOfficeHoursDir(): string {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), "skill-e2e-office-hours-phase4-"),
  );
...
`,
  ...
  if (fmtStart < 0 || phase4Start < 0 || phase4End < 0) {
    throw new Error(
      "skill-e2e-office-hours-phase4: failed to slice SKILL.md — section markers not found.",
    );
  }
  const slice = [
...
describeIfSelected(
  "Office Hours Phase 4 — Architectural fork must surface AskUserQuestion",
