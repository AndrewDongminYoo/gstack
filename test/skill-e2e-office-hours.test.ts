...
describeIfSelected(
  "Office Hours Forcing Energy E2E",
  ...
  () => {
    ...
    beforeAll(() => {
      workDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "skill-e2e-office-hours-forcing-"),
      );
      ...
      );
      {
        ...
        if (fs.existsSync(_sec))
          fs.cpSync(_sec, path.join(workDir, "office-hours", "sections"), {
            recursive: true,
          });
      }
    });
...
Write Q3 output — the forcing question you would ask this founder — to ${workDir}/q3.md. Write ONLY the question prose. No conversational wrapper, no meta-commentary, no Q1/Q2 recap.`,
        ...
        if (!fs.existsSync(q3Path)) {
          throw new Error(
            "Agent did not emit q3.md — forcing energy eval requires Q3 output",
          );
        }
...
describeIfSelected(
  "Office Hours Builder Wildness E2E",
  ...
  () => {
    ...
    beforeAll(() => {
      workDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "skill-e2e-office-hours-builder-"),
      );
      const run = (cmd: string, args: string[]) =>
        spawnSync(cmd, args, { cwd: workDir, stdio: "pipe", timeout: 5000 });
...
Write your response — the three adjacent unlocks — to ${workDir}/unlocks.md. Write ONLY the response prose. No meta-commentary, no mode recap. Lead with the fun; let me edit it down later.`,
        ...
        if (!fs.existsSync(unlocksPath)) {
          throw new Error(
            "Agent did not emit unlocks.md — builder wildness eval requires output",
          );
        }
