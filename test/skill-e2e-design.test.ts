...
Write DESIGN.md and CLAUDE.md (or update it) in the working directory.`,
        ...
        const missingSections = Object.entries(sectionSynonyms)
          .filter(
            ([_, synonyms]) =>
              !synonyms.some((s) => designContent.toLowerCase().includes(s)),
          )
          .map(([name]) => name);
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        const researchDir = fs.mkdtempSync(
          path.join(os.tmpdir(), "skill-e2e-research-"),
        );
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        const previewDir = fs.mkdtempSync(
          path.join(os.tmpdir(), "skill-e2e-preview-"),
        );
    ...
    function setupReviewDir(): string {
      ...
      );
      {
        ...
        if (fs.existsSync(_sec))
          fs.cpSync(_sec, path.join(dir, "plan-design-review", "sections"), {
            recursive: true,
          });
      }
