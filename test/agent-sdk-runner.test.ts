...
function makeStubProvider(config: StubConfig): QueryProvider {
  ...
  const provider: QueryProvider = (params) => {
    ...
    const prompt =
      typeof params.prompt === "string" ? params.prompt : "<iterable>";
  ...
  test("systemPrompt, model, cwd, allowedTools, disallowedTools, permissionMode, settingSources, env, pathToClaudeCodeExecutable reach query()", async () => {
    ...
    const stub: StubConfig = {
      streams: [
        [
          systemInit(),
          assistantTurn([{ type: "text", text: "ok" }]),
          resultSuccess(),
        ],
      ],
      calls: [],
...
describe("runAgentSdkTest — canUseTool extension", () => {
  ...
  test("passThroughNonAskUserQuestion helper returns allow+updatedInput", async () => {
    const { passThroughNonAskUserQuestion } =
      await import("../test/helpers/agent-sdk-runner");
    const result = passThroughNonAskUserQuestion("Read", {
      file_path: "/tmp/x",
    });
...
describe("runAgentSdkTest — rate-limit retry", () => {
  test("retryable on thrown 429-shaped error, then succeeds on 2nd attempt", async () => {
    ...
    const stub: StubConfig = {
      ...
      throwAt: 0,
      throwError: Object.assign(new Error("429 too many requests"), {
        status: 429,
      }),
      calls: [],
    };
...
describe("rate-limit detectors", () => {
  test("isRateLimitThrown matches status 429, message, name", () => {
    expect(
      isRateLimitThrown(Object.assign(new Error("boom"), { status: 429 })),
    ).toBe(true);
    ...
    expect(
      isRateLimitThrown(
        Object.assign(new Error("x"), { name: "RateLimitError" }),
      ),
    ).toBe(true);
  ...
  test("produces a SkillTestResult-shaped object", async () => {
    ...
    const stub: StubConfig = {
      streams: [
        [
          systemInit(),
          assistantTurn([{ type: "text", text: "hi" }]),
          resultSuccess(0.02, 1),
        ],
      ],
      calls: [],
    };
...
describe("validateFixtures", () => {
  ...
  test("rejects id with uppercase or unsafe chars", () => {
    expect(() => validateFixtures([base({ id: "Test_Fixture" })])).toThrow(
      /id must be/,
    );
  });

  test("rejects duplicate ids", () => {
    expect(() => validateFixtures([base(), base()])).toThrow(
      /duplicate fixture id/,
    );
  });

  test("rejects non-integer trials", () => {
    expect(() => validateFixtures([base({ trials: 3.5 })])).toThrow(
      /trials must be/,
    );
  });

  test("rejects trials < 3", () => {
    expect(() => validateFixtures([base({ trials: 2 })])).toThrow(
      /trials must be/,
    );
  });

  test("rejects concurrency < 1", () => {
    expect(() => validateFixtures([base({ concurrency: 0 })])).toThrow(
      /concurrency must be/,
    );
  });

  test("rejects non-integer concurrency", () => {
    expect(() => validateFixtures([base({ concurrency: 2.5 })])).toThrow(
      /concurrency must be/,
    );
  });

  test("rejects empty model", () => {
    expect(() => validateFixtures([base({ model: "" })])).toThrow(
      /model must be/,
    );
  });

  test("rejects empty userPrompt", () => {
    expect(() => validateFixtures([base({ userPrompt: "" })])).toThrow(
      /userPrompt must be/,
    );
  });
  ...
  test("rejects absolute overlayPath", () => {
    expect(() =>
      validateFixtures([base({ overlayPath: "/etc/passwd" })]),
  ...
  test("rejects missing overlay file", () => {
    expect(() =>
      validateFixtures([
        base({ overlayPath: "model-overlays/nonexistent.md" }),
      ]),
  ...
  test("rejects non-function setupWorkspace", () => {
    expect(() =>
      validateFixtures([
        base({
          setupWorkspace: "not a function" as unknown as (d: string) => void,
        }),
      ]),
  ...
  test("rejects non-function metric", () => {
    expect(() =>
      validateFixtures([
        base({ metric: null as unknown as (r: AgentSdkResult) => number }),
      ]),
  ...
  test("rejects non-function pass", () => {
    expect(() =>
      validateFixtures([
        base({ pass: undefined as unknown as OverlayFixture["pass"] }),
      ]),
