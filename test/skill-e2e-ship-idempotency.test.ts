...
function buildShippedFixture(): ShipFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      { name: "fixture", version: "0.0.1", private: true },
      null,
      2,
    ) + "\n",
  );
  ...
  fs.writeFileSync(
    path.join(workTree, "package.json"),
    JSON.stringify(
      { name: "fixture", version: "0.0.2", private: true },
      null,
      2,
    ) + "\n",
  );
...
function snapshotFixture(workTree: string): FixtureSnapshot {
  const versionFile = fs
    .readFileSync(path.join(workTree, "VERSION"), "utf-8")
    .trim();
  const pkg = JSON.parse(
    fs.readFileSync(path.join(workTree, "package.json"), "utf-8"),
  );
  ...
  const changelogEntryCount = (changelog.match(/^##\s*\[0\.0\.2\]/gm) ?? [])
    .length;
  ...
  const log = spawnSync("git", ["log", "--format=%s", "main..HEAD"], {
    cwd: workTree,
    stdio: "pipe",
  });
  const subjects = log.stdout?.toString() ?? "";
  const bumpCommitCount = subjects
    .split("\n")
...
describeE2E("/ship idempotency E2E (periodic, real-PTY)", () => {
  test("rerunning /ship on an already-shipped branch detects ALREADY_BUMPED and does not mutate fixture", async () => {
    ...
    try {
      ...
      while (Date.now() - start < budgetMs) {
        ...
        if (
          isNumberedOptionListVisible(tail) &&
        ...
        if (
          /STATE:\s*ALREADY_BUMPED|"state":\s*"ALREADY_BUMPED"/.test(visible)
        ) {
          outcome = "detected";
    ...
    try {
      ...
      try {
        fs.rmSync(path.dirname(fixture.workTree), {
          recursive: true,
          force: true,
        });
      } catch {
