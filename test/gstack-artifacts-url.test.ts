...
describe("gstack-artifacts-url", () => {
  ...
  test("--to https from ssh:// scheme (gitlab self-hosted style)", () => {
    const r = run([
      "--to",
      "https",
      "ssh://git@gitlab.example.org/team/gstack-artifacts-team.git",
    ]);
    expect(r.stdout).toBe(
      "https://gitlab.example.org/team/gstack-artifacts-team",
    );
  });
  ...
  test("--host extracts hostname from any form", () => {
    ...
    expect(run(["--host", "ssh://git@gitlab.example.org/x/y.git"]).stdout).toBe(
      "gitlab.example.org",
    );
  });
  ...
  test("--owner-repo extracts the path segment", () => {
    ...
    expect(
      run(["--owner-repo", "git@github.com:team/gstack-artifacts-team.git"])
        .stdout,
