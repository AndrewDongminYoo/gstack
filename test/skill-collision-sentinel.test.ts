...
describe("skill-collision-sentinel", () => {
  ...
  test("no duplicate skill names within gstack", () => {
    ...
    for (const { name, templatePath } of skills) {
      if (seen.has(name)) {
        dupes.push(
          `${name} appears in both ${seen.get(name)} and ${templatePath}`,
        );
      } else {
    ...
    test(`no skill name collides with a ${host} built-in (or has written justification)`, () => {
      ...
      if (collisions.length > 0) {
        const msg = collisions
          ...
          .join("\n\n");
        throw new Error(
          `Found ${collisions.length} unresolved collision(s) with ${host} built-ins:\n\n${msg}`,
        );
      }
    });
  }
  ...
  test("KNOWN_COLLISIONS_TOLERATED entries are all still active collisions", () => {
    ...
    for (const name of Object.keys(KNOWN_COLLISIONS_TOLERATED)) {
      if (!skillNames.has(name)) {
        stale.push(
          `  "${name}" is in KNOWN_COLLISIONS_TOLERATED but no gstack skill has that name — remove the exception`,
        );
      } else if (!allBuiltins.has(name)) {
        stale.push(
          `  "${name}" is in KNOWN_COLLISIONS_TOLERATED but no host's KNOWN_BUILTINS lists it — remove the exception`,
        );
      }
    }
    if (stale.length > 0) {
