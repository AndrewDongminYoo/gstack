...
describe("Audit compliance", () => {
  ...
  test("preamble telemetry calls are conditional on _TEL and binary existence", () => {
    ...
    const submoduleFiles = existsSync(preambleDir)
      ? readdirSync(preambleDir)
          .filter((f) => f.endsWith(".ts"))
          .map((f) => readFileSync(join(preambleDir, f), "utf-8"))
      : [];
    const rootPreamble = readFileSync(
      join(ROOT, "scripts/resolvers/preamble.ts"),
      "utf-8",
    );
  ...
  test("bun install uses checksum-verified method", () => {
    const browseResolver = readFileSync(
      join(ROOT, "scripts/resolvers/browse.ts"),
      "utf-8",
    );
  ...
  test("browse wraps untrusted content with trust boundary markers", () => {
    const commands = readFileSync(
      join(ROOT, "browse/src/commands.ts"),
      "utf-8",
    );
    ...
    const meta = readFileSync(
      join(ROOT, "browse/src/meta-commands.ts"),
      "utf-8",
    );
  ...
  test("review.ts has data flow documentation", () => {
    const review = readFileSync(
      join(ROOT, "scripts/resolvers/review.ts"),
      "utf-8",
    );
