...
describe("validateHostConfig", () => {
  ...
  test("empty displayName is caught", () => {
    const c = makeValid();
    c.displayName = "";
    expect(validateHostConfig(c).some((e) => e.includes("displayName"))).toBe(
      true,
    );
  });
  ...
  test("invalid cliCommand is caught", () => {
    ...
    expect(validateHostConfig(c).some((e) => e.includes("cliCommand"))).toBe(
      true,
    );
  });
  ...
  test("invalid cliAlias is caught", () => {
    ...
    expect(validateHostConfig(c).some((e) => e.includes("cliAlias"))).toBe(
      true,
    );
  });
  ...
  test("invalid globalRoot is caught", () => {
    ...
    expect(validateHostConfig(c).some((e) => e.includes("globalRoot"))).toBe(
      true,
    );
  });
  ...
  test("invalid localSkillRoot is caught", () => {
    ...
    expect(
      validateHostConfig(c).some((e) => e.includes("localSkillRoot")),
    ).toBe(true);
  });
  ...
  test("invalid hostSubdir is caught", () => {
    ...
    expect(validateHostConfig(c).some((e) => e.includes("hostSubdir"))).toBe(
      true,
    );
  });
  ...
  test("invalid frontmatter.mode is caught", () => {
    ...
    expect(
      validateHostConfig(c).some((e) => e.includes("frontmatter.mode")),
    ).toBe(true);
  });
  ...
  test("invalid linkingStrategy is caught", () => {
    ...
    expect(
      validateHostConfig(c).some((e) => e.includes("linkingStrategy")),
    ).toBe(true);
  });
...
describe("HOST_PATHS derivation from configs", () => {
  ...
  test("localSkillRoot matches config for every host", () => {
    for (const config of ALL_HOST_CONFIGS) {
      expect(HOST_PATHS[config.name].localSkillRoot).toBe(
        config.localSkillRoot,
      );
    }
  });
...
describe("golden-file regression", () => {
  ...
  test("Claude ship skill matches golden baseline", () => {
    const golden = fs.readFileSync(
      path.join(GOLDEN_DIR, "claude-ship-SKILL.md"),
      "utf-8",
    );
  ...
  test("Codex ship skill matches golden baseline", () => {
    const golden = fs.readFileSync(
      path.join(GOLDEN_DIR, "codex-ship-SKILL.md"),
      "utf-8",
    );
    const current = fs.readFileSync(
  ...
  test("Factory ship skill matches golden baseline", () => {
    const golden = fs.readFileSync(
      path.join(GOLDEN_DIR, "factory-ship-SKILL.md"),
      "utf-8",
    );
    const current = fs.readFileSync(
...
describe("host config correctness", () => {
  ...
  test("factory has conditional disable-model-invocation field", () => {
    ...
    expect(factory.frontmatter.conditionalFields![0].if).toEqual({
      sensitive: true,
    });
    expect(factory.frontmatter.conditionalFields![0].add).toEqual({
  ...
  test("openclaw has tool rewrites for exec/read/write", () => {
    ...
    expect(openclaw.toolRewrites!["use the Bash tool"]).toBe(
      "use the exec tool",
    );
    expect(openclaw.toolRewrites!["use the Read tool"]).toBe(
      "use the read tool",
    );
  });
  ...
  test("openclaw has CLAUDE.md→AGENTS.md path rewrite", () => {
    expect(
      openclaw.pathRewrites.some(
        (r) => r.from === "CLAUDE.md" && r.to === "AGENTS.md",
      ),
    ).toBe(true);
  });
