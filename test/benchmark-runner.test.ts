...
test("estimateCostUsd applies cached input discount alongside uncached input", () => {
  ...
  const cost1 = estimateCostUsd(
    { input: 0, output: 0, cached: 1_000_000 },
    "claude-opus-4-7",
  );
  expect(cost1).toBeCloseTo(1.5, 2);
  // 500K uncached input + 500K cached → $7.50 + $0.75 = $8.25
  const cost2 = estimateCostUsd(
    { input: 500_000, output: 0, cached: 500_000 },
    "claude-opus-4-7",
  );
...
test("missingTools reports unsupported tools per provider", () => {
  ...
  expect(missingTools("gpt", ["Edit", "Glob", "Grep"])).toEqual([
    "Edit",
    "Glob",
    "Grep",
  ]);
  ...
  expect(
    missingTools("claude", ["Edit", "Glob", "Grep", "Bash", "Read"]),
  ).toEqual([]);
