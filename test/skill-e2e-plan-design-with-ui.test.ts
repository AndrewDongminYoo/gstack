...
describeE2E("/plan-design-review with UI scope (gate)", () => {
  test("reaches a real skill AskUserQuestion (or plan_ready) without echoing the no-UI early-exit phrase", async () => {
    ...
    let outcome: "real_question" | "plan_ready" | "timeout" | "exited" =
      "timeout";
    let evidence = "";
