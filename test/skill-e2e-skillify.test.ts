...
function setupSkillifyWorkdir(
  suffix: string,
  installSkills: string[] = ["scrape", "skillify"],
): Workdir {
  const workDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `skill-e2e-skillify-${suffix}-`),
  );
...
describeIfSelected(
  "Browser-skills Phase 2a E2E (/scrape + /skillify)",
  [
    "scrape-match-path",
  ...
  () => {
    ...
    testConcurrentIfSelected(
      "scrape-match-path",
      async () => {
        const { workDir, gstackHome } = setupSkillifyWorkdir("match", [
          "scrape",
        ]);
...
Do NOT enter the prototype phase. Do NOT use AskUserQuestion.`,
        ...
        const ranBundledSkill = cmds.some((c) =>
          /\bskill\s+run\s+hackernews-frontpage\b/.test(c),
        );
        ...
        recordE2E(
          evalCollector,
          "scrape match-path routes to bundled skill",
          "Phase 2a E2E",
          result,
          {
    ...
    testConcurrentIfSelected(
      "scrape-prototype-path",
      async () => {
        const { workDir, gstackHome } = setupSkillifyWorkdir("prototype", [
          "scrape",
        ]);
...
Do NOT use AskUserQuestion.`,
        ...
        const fetchedHtml = cmds.some((c) =>
          /\bgoto\b|\bhtml\b|\btext\b/.test(c),
        );
        ...
        recordE2E(
          evalCollector,
          "scrape prototype-path drives $B + emits JSON + nudges skillify",
          "Phase 2a E2E",
          result,
          {
            passed:
    ...
    testConcurrentIfSelected(
      "skillify-happy-path",
      async () => {
        const { workDir, gstackHome } = setupSkillifyWorkdir("happy", [
          "scrape",
          "skillify",
        ]);
        const fixturePath = path.join(workDir, "fixture.html");
...
Do NOT halt for clarification.`,
        ...
        recordE2E(
          evalCollector,
          "skillify happy path writes well-formed skill on disk",
          "Phase 2a E2E",
          result,
          {
    ...
    testConcurrentIfSelected(
      "skillify-provenance-refusal",
      async () => {
        const { workDir, gstackHome } = setupSkillifyWorkdir("refusal", [
          "skillify",
        ]);
...
write any files.`,
        ...
        const surface = fullSurface(result);
        const refusalText =
          /no recent \/?scrape result|run \/scrape.*first|no prior \/?scrape/i.test(
            surface,
          );
        ...
        recordE2E(
          evalCollector,
          "skillify D1 refusal — no on-disk write",
          "Phase 2a E2E",
          result,
          {
    ...
    testConcurrentIfSelected(
      "skillify-approval-reject",
      async () => {
        const { workDir, gstackHome } = setupSkillifyWorkdir("reject", [
          "scrape",
          "skillify",
        ]);
        const fixturePath = path.join(workDir, "fixture.html");
...
Use HOME=${workDir}. Do NOT commit the skill.`,
        ...
        recordE2E(
          evalCollector,
          "skillify approval-reject leaves no on-disk artifact",
          "Phase 2a E2E",
          result,
          {
            passed:
