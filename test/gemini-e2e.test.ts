...
const evalCollector =
  evalsEnabled && !SKIP ? new EvalCollector("e2e-gemini") : null;
  ...
  testIfSelected(
    ...
    async () => {
      ...
      const result = await runGeminiSkill({
        prompt:
          "What is this project? Answer in one sentence based on the README.",
        timeoutMs: 90_000,
        cwd: testWorktree,
      });
