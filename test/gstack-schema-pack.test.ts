...
describe("gstack-core schema pack", () => {
  ...
  test("gstack/take exists with kind=bet supported (Phase 2 / E5)", () => {
    const take = GSTACK_CORE_SCHEMA_PACK.page_types.find(
      (t) => t.type === "gstack/take",
    );
  ...
  test("gstack/product has expected emits_links graph (product → goal/persona/brand/etc.)", () => {
    const product = GSTACK_CORE_SCHEMA_PACK.page_types.find(
      (t) => t.type === "gstack/product",
    )!;
    const verbs = (product.emits_links ?? []).map(
      (l) => `${l.verb}:${l.target_type}`,
    );
  ...
  test("gstack/goal has lifecycle status enum (active/resolved/expired/archived)", () => {
    const goal = GSTACK_CORE_SCHEMA_PACK.page_types.find(
      (t) => t.type === "gstack/goal",
    )!;
  ...
  test("gstack/skill-run records the bet count for calibration coverage", () => {
    const sr = GSTACK_CORE_SCHEMA_PACK.page_types.find(
      (t) => t.type === "gstack/skill-run",
    )!;
