...
import {
  checkOrdering,
  checkCompleteness,
  discoverCarvedSkills,
} from "./helpers/carve-guard-checks";
...
function writeCarve(
  skill: string,
  opts: { stop: boolean; autoGen: boolean; leakBody: boolean },
) {
  ...
  fs.writeFileSync(
    path.join(secDir, "body.md"),
    `${header}## Heavy Body\nThe real work lives here. MOVED_MARKER.\n`,
  );
  const stopLine = opts.stop
    ? "> **STOP.** Before doing the work, Read `sections/body.md` and execute it.\n"
    : "";
...
describe("guard-of-guards — the guards bite (gate, free)", () => {
  ...
  test("E2 fails when the STOP-Read directive is removed", () => {
    ...
    expect(failures.some((f) => f.includes("no STOP-Read directive"))).toBe(
      true,
    );
  ...
  test("E2 fails when heavy body leaks back into the skeleton", () => {
    ...
    expect(failures.some((f) => f.includes("still in the skeleton"))).toBe(
      true,
    );
  ...
  test("E2 fails when a section is hand-edited (no AUTO-GENERATED header)", () => {
    ...
    fs.rmSync(path.join(root, "handeditskill"), {
      recursive: true,
      force: true,
    });
  });
  ...
  test("E1 fails when a skill is carved on disk but missing from the registry", () => {
    ...
    expect(
      failures.some(
        (f) =>
          f.includes("unregisteredskill") && f.includes("NOT in CARVE_GUARDS"),
      ),
    ).toBe(true);
    fs.rmSync(path.join(root, "unregisteredskill"), {
      recursive: true,
      force: true,
    });
  });
