  ...
  for (const skill of CARVED_SKILLS) {
    describe(skill, () => {
      ...
      const manifest = JSON.parse(
        fs.readFileSync(path.join(sectionsDir, "manifest.json"), "utf-8"),
      );
      const sectionTmpls = fs
      ...
      test("every manifest entry has a .md.tmpl source AND a generated .md", () => {
        for (const s of manifest.sections) {
          expect(fs.existsSync(path.join(sectionsDir, `${s.file}.tmpl`))).toBe(
            true,
          );
      ...
      test("no generated orphan: every sections/X.md has a sections/X.md.tmpl → FAIL", () => {
        const orphans = sectionMds.filter(
          (md) => !sectionTmpls.includes(`${md}.tmpl`),
        );
      ...
      test("no hand-edited generated file: every sections/X.md has the AUTO-GENERATED header → FAIL", () => {
        for (const md of sectionMds) {
          const head = fs
            .readFileSync(path.join(sectionsDir, md), "utf-8")
            .slice(0, 120);
      ...
      test("manifest orphan check (WARN in v2.0): every .md.tmpl is listed", () => {
        const listed = new Set(
          manifest.sections.map((s: { file: string }) => `${s.file}.tmpl`),
        );
        ...
        if (unlisted.length > 0) {
          ...
          console.warn(
            `[section-manifest] ${skill} manifest orphan(s) (not in manifest.json): ${unlisted.join(", ")}`,
          );
        }
