...
function run(argv: string[], opts: { env?: Record<string, string> } = {}) {
  const env = {
...
function readState(): {
  sources: Array<{ id: string; local_path: string; federated: boolean }>;
} {
...
function gbrainCalls(): string[] {
  ...
  return fs
    .readFileSync(gbrainCallLog, "utf-8")
    .split("\n")
    .filter((l) => l.trim());
}
...
function setupGstackRepo(remoteUrl: string) {
  ...
  spawnSync("git", ["-C", gstackHome, "init", "-q", "-b", "main"], {
    stdio: "pipe",
  });
  spawnSync(
    "git",
    ["-C", gstackHome, "config", "user.email", "test@example.com"],
    { stdio: "pipe" },
  );
  spawnSync("git", ["-C", gstackHome, "config", "user.name", "test"], {
    stdio: "pipe",
  });
  fs.writeFileSync(path.join(gstackHome, ".brain-allowlist"), "# allowlist\n");
  spawnSync("git", ["-C", gstackHome, "add", "."], { stdio: "pipe" });
  spawnSync("git", ["-C", gstackHome, "commit", "-q", "-m", "init"], {
    stdio: "pipe",
  });
  spawnSync("git", ["-C", gstackHome, "remote", "add", "origin", remoteUrl], {
    stdio: "pipe",
  });
}
...
describe("gstack-gbrain-source-wireup — wireup mode", () => {
  ...
  test("idempotent re-run after success: no new sources add call", () => {
    ...
    const callsAfterFirst = gbrainCalls().filter((c) =>
      c.startsWith("gbrain sources add"),
    ).length;
    expect(callsAfterFirst).toBe(1);
    run([], { env: { GSTACK_BRAIN_NO_SYNC: "1" } });
    const callsAfterSecond = gbrainCalls().filter((c) =>
      c.startsWith("gbrain sources add"),
    ).length;
  ...
  test("drift recovery: existing source with different path triggers remove + add", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        sources: [
          {
            id: "gstack-brain-user",
            local_path: "/old/stale/path",
            federated: true,
          },
        ],
      }),
    ...
    expect(
      calls.some((c) =>
        c.startsWith("gbrain sources remove gstack-brain-user"),
      ),
    ).toBe(true);
    expect(
      calls.some((c) =>
        c.includes(
          `gbrain sources add gstack-brain-user --path ${worktreeDir}`,
        ),
      ),
    ).toBe(true);
    const state = readState();
...
describe("gstack-gbrain-source-wireup — --database-url lock (defends against external config rewrites)", () => {
  test("--database-url flag is exported as GBRAIN_DATABASE_URL to child gbrain calls", () => {
    ...
    makeFakeGbrain({});
    const TARGET =
      "postgresql://postgres.abc:pw@aws.pooler.supabase.com:5432/postgres";
    const r = run(["--database-url", TARGET], {
      env: { GSTACK_BRAIN_NO_SYNC: "1" },
    });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
    // every gbrain invocation should carry the locked URL
    const writingCalls = calls.filter(
      (c) => c.includes("sources") || c.includes("sync"),
    );
  ...
  test("falls back to ~/.gbrain/config.json database_url when no flag and no env", () => {
    ...
    makeFakeGbrain({});
    const FILE_URL =
      "postgresql://postgres.xyz:pw@aws.pooler.supabase.com:5432/postgres";
...
describe("gstack-gbrain-source-wireup — defensive paths", () => {
  test("--no-pull skips HEAD advance on existing worktree", () => {
    ...
    spawnSync(
      "git",
      ["-C", gstackHome, "commit", "-q", "-m", "second commit"],
      { stdio: "pipe" },
    );
    const parentHeadAfter = spawnSync(
      "git",
      ["-C", gstackHome, "rev-parse", "HEAD"],
      {
        encoding: "utf-8",
      },
    ).stdout.trim();
    const worktreeHeadBefore = spawnSync(
      "git",
      ["-C", worktreeDir, "rev-parse", "HEAD"],
      {
        encoding: "utf-8",
      },
    ).stdout.trim();
    ...
    expect(r.status).toBe(0);
    const worktreeHeadAfter = spawnSync(
      "git",
      ["-C", worktreeDir, "rev-parse", "HEAD"],
      {
        encoding: "utf-8",
