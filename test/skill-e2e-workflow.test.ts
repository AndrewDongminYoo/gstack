...
describeIfSelected("Ship workflow E2E", ["ship-local-workflow"], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "gstack-ship-remote-"),
    );
  ...
  afterAll(() => {
    try {
      fs.rmSync(shipWorkDir, { recursive: true, force: true });
    } catch {}
    try {
      fs.rmSync(shipRemoteDir, { recursive: true, force: true });
    } catch {}
  });
...
4. Push to origin: git push origin feature/ship-test`,
      ...
      const branchCheck = spawnSync(
        "git",
        ["branch", "--list", "feature/ship-test"],
        { cwd: shipRemoteDir, stdio: "pipe" },
      );
      ...
      console.log(
        `Branch pushed: ${branchExists}, VERSION: ${versionContent}, bumped: ${versionBumped}`,
      );
    },
    150_000,
  );
...
describeIfSelected("Test Coverage Audit E2E", ["ship-coverage-audit"], () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      path.join(coverageDir, "package.json"),
      JSON.stringify(
        {
    ...
    fs.writeFileSync(
      path.join(coverageDir, "vitest.config.ts"),
    ...
    fs.writeFileSync(
      path.join(coverageDir, "src", "billing.ts"),
      `
...
`,
    ...
    fs.writeFileSync(
      path.join(coverageDir, "test", "billing.test.ts"),
      `
...
Output the diagram directly.`,
      ...
      const hasGap =
        output.includes("GAP") ||
        output.includes("gap") ||
        output.includes("NO TEST");
      const hasTested =
        output.includes("TESTED") ||
        output.includes("tested") ||
        output.includes("✓");
      const hasCoverage =
        output.includes("COVERAGE") ||
        output.includes("coverage") ||
        output.includes("paths tested");
