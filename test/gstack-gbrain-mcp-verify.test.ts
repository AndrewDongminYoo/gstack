...
function makeFakeCurl(opts: {
  ...
  const bodyTools =
    opts.bodyOnTools ?? '{"jsonrpc":"2.0","id":2,"result":{"tools":[]}}';
...
describe("gstack-gbrain-mcp-verify", () => {
  test("SUCCESS: returns server name + version, sources_add_url_supported=false when no sources_add tool", () => {
    ...
    const toolsBody =
      '{"jsonrpc":"2.0","id":2,"result":{"tools":[{"name":"search"},{"name":"put_page"}]}}';
    makeFakeCurl({
      httpCode: "200",
      bodyOnInit: initBody,
      bodyOnTools: toolsBody,
    });
  ...
  test("SUCCESS: sources_add_url_supported=true when MCP exposes a sources_add tool", () => {
    ...
    const toolsBody =
      '{"jsonrpc":"2.0","id":2,"result":{"tools":[{"name":"search"},{"name":"sources_add"}]}}';
    makeFakeCurl({
      httpCode: "200",
      bodyOnInit: initBody,
      bodyOnTools: toolsBody,
  ...
  test("AUTH: HTTP 500 with stale-token-shaped body", () => {
    makeFakeCurl({
      httpCode: "500",
      bodyOnInit:
        '{"error":"server_error","error_description":"Internal Server Error: invalid auth token"}',
    });
  ...
  test("MALFORMED: HTTP 500 without auth-shape (e.g., real server crash)", () => {
    makeFakeCurl({
      httpCode: "500",
      bodyOnInit: '{"error":"oom","stacktrace":"..."}',
    });
    const r = runVerify("faketoken", "https://example.com/mcp");
    expect(r.code).toBe(1);
    const j = JSON.parse(r.stdout);
  ...
  test("MALFORMED: 200 OK but missing serverInfo", () => {
    makeFakeCurl({
      httpCode: "200",
      bodyOnInit: '{"jsonrpc":"2.0","id":1,"result":{}}',
    });
    const r = runVerify("faketoken", "https://example.com/mcp");
    expect(r.code).toBe(1);
