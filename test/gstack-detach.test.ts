...
describe("gstack-detach", () => {
  test("detaches (different pgid), returns immediately, completes, writes EXIT sentinel", () => {
    ...
    try {
      ...
      const r = spawnSync(
        DETACH,
        ["--log", log, "--", "bash", "-c", "sleep 2; echo body-ran"],
      ...
      expect(
        waitFor(() => logHas(log, "### gstack-detach EXIT=0 ###"), 8000),
      ).toBe(true);
  ...
  test("default log is run-scoped under ~/.gstack-dev/eval-runs (no shared /tmp)", () => {
    const r = spawnSync(DETACH, ["--label", "unittest", "--", "true"], {
      encoding: "utf-8",
      timeout: 10000,
    });
    ...
    try {
      ...
      if (log) fs.rmSync(log, { force: true });
    }
  }, 12000);
  ...
  test("watchdog kills a stalled run and records EXIT=timeout (no silent hang)", () => {
    ...
    const log = path.join(dir, "run.log");
    try {
      spawnSync(DETACH, ["--log", log, "--timeout", "1", "--", "sleep", "60"], {
        encoding: "utf-8",
        timeout: 10000,
      });
      expect(
        waitFor(() => logHas(log, "### gstack-detach EXIT=timeout ###"), 12000),
      ).toBe(true);
  ...
  test("machine --lock serializes concurrent runs (second WAITS for the first)", () => {
    ...
    try {
      ...
      spawnSync(DETACH, ["--log", logA, "--lock", lock, "--", "sleep", "3"], {
        encoding: "utf-8",
        timeout: 10000,
      });
      ...
      spawnSync(
        DETACH,
        ["--log", logB, "--lock", lock, "--", "echo", "second-ran"],
        { encoding: "utf-8", timeout: 10000 },
      );
      ...
      expect(
        waitFor(() => logHas(logB, "### gstack-detach EXIT=0 ###"), 12000),
      ).toBe(true);
      expect(logHas(logB, "second-ran")).toBe(true);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
      fs.rmSync(path.join(os.homedir(), ".gstack", "locks", `${lock}.lock`), {
        force: true,
      });
    }
  }, 20000);
