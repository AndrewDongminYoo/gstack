...
function makeFakeGh(
  ...
  const webUrl =
    opts.webUrl ?? `https://github.com/testuser/gstack-artifacts-testuser`;
...
function makeFakeGlab(
  ...
  const repoCreate = opts.repoCreate ?? "success";
  const webUrl =
    opts.webUrl ?? "https://gitlab.com/testuser/gstack-artifacts-testuser";
  const script = `#!/bin/bash
...
describe("gstack-artifacts-init provider selection", () => {
  ...
  test("both gh and glab authed → interactive prompt picks GitHub by default (Enter = 1)", () => {
    ...
    expect(r.status).toBe(0);
    expect(
      readCalls(ghCallLog).some((c) => c.startsWith("gh repo create")),
    ).toBe(true);
    expect(
      readCalls(glabCallLog).some((c) => c.startsWith("glab repo create")),
    ).toBe(false);
  });
  ...
  test("neither authed → falls through to manual URL paste", () => {
    // No gh, no glab fakes.
    const r = run([], {
      input: "https://github.com/testuser/gstack-artifacts-testuser\n",
    });
    expect(r.status).toBe(0);
...
describe("gstack-artifacts-init canonical URL storage (codex Finding #10)", () => {
  ...
  test("strips trailing .git from gh repo view output", () => {
    makeFakeGh({
      webUrl: "https://github.com/testuser/gstack-artifacts-testuser.git",
    });
    const r = run(["--host", "github"]);
    expect(r.status).toBe(0);
    const stored = fs
  ...
  test("configures git origin with SSH form (derived from canonical HTTPS)", () => {
    ...
    const remote = spawnSync(
      "git",
      ["-C", tmpHome, "remote", "get-url", "origin"],
      { encoding: "utf-8" },
    );
    expect(remote.stdout.trim()).toBe(
      "git@github.com:testuser/gstack-artifacts-testuser.git",
    );
  });
});
...
describe("gstack-artifacts-init brain-admin hookup printout (codex Finding #3)", () => {
  ...
  test("--url-form-supported true prints the one-liner with --url", () => {
    ...
    expect(r.stdout).toContain(
      "gbrain sources add gstack-artifacts-testuser --url",
    );
