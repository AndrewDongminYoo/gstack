...
function runMigration(tmpHome: string): {
  exitCode: number;
  stdout: string;
  stderr: string;
} {
...
function setupFakeGstackRoot(tmpHome: string): string {
  ...
  fs.writeFileSync(
    path.join(gstackDir, "checkpoint", "SKILL.md"),
    "# fake gstack checkpoint\n",
  );
  return gstackDir;
}
...
describe("migration v1.1.3.0 — checkpoint ownership guard", () => {
  ...
  beforeEach(() => {
    tmpHome = fs.mkdtempSync(
      path.join(os.tmpdir(), "gstack-migration-ownership-"),
    );
  });
  ...
  test("scenario B: directory with SKILL.md symlinked into gstack → removed", () => {
    ...
    expect(result.stdout).toContain(
      "Removed stale /checkpoint install directory",
    );
  });
  ...
  test("scenario E: nothing to do → no-op exit 0 (idempotent)", () => {
    ...
    fs.rmSync(path.join(tmpHome, ".claude", "skills", "gstack", "checkpoint"), {
      recursive: true,
      force: true,
    });
