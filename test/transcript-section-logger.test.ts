...
const read = (fp: string): ToolCallLike => ({
  tool: "Read",
  input: { file_path: fp },
  output: "",
});
const bash = (command: string): ToolCallLike => ({
  tool: "Bash",
  input: { command },
  output: "",
});
...
describe("extractShipActions", () => {
  ...
  test("VERSION write counts as a version bump even without the CLI", () => {
    const result = {
      toolCalls: [
        { tool: "Write", input: { file_path: "VERSION" }, output: "" },
      ],
    };
...
describe("compareShipActions", () => {
  ...
  test("passes when the sectioned run performs every baseline action", () => {
    const diff = compareShipActions(baseline, [
      ...baseline.actions,
      "merged_base",
    ]);
...
describe("baseline persistence", () => {
  ...
  test("round-trips a baseline to disk", () => {
    const baseline: ShipBaseline = {
      tag: "monolith",
      situation: "no-plan-file",
      actions: ["ran_tests", "committed"],
      sectionReads: [],
      capturedAt: "2026-05-30T00:00:00Z",
    };
