...
function scanDocsForConfigKeys(): {
  docPath: string;
  key: string;
  line: number;
}[] {
    ...
    for (const ent of fs.readdirSync(cur, { withFileTypes: true })) {
      ...
      lines.forEach((line, idx) => {
        ...
        for (const m of line.matchAll(
          /gstack-config\s+(?:set|get)\s+([a-z][a-z0-9_]*)/g,
        )) {
...
describe("docs ↔ gstack-config key drift guard", () => {
  ...
  test.skipIf(process.platform === "win32")(
    "`explain_level` is exposed as a documented default",
    () => {
      const tmpHome = fs.mkdtempSync(
        path.join(require("os").tmpdir(), "gstack-cfg-"),
      );
      try {
  ...
  test.skipIf(process.platform === "win32")(
    "`gstack-config get artifacts_sync_mode` returns a value (the rename landed)",
    () => {
