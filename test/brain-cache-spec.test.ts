...
describe("brain-cache-spec internal consistency", () => {
  ...
  test("salience allowlist has sane defaults (no personal/family/therapy)", () => {
    ...
    for (const prefix of blocked) {
      expect(SALIENCE_DEFAULT_ALLOWLIST.some((p) => p.startsWith(prefix))).toBe(
        false,
      );
    }
  ...
  test("transport policy defaults exist for all transport modes", () => {
    const required = [
      "local-pglite",
      "local-stdio",
      "remote-http-single-tenant",
      "remote-http-ambiguous",
    ];
  ...
  test("user-slug resolution chain has 4 deterministic fallbacks ending in non-empty", () => {
    ...
    expect(
      USER_SLUG_RESOLUTION_ORDER[USER_SLUG_RESOLUTION_ORDER.length - 1],
    ).toBe("anonymous_hostname_sha8");
  });
  ...
  test("helpers return correct values for known names", () => {
    ...
    expect(getSkillSubset("plan-eng-review")).toEqual([
      "product",
      "recent-decisions",
    ]);
