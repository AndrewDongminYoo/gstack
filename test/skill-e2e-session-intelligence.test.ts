...
describeIfSelected(
  ...
  [
    ...
    "context-save-writes-file",
    "context-restore-loads-latest",
  ],
  () => {
    let workDir: string;
    ...
    beforeAll(() => {
      workDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "skill-e2e-session-intel-"),
      );
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        console.log(
          `Timeline flow: ${lines.length} events written, read output ${readOutput.length} chars`,
        );
      },
      30_000,
    );
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        fs.writeFileSync(
          path.join(projectDir, "timeline.jsonl"),
          timelineEntry + "\n",
        );
...
- Look for "RECENT ARTIFACTS" and "LAST_SESSION" in the output.`,
        ...
        const foundArtifacts =
          output.includes("recent artifacts") || output.includes("ceo-plans");
        const foundLastSession =
          output.includes("last_session") || output.includes("ship");
        const foundTimeline =
          output.includes("timeline") || output.includes("completed");
        ...
        console.log(
          `Context recovery: artifacts=${foundArtifacts}, lastSession=${foundLastSession}, timeline=${foundTimeline}`,
        );
      },
      180_000,
    );
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        copyDirSync(
          path.join(ROOT, "context-save"),
          path.join(workDir, "context-save"),
        );

        // Add a staged change so /context-save has something to capture
        fs.writeFileSync(
          path.join(workDir, "feature.ts"),
          "export function newFeature() { return true; }\n",
        );
        spawnSync("git", ["add", "feature.ts"], {
          cwd: workDir,
          stdio: "pipe",
          timeout: 5000,
        });
        ...
        const full = fs.readFileSync(
          path.join(ROOT, "context-save", "SKILL.md"),
          "utf-8",
        );
        const saveStart = full.indexOf("## Save flow");
        const listStart = full.indexOf("## List flow");
        const saveSection = full.slice(
          saveStart,
          listStart > saveStart ? listStart : undefined,
        );
...
- Do NOT use AskUserQuestion.`,
        ...
        if (fileCreated) {
          fileContent = fs.readFileSync(
            path.join(checkpointDir, files[0]),
            "utf-8",
          );
        }

        const hasYamlFrontmatter =
          fileContent.includes("---") && fileContent.includes("status:");
        const hasBranch =
          fileContent.includes("branch:") || fileContent.includes("main");
        ...
        console.log(
          `context-save: ${files.length} files created, YAML frontmatter: ${hasYamlFrontmatter}, branch: ${hasBranch}`,
        );
      },
      180_000,
    );
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        copyDirSync(
          path.join(ROOT, "context-restore"),
          path.join(workDir, "context-restore"),
        );
        ...
        const olderFile = path.join(
          checkpointDir,
          "20260101-120000-old-work.md",
        );
        const newerFile = path.join(
          checkpointDir,
          "20260202-130000-newer-wintermute-work.md",
        );
        fs.writeFileSync(
          olderFile,
          `---
...
`,
        ...
        const full = fs.readFileSync(
          path.join(ROOT, "context-restore", "SKILL.md"),
          "utf-8",
        );
        const restoreStart = full.indexOf("## Restore flow");
        const importantStart = full.indexOf("## Important Rules", restoreStart);
        const restoreSection = full.slice(
          restoreStart,
          importantStart > restoreStart ? importantStart : undefined,
        );
...
- Do NOT use AskUserQuestion. Just present the content of the newest file.`,
        ...
        const loadedNewer =
          output.includes("newer wintermute work") ||
          output.includes("wintermute integration");
        const loadedOlder =
          output.includes("old work") && !output.includes("newer");
        const exitOk = ["success", "error_max_turns"].includes(
          result.exitReason,
        );
        ...
        console.log(
          `context-restore: loadedNewer=${loadedNewer}, loadedOlder=${loadedOlder}`,
        );
      },
      180_000,
    );
