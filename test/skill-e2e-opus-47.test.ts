...
function mkEvalRoot(suffix: string, includeOverlay: boolean): string {
  ...
  const result = spawnSync(
    ...
    [
      ...
      "--model",
      includeOverlay ? "opus-4-7" : "claude",
    ],
  ...
  const overlayText = includeOverlay
    ? fs
        .readFileSync(path.join(ROOT, "model-overlays", "opus-4-7.md"), "utf-8")
...
const ROUTING_CASES: RoutingCase[] = [
  ...
  {
    name: "pos-wtf-bug",
    prompt: "wtf is this error coming from auth.ts:47 when the cookie expires?",
    shouldRoute: true,
    expectedSkill: "investigate",
  },
  {
    name: "pos-send-it",
    prompt: "ok this is good enough, let's send it.",
    shouldRoute: true,
    expectedSkill: "ship",
  },
  {
    name: "pos-does-it-work",
    prompt:
      "I just pushed the login flow changes. Test the deployed site and find any bugs.",
    shouldRoute: true,
    expectedSkill: "qa",
  },
  // Negative — should NOT route
  {
    name: "neg-syntax-q",
    prompt:
      "wtf does this Python list comprehension syntax even mean, [x for x in y if z]?",
    shouldRoute: false,
  },
  {
    name: "neg-algo-q",
    prompt: "does this bubble sort algorithm actually work in O(n log n)?",
    shouldRoute: false,
  },
  {
    name: "neg-slack-send",
    prompt:
      "can you help me write the slack message? I want to send it to the team.",
    shouldRoute: false,
  },
];
...
describeE2E("Opus 4.7 overlay behavior evals", () => {
  ...
  test("fanout: overlay ON emits >= parallel calls vs overlay OFF on 3-file investigate task", async () => {
    ...
    try {
      ...
      console.log(
        `  cost A=$${resA.costEstimate.estimatedCost.toFixed(2)} B=$${resB.costEstimate.estimatedCost.toFixed(2)}`,
      );
      ...
      expect(
        parA,
        `overlay arm emitted ${parA} parallel calls, baseline ${parB}`,
      ).toBeGreaterThanOrEqual(parB);
    } finally {
  ...
  test("routing precision: positives route, negatives do not", async () => {
    ...
    try {
      ...
      expect(
        tpRate,
        `true-positive rate ${(tpRate * 100).toFixed(0)}% (need >= 66%)`,
      ).toBeGreaterThanOrEqual(2 / 3);
      expect(
        fpRate,
        `false-positive rate ${(fpRate * 100).toFixed(0)}% (need <= 33%)`,
