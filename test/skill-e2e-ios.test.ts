...
function startStubStateServer(
  initial: StubState,
): Promise<{ server: Server; port: number; state: StubState }> {
  const state = { ...initial };
    ...
    const server = createServer((req, res) => {
      ...
      req.on("end", () => {
        ...
        if (req.method === "POST" && url === "/session/acquire") {
          if (activeSession)
            return respond(res, 423, { error: "device_locked" });
          activeSession =
            "stub-session-" + Math.random().toString(16).slice(2, 8);
        ...
        if (req.method === "POST" && url === "/state/restore") {
          const payload = JSON.parse(body || "{}");
          if (
            payload._accessor_hash &&
            payload._accessor_hash !== "stub-hash"
          ) {
            return respond(res, 409, { error: "schema_mismatch" });
          }
          if (payload.keys?.loggedIn !== undefined)
            state.loggedIn = payload.keys.loggedIn;
          if (payload.keys?.username !== undefined)
            state.username = payload.keys.username;
          return respond(res, 200, { ok: true });
        }
...
`,
    ...
    expect(result.specs[0]!.fields.map((f) => f.name).sort()).toEqual([
      "counter",
      "isLoggedIn",
      "username",
    ]);
  ...
  test("NO_DEVICE: cache hit on rerun", () => {
    ...
    writeFileSync(
      join(srcDir, "AppState.swift"),
      "@Observable class A { @Snapshotable var x: Int = 0 }",
    );
    const cacheRoot = join(workDir, "cache");
    const r1 = generate({
      inputDir: srcDir,
  ...
  test("NO_DEVICE: schema mismatch returns 409 on restore", async () => {
    const workDir = makeWorkDir();
    const stub = await startStubStateServer({
      loggedIn: false,
      username: "",
      rawTaps: [],
    });
    try {
      ...
      try {
        ...
        const acqR = await fetchJson(
          "POST",
          `http://127.0.0.1:${daemon.loopbackPort}/session/acquire`,
        );
        expect(acqR.status).toBe(200);
        const sessionId = (acqR.body as { session_id: string }).session_id;

        // Restore with wrong schema hash
        const restoreR = await fetchJson(
          "POST",
          `http://127.0.0.1:${daemon.loopbackPort}/state/restore`,
          {
            headers: {
              "content-type": "application/json",
        ...
        expect((restoreR.body as { error: string }).error).toBe(
          "schema_mismatch",
        );
      } finally {
        await daemon.close();
  ...
  test("SCENARIO: acquire → snapshot → restore → tap → release", async () => {
    ...
    try {
      ...
      try {
        ...
        expect(
          (snap.body as { keys: { loggedIn: boolean } }).keys.loggedIn,
        ).toBe(false);
        ...
        expect(
          (snap2.body as { keys: { loggedIn: boolean; username: string } })
            .keys,
        ).toEqual({
          loggedIn: true,
          username: "agent@e2e",
  ...
  test("SCENARIO: tailnet allowlist gate + mint + audit log", async () => {
    ...
    try {
      ...
      try {
        ...
        const sessionToken = (minted.body as { session_token: string })
          .session_token;
  ...
  test("SCENARIO: capability-tier enforcement — observe token cannot /tap", async () => {
    ...
    try {
      ...
      try {
        ...
        expect((tap.body as { error: string }).error).toBe(
