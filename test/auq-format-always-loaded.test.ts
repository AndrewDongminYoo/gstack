...
const MANDATORY: Array<{ name: string; re: RegExp }> = [
  {
    name: "## AskUserQuestion Format header",
    re: /##\s*AskUserQuestion Format/i,
  },
  ...
  {
    name: "AUQ-failure fallback subsection",
    re: /When AskUserQuestion is unavailable or a call fails/i,
  },
...
const PER_SKILL_RULES: Record<string, RegExp[]> = {
  ...
  codex: [
    /Synthesis recommendation \(REQUIRED\)/i,
    /Recommendation\s*:\s*<action>\s*because/i,
  ],
};
...
describe("AUQ format is always-loaded (token-reduction safety net)", () => {
  ...
  for (const { skill, skillMd, sectionsDir } of skills.filter(
    (s) => s.sectionsDir,
  )) {
    test(`${skill} (carved): AUQ format block lives in the skeleton, not only sections/`, () => {
      ...
      expect(fs.readdirSync(sectionsDir!).some((f) => f.endsWith(".md"))).toBe(
        true,
      );
    });
  }
  ...
  for (const [skill, rules] of Object.entries(PER_SKILL_RULES)) {
    test(`${skill}: per-skill AUQ rules survive in skeleton+sections union`, () => {
      ...
      if (fs.existsSync(secDir)) {
        for (const f of fs
          .readdirSync(secDir)
          .filter((f) => f.endsWith(".md") && !f.endsWith(".md.tmpl"))) {
