...
describe("hermetic wiring tripwire", () => {
  ...
  test("claude runners gate --strict-mcp-config on isHermeticEnabled()", () => {
    ...
    for (const rel of [
      ...
      const gated =
        /if\s*\(\s*isHermeticEnabled\(\)\s*\)\s*(args\.push\(\s*)?['"]--strict-mcp-config['"]/.test(
          src,
        ) ||
        /const hermetic = isHermeticEnabled\(\);[\s\S]{0,200}if\s*\(hermetic\)\s*args\.push\(\s*['"]--strict-mcp-config['"]/.test(
          src,
        );
      expect(
        gated,
        `${rel}: --strict-mcp-config must be gated on isHermeticEnabled()`,
      ).toBe(true);
    }
  });
  ...
  test("no test callsite passes the whole operator env as a RUNNER override", () => {
    ...
    const DIRECT_SPAWN =
      /\b(spawnSync|spawn|execSync|exec|Bun\.spawn|Bun\.spawnSync)\s*\(/;
    ...
    const walk = (dir: string) => {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        for (let i = 0; i < lines.length; i++) {
          if (
            !/env:\s*(\{\s*\.\.\.\s*process\.env|process\.env\b(?!\.))/.test(
              lines[i],
            )
          )
            continue;
