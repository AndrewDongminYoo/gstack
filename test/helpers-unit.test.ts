...
describe("parseNumberedOptions", () => {
  ...
  test("parses cursor on a non-first option", () => {
    const visible = ["  1. Option A", "❯ 2. Option B", "  3. Option C"].join(
      "\n",
    );
...
function makeDelta(
  ...
  return {
    ...
    before: {
      passed: true,
      cost_usd: 0,
      tool_summary: beforeTools,
      turns_used: beforeTurns,
    },
    after: {
      passed: true,
      cost_usd: 0,
      tool_summary: afterTools,
      turns_used: afterTurns,
    },
...
describe("findBudgetRegressions", () => {
  ...
  test("skips when prior tool count is below the floor (noise floor)", () => {
    ...
    const c = makeComparison([makeDelta("tiny", { Bash: 1 }, { Bash: 4 })]);
    expect(findBudgetRegressions(c)).toEqual([]);
  });
...
describe("assertNoBudgetRegression", () => {
  test("does not throw on a clean comparison", () => {
    const c = makeComparison([makeDelta("a", { Bash: 10 }, { Bash: 11 })]);
