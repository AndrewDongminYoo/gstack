...
describe("gstack-gbrain-supabase-verify", () => {
  ...
  test("rejects direct-connection URL with exit code 3", () => {
    const url =
      "postgresql://postgres:secret@db.abcdefghijk.supabase.co:5432/postgres";
  ...
  test("rejects wrong scheme", () => {
    const r = runVerify(
      "mysql://user:pass@aws-0-us-east-1.pooler.supabase.com:6543/postgres",
    );
  ...
  test("rejects wrong host (not *.pooler.supabase.com)", () => {
    const r = runVerify(
      "postgresql://postgres.ref:pass@example.com:6543/postgres",
    );
    expect(r.status).toBe(2);
