...
beforeAll(() => {
  ...
  const otherEntries = [
    {
      ...
      skill: "test",
      type: "pattern",
      key: "foreign-observed",
      insight: "A foreign observed insight",
      confidence: 8,
      source: "observed",
      trusted: false,
      files: [],
    },
    {
      ...
      type: "pattern",
      key: "foreign-user",
      insight: "A foreign user-stated insight",
      confidence: 8,
      source: "user-stated",
      trusted: true,
      files: [],
    ...
    // existed). The old `=== false` denylist admitted these; the allowlist must exclude.
    {
      ...
      type: "pattern",
      key: "foreign-legacy",
      insight: "A foreign legacy insight with no trusted field",
      confidence: 8,
      source: "observed",
      files: [],
    },
  ...
  fs.writeFileSync(
    path.join(projDir, "learnings.jsonl"),
    entries.map((e) => JSON.stringify(e)).join("\n") + "\n",
  );
  fs.writeFileSync(
    path.join(otherProjDir, "learnings.jsonl"),
