...
describeE2E("/autoplan chain ordering (periodic)", () => {
  test("phases run sequentially: Phase 1 (CEO) before Phase 3 (Eng), Phase 2 (Design) between when present", async () => {
    // UI-heavy fixture so Phase 2 runs.
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "gstack-autoplan-chain-"),
    );
    try {
      ...
      const hits: PhaseHit[] = [];
      let outcome: "chain_complete" | "plan_ready" | "timeout" | "exited" =
        "timeout";
      let evidence = "";
