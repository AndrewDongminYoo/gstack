...
function runLog(
  ...
  try {
    const stdout = execSync(
      `${BIN}/gstack-learnings-log '${input.replace(/'/g, "'\\''")}'`,
      execOpts,
    ).trim();
...
describe("gstack-learnings-log", () => {
  test("appends valid JSON to learnings.jsonl", () => {
    const input =
      '{"skill":"review","type":"pattern","key":"test-key","insight":"test insight","confidence":8,"source":"observed"}';
  ...
  test("auto-injects timestamp when ts is missing", () => {
    const input =
      '{"skill":"review","type":"pattern","key":"ts-test","insight":"test","confidence":5,"source":"observed"}';
    runLog(input);
  ...
  test("append-only: duplicate keys create multiple entries", () => {
    const input1 =
      '{"skill":"review","type":"pattern","key":"dup-key","insight":"first version","confidence":6,"source":"observed"}';
    const input2 =
      '{"skill":"review","type":"pattern","key":"dup-key","insight":"second version","confidence":8,"source":"observed"}';
    runLog(input1);
    runLog(input2);
  ...
  test('accepts type:"investigation" (regression: #1423)', () => {
    const input =
      '{"skill":"investigate","type":"investigation","key":"root-cause","insight":"verified","confidence":9,"source":"observed"}';
    const result = runLog(input);
    expect(result.exitCode).toBe(0);
    const f = findLearningsFile();
    expect(f).not.toBeNull();
...
describe("gstack-learnings-search", () => {
  ...
  test("returns formatted output when learnings exist", () => {
    runLog(
      '{"skill":"review","type":"pattern","key":"test-search","insight":"search test insight","confidence":7,"source":"observed"}',
    );
    const output = runSearch();
  ...
  test("filters by --type", () => {
    runLog(
      '{"skill":"review","type":"pattern","key":"p1","insight":"a pattern","confidence":7,"source":"observed"}',
    );
    runLog(
      '{"skill":"review","type":"pitfall","key":"p2","insight":"a pitfall","confidence":7,"source":"observed"}',
    );
  ...
  test("filters by --query", () => {
    runLog(
      '{"skill":"review","type":"pattern","key":"auth-bypass","insight":"check session tokens","confidence":7,"source":"observed"}',
    );
    runLog(
      '{"skill":"review","type":"pattern","key":"n-plus-one","insight":"use includes for associations","confidence":7,"source":"observed"}',
    );
  ...
  test("respects --limit", () => {
    for (let i = 0; i < 5; i++) {
      runLog(
        `{"skill":"review","type":"pattern","key":"limit-${i}","insight":"insight ${i}","confidence":7,"source":"observed"}`,
      );
    }
  ...
  test("applies confidence decay for observed/inferred sources", () => {
    ...
    runLog(
      `{"skill":"review","type":"pattern","key":"decay-test","insight":"old observation","confidence":8,"source":"observed","ts":"${ts}"}`,
    );
  ...
  test("does NOT decay user-stated learnings", () => {
    const ts = new Date(Date.now() - 90 * 86400000).toISOString();
    runLog(
      `{"skill":"review","type":"preference","key":"no-decay-test","insight":"user preference","confidence":9,"source":"user-stated","ts":"${ts}"}`,
    );
  ...
  test("skips malformed JSONL lines gracefully", () => {
    ...
    runLog(
      '{"skill":"review","type":"pattern","key":"valid-entry","insight":"valid","confidence":7,"source":"observed"}',
    );
    const f = findLearningsFile();
    expect(f).not.toBeNull();
    fs.appendFileSync(f!, "\nthis is not json\n");
    fs.appendFileSync(
      f!,
      '{"skill":"review","type":"pattern","key":"also-valid","insight":"also valid","confidence":6,"source":"observed","ts":"2026-03-28T00:00:00Z"}\n',
    );
...
describe("gstack-learnings-log edge cases", () => {
  test("preserves existing timestamp when ts is present", () => {
    const input =
      '{"skill":"review","type":"pattern","key":"ts-preserve","insight":"test","confidence":5,"source":"observed","ts":"2025-06-15T10:00:00Z"}';
    runLog(input);
  ...
  test("handles JSON with special characters in insight", () => {
    const input = JSON.stringify({
      ...
      type: "pattern",
      key: "special-chars",
      insight: 'Use "quotes" and \\backslashes',
      confidence: 7,
      source: "observed",
    });
    runLog(input);
