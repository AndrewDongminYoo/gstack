...
describeIfSelected(
  "first-run scaffold detection (E2E)",
  ["first-task-scaffold"],
  () => {
    testIfSelected(
      "first-task-scaffold",
      async () => {
        if (!process.env.ANTHROPIC_API_KEY) {
          throw new Error(
            "first-task-scaffold requires ANTHROPIC_API_KEY (source ~/.zshrc); refusing to skip",
          );
        }
        ...
        try {
          ...
          execSync("git add -A && git commit -qm init", {
            cwd: nodeDir,
            env: GIT_ENV,
          });
          ...
          const greenOut = await detectVia(
            greenDir,
            "first-task-scaffold-greenfield",
          );
