...
describe("is-conductor", () => {
  test("true when CONDUCTOR_WORKSPACE_PATH is set", () => {
    expect(
      isConductor({ CONDUCTOR_WORKSPACE_PATH: "/Users/x/conductor/ws" }),
    ).toBe(true);
  });
  ...
  test("true when both are set", () => {
    expect(
      isConductor({ CONDUCTOR_WORKSPACE_PATH: "/ws", CONDUCTOR_PORT: "55070" }),
    ).toBe(true);
  });
  ...
  test("false when the vars are present but empty (Codex #1 hardening — empty != set)", () => {
    expect(
      isConductor({ CONDUCTOR_WORKSPACE_PATH: "", CONDUCTOR_PORT: "" }),
    ).toBe(false);
  });
  ...
  test("defaults to process.env when no arg is passed", () => {
    ...
    try {
      ...
      if (!process.env.CONDUCTOR_WORKSPACE_PATH)
        expect(isConductor()).toBe(false);
    } finally {
