...
function scaffoldFixture(prefix: string): {
  workDir: string;
  stateRoot: string;
  slug: string;
} {
  ...
  // Copy hooks dir.
  copyDirSync(
    path.join(ROOT, "hosts", "claude", "hooks"),
    path.join(workDir, "hosts", "claude", "hooks"),
  );
...
describeIfSelected(
  "PlanTune cathedral E2E: hook capture",
  ["plan-tune-hook-capture"],
  () => {
    ...
    testConcurrentIfSelected("hook directly invoked → log fills", async () => {
      ...
      const payload = {
        ...
        tool_response: {
          answers: [{ option_label: "A) Fix now (recommended)" }],
        },
        cwd: fixture.workDir,
      };
...
describeIfSelected(
  "PlanTune cathedral E2E: enforcement",
  ["plan-tune-enforcement"],
  () => {
    let fixture: ReturnType<typeof scaffoldFixture>;
    ...
    beforeAll(() => {
      ...
      fs.mkdirSync(path.join(fixture.stateRoot, "projects", fixture.slug), {
        recursive: true,
      });
      fs.writeFileSync(
        path.join(
    ...
    testConcurrentIfSelected(
      "PreToolUse hook denies + logs auto-decided event",
      async () => {
        const hookPath = path.join(
          fixture.workDir,
          "hosts",
...
describeIfSelected(
  "PlanTune cathedral E2E: annotation",
  ["plan-tune-annotation"],
  () => {
    let fixture: ReturnType<typeof scaffoldFixture>;
    ...
    testConcurrentIfSelected(
      "PreToolUse hook surfaces memory nugget on defer",
      async () => {
        const hookPath = path.join(
          fixture.workDir,
          "hosts",
        ...
        const payload = {
          ...
          tool_input: {
            questions: [
              {
                question:
                  "<gstack-qid:ship-todos-reorganize> Reorganize TODOs?",
                options: ["A) Accept (recommended)", "B) Skip"],
              },
            ],
          },
...
describeIfSelected(
  "PlanTune cathedral E2E: codex import",
  ["plan-tune-codex-import"],
  () => {
    let fixture: ReturnType<typeof scaffoldFixture>;
    ...
    testConcurrentIfSelected(
      "importer extracts events with codex-import-marker source",
      async () => {
        const bin = path.join(
          fixture.workDir,
          "bin",
...
describeIfSelected(
  "PlanTune cathedral E2E: dream cycle",
  ["plan-tune-dream-cycle"],
  () => {
    let fixture: ReturnType<typeof scaffoldFixture>;
    ...
    testConcurrentIfSelected(
      "apply → re-fire → memory injected via additionalContext",
      async () => {
        // 1. Apply the proposal via gstack-distill-apply.
        const applyBin = path.join(
          fixture.workDir,
          "bin",
          "gstack-distill-apply",
        );
