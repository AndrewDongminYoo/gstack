  ...
  for (const name of input.providers) {
    ...
    if (!factory) {
      entries.push({
        ...
        family: "claude",
        available: false,
        unavailable_reason: `unknown provider: ${name}`,
      });
      continue;
    }
...
export function formatTable(report: BenchmarkReport): string {
  ...
  for (const e of report.entries) {
    if (!e.available) {
      rows.push(
        `${pad(e.provider, 20)} ${pad("-", 9)} ${pad("-", 20)} ${pad("-", 10)} ${pad("-", 9)} ${pad("-", 12)} unavailable: ${e.unavailable_reason ?? "unknown"}`,
      );
      continue;
    }
    const r = e.result!;
    if (r.error) {
      rows.push(
        `${pad(r.modelUsed, 20)} ${pad(msToStr(r.durationMs), 9)} ${pad(`${r.tokens.input}→${r.tokens.output}`, 20)} ${pad(fmtCost(e.costUsd), 10)} ${pad("-", 9)} ${pad(String(r.toolCalls), 12)} ERROR ${r.error.code}: ${r.error.reason.slice(0, 40)}`,
      );
      continue;
    }
    const quality =
      e.qualityScore !== undefined ? `${e.qualityScore.toFixed(1)}/10` : "-";
    rows.push(
      `${pad(r.modelUsed, 20)} ${pad(msToStr(r.durationMs), 9)} ${pad(`${r.tokens.input}→${r.tokens.output}`, 20)} ${pad(fmtCost(e.costUsd), 10)} ${pad(quality, 9)} ${pad(String(r.toolCalls), 12)}`,
    );
  }
  return rows.join("\n");
}
...
export function formatMarkdown(report: BenchmarkReport): string {
  ...
  for (const e of report.entries) {
    if (!e.available) {
      lines.push(
        `| ${e.provider} | - | - | - | - | - | unavailable: ${e.unavailable_reason ?? "unknown"} |`,
      );
      continue;
    }
    const r = e.result!;
    if (r.error) {
      lines.push(
        `| ${r.modelUsed} | ${msToStr(r.durationMs)} | ${r.tokens.input}→${r.tokens.output} | ${fmtCost(e.costUsd)} | - | ${r.toolCalls} | ERROR ${r.error.code}: ${r.error.reason.slice(0, 80)} |`,
      );
      continue;
    }
    const quality =
      e.qualityScore !== undefined ? `${e.qualityScore.toFixed(1)}/10` : "-";
    lines.push(
      `| ${r.modelUsed} | ${msToStr(r.durationMs)} | ${r.tokens.input}→${r.tokens.output} | ${fmtCost(e.costUsd)} | ${quality} | ${r.toolCalls} | |`,
    );
  }
  return lines.join("\n");
}
