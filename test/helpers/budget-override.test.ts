...
const TMP_HOME = fs.mkdtempSync(
  path.join(os.tmpdir(), "budget-override-test-"),
);
...
describe("logBudgetOverride", () => {
  ...
  test("writes one JSONL line per call with required fields", () => {
    ...
    expect(entry.reason).toBe(
      "model price went up, will rebase the cap next sprint",
    );
    expect(entry.details).toEqual({
      tier: "e2e",
      cap: 25,
      observed_cost_usd: 31.4,
    });
  ...
  test("never throws even when audit directory is missing — creates it", () => {
    ...
    try {
      fs.rmSync(path.join(TMP_HOME, "analytics"), {
        recursive: true,
        force: true,
      });
    } catch {
    ...
    expect(() =>
      logBudgetOverride({ scope: "recreate", reason: "test" }),
    ).not.toThrow();
    expect(fs.existsSync(AUDIT_PATH)).toBe(true);
  });
  ...
  test("survives an unwritable audit path (logs warning, does not throw)", () => {
    ...
    expect(() =>
      logBudgetOverride({ scope: "unwritable", reason: "fs error path" }),
    ).not.toThrow();
