...
export function findBudgetRegressions(
  ...
  const cap =
    opts?.ratioCap ??
    (Number.isFinite(envRatio) && envRatio > 0 ? envRatio : 2.0);
  ...
  for (const d of comparison.deltas) {
    const beforeTools = Object.values(d.before.tool_summary ?? {}).reduce(
      (a, b) => a + b,
      0,
    );
    const afterTools = Object.values(d.after.tool_summary ?? {}).reduce(
      (a, b) => a + b,
      0,
    );
...
export function assertNoBudgetRegression(
  ...
  const cap =
    opts?.ratioCap ?? (Number(process.env.GSTACK_BUDGET_RATIO) || 2.0);
  const lines = regressions.map(
    (r) =>
      `  "${r.testName}" ${r.metric}: ${r.before} → ${r.after} (${r.ratio.toFixed(2)}× > ${cap.toFixed(2)}× cap)`,
  );
  throw new Error(
