...
export const PRICING: Record<string, ModelPricing> = {
  ...
  // OpenAI (GPT + o-series)
  "gpt-5.4": { input_per_mtok: 2.5, output_per_mtok: 10.0, as_of: "2026-04" },
  "gpt-5.4-mini": {
    input_per_mtok: 0.6,
    output_per_mtok: 2.4,
    as_of: "2026-04",
  },
  o3: { input_per_mtok: 15.0, output_per_mtok: 60.0, as_of: "2026-04" },
  "o4-mini": { input_per_mtok: 1.1, output_per_mtok: 4.4, as_of: "2026-04" },
...
export function estimateCostUsd(
  ...
  if (!row) {
    if (!WARNED.has(model)) {
      WARNED.add(model);
      console.error(
        `WARN: no pricing for model ${model}; returning 0. Add it to test/helpers/pricing.ts.`,
      );
    }
    return 0;
  }
  ...
  const cachedCost =
    ((tokens.cached ?? 0) * row.input_per_mtok * cachedDiscount) / 1_000_000;
