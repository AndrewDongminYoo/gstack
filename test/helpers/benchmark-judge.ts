...
export async function judgeEntries(report: BenchmarkReport): Promise<void> {
  if (!process.env.ANTHROPIC_API_KEY) {
    throw new Error(
      "ANTHROPIC_API_KEY not set — judge requires Anthropic access.",
    );
  }
  const { default: Anthropic } = await import("@anthropic-ai/sdk").catch(() => {
    throw new Error(
      "@anthropic-ai/sdk not installed — run `bun add @anthropic-ai/sdk` if you want the judge.",
    );
  });
  const client = new (Anthropic as unknown as new (opts: {
    apiKey: string;
  }) => {
    messages: {
      create: (
  ...
  const successful = report.entries.filter(
