  ...
  async available(): Promise<AvailabilityCheck> {
    ...
    if (res.status !== 0) {
      return {
        ok: false,
        reason:
          "gemini CLI not found on PATH. Install per https://github.com/google-gemini/gemini-cli",
      };
    }
    ...
    if (!hasCfg && !hasKey) {
      return {
        ok: false,
        reason:
          "No Gemini auth found. Log in via `gemini login` or export GOOGLE_API_KEY.",
      };
    }
    return { ok: true };
  }
