...
describe("gstack-brain-enqueue", () => {
  ...
  test("enqueues when mode is full and .git exists", () => {
    ...
    const queue = fs.readFileSync(
      path.join(tmpHome, ".brain-queue.jsonl"),
      "utf-8",
    );
  ...
  test("skip list honored", () => {
    ...
    run(["gstack-config", "set", "artifacts_sync_mode", "full"]);
    fs.writeFileSync(
      path.join(tmpHome, ".brain-skip.txt"),
      "projects/foo/secret.jsonl\n",
    );
    ...
    for (let i = 0; i < 10; i++) {
      procs.push(
        new Promise<void>((resolve) => {
          const r = spawnSync(
            path.join(BIN, "gstack-brain-enqueue"),
            [`file-${i}.jsonl`],
            {
...
describe("gstack-jsonl-merge", () => {
  ...
  test("falls back to hash order for lines without ts", () => {
    ...
    const again = spawnSync(path.join(BIN, "gstack-jsonl-merge"), [
      base,
      ours,
      theirs,
    ]);
...
describe("init + sync + restore round-trip", () => {
  test("init creates canonical files + registers drivers", () => {
    ...
    expect(fs.existsSync(path.join(tmpHome, ".brain-privacy-map.json"))).toBe(
      true,
    );
    expect(fs.existsSync(path.join(tmpHome, ".gitattributes"))).toBe(true);
    expect(fs.existsSync(path.join(tmpHome, ".git/hooks/pre-commit"))).toBe(
      true,
    );
  ...
  test("full sync: init → enqueue → --once → commit pushed", () => {
    ...
    const log = spawnSync(
      "git",
      ["--git-dir=" + bareRemote, "log", "--oneline"],
      { encoding: "utf-8" },
    );
  ...
  test("restore round-trip: writes on machine A visible on machine B", () => {
    ...
    const aLearning =
      '{"skill":"x","insight":"machine A wisdom","ts":"2026-04-22T10:00:00Z"}\n';
    fs.writeFileSync(
      path.join(tmpHome, "projects/myproj/learnings.jsonl"),
      aLearning,
    );
    ...
    const restored = fs.readFileSync(
      path.join(machineB, "projects/myproj/learnings.jsonl"),
      "utf-8",
    );
    ...
    const cfg = spawnSync(
      "git",
      ["-C", machineB, "config", "--get", "merge.jsonl-append.driver"],
      { encoding: "utf-8" },
    );
    expect(cfg.stdout).toContain("gstack-jsonl-merge");
  ...
  for (const [name, content] of SECRETS) {
    test(`blocks ${name}`, () => {
      ...
      const status = JSON.parse(
        fs.readFileSync(path.join(tmpHome, ".brain-sync-status.json"), "utf-8"),
      );
...
describe("gstack-brain-uninstall", () => {
  test("removes sync config but preserves learnings/project data", () => {
    run(["gstack-artifacts-init", "--remote", bareRemote]);
    fs.mkdirSync(path.join(tmpHome, "projects", "user-data"), {
      recursive: true,
    });
    const preservedContent = '{"keep":"me","ts":"2026-04-22T12:00:00Z"}\n';
    fs.writeFileSync(
      path.join(tmpHome, "projects/user-data/learnings.jsonl"),
      preservedContent,
    );
