...
function run(env: Record<string, string | undefined>): Record<string, string> {
  const result = spawnSync("bash", [BIN], {
    env: { PATH: process.env.PATH, USERPROFILE: "", ...env } as Record<
      string,
      string
    >,
    encoding: "utf-8",
  ...
  if (result.status !== 0) {
    throw new Error(
      `gstack-paths failed (status ${result.status}): ${result.stderr}`,
    );
  }
...
describe("gstack-paths", () => {
  ...
  test("PLAN_ROOT chain: GSTACK_PLAN_DIR > CLAUDE_PLANS_DIR > HOME > CWD", () => {
    expect(
      run({ GSTACK_PLAN_DIR: "/tmp/explicit", HOME: "/h" }).PLAN_ROOT,
    ).toBe("/tmp/explicit");
    expect(run({ CLAUDE_PLANS_DIR: "/tmp/claude", HOME: "/h" }).PLAN_ROOT).toBe(
      "/tmp/claude",
    );
    expect(run({ HOME: "/tmp/myhome" }).PLAN_ROOT).toBe(
      "/tmp/myhome/.claude/plans",
    );
