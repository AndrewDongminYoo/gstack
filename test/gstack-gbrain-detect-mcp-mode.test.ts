...
function makeFakeClaude(opts: {
  ...
  const { hasGetJson, getJsonOutput, hasMcpList, mcpListOutput, exitOnAll } =
    opts;
...
function runDetect(extraEnv: Record<string, string> = {}): {
  code: number;
  json: any;
  stderr: string;
} {
...
describe("gbrain_mcp_mode — Tier 2: claude mcp list text-grep", () => {
  test("falls back to mcp list when get --json fails", () => {
    makeFakeClaude({
      ...
      hasMcpList: true,
      mcpListOutput:
        "gbrain: https://wintermute.tail554574.ts.net:3131/mcp (HTTP) - ✓ Connected",
    });
    expect(runDetect().json.gbrain_mcp_mode).toBe("remote-http");
  });
  ...
  test("mcp list with no gbrain entry → none", () => {
    makeFakeClaude({
      ...
      hasMcpList: true,
      mcpListOutput:
        "posthog: https://mcp.posthog.com/mcp (HTTP)\nslack: https://slack.com/mcp (HTTP)",
    });
...
describe("gbrain_mcp_mode — Tier 3: ~/.claude.json jq read", () => {
  ...
  test("no gbrain entry in ~/.claude.json → none", () => {
    fs.writeFileSync(
      path.join(tmpHome, ".claude.json"),
      JSON.stringify({
        mcpServers: { posthog: { type: "url", url: "https://x" } },
      }),
    );
    expect(runDetect().json.gbrain_mcp_mode).toBe("none");
  });
