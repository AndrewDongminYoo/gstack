...
function runMigration(fakeHome: string): {
  code: number;
  stdout: string;
  stderr: string;
} {
...
describe("v1.38.1.0 migration", () => {
  test("adds patterns to allowlist before USER ADDITIONS marker", () => {
    ...
    try {
      writeFileSync(
        join(home, ".gstack", ".brain-allowlist"),
        [
  ...
  test("adds entries to privacy-map.json via jq (preserves JSON validity)", () => {
    ...
    try {
      writeFileSync(
        join(home, ".gstack", ".brain-privacy-map.json"),
        JSON.stringify(
          [
      ...
      expect(
        parsed.find((e: any) => e.pattern === "projects/*/*-design-*.md").class,
      ).toBe("artifact");
    } finally {
      rmSync(home, { recursive: true, force: true });
    }
  ...
  test("adds union-merge rules to gitattributes", () => {
    ...
    try {
      writeFileSync(
        join(home, ".gstack", ".gitattributes"),
        [
  ...
  test("is idempotent: re-running on already-patched files is a no-op", () => {
    ...
    try {
      writeFileSync(
        join(home, ".gstack", ".brain-allowlist"),
        ["projects/*/learnings.jsonl", "# ---- USER ADDITIONS BELOW"].join(
          "\n",
        ) + "\n",
      );
      writeFileSync(
      ...
      rmSync(join(home, ".gstack", ".migrations"), {
        recursive: true,
        force: true,
      });

      const beforeAllowlist = readFileSync(
        join(home, ".gstack", ".brain-allowlist"),
        "utf-8",
      );
      const beforePrivacy = readFileSync(
        join(home, ".gstack", ".brain-privacy-map.json"),
        "utf-8",
      );
      const beforeAttrs = readFileSync(
        join(home, ".gstack", ".gitattributes"),
        "utf-8",
      );
      ...
      const afterAllowlist = readFileSync(
        join(home, ".gstack", ".brain-allowlist"),
        "utf-8",
      );
      const afterPrivacy = readFileSync(
        join(home, ".gstack", ".brain-privacy-map.json"),
        "utf-8",
      );
      const afterAttrs = readFileSync(
        join(home, ".gstack", ".gitattributes"),
        "utf-8",
      );
  ...
  test("repairs privacy-map even when allowlist is missing (per-file independence)", () => {
    ...
    try {
      ...
      const parsed = JSON.parse(
        readFileSync(join(home, ".gstack", ".brain-privacy-map.json"), "utf-8"),
      );
      const patterns = parsed.map((e: any) => e.pattern);
      expect(patterns).toContain("projects/*/*-design-*.md");
  ...
  test("migration marker prevents re-running", () => {
    ...
    try {
      ...
      expect(
        existsSync(join(home, ".gstack", ".migrations", "v1.38.1.0.done")),
      ).toBe(true);
      ...
      expect(
        readFileSync(join(home, ".gstack", ".brain-allowlist"), "utf-8"),
      ).toBe("# minimal\n");
    } finally {
      rmSync(home, { recursive: true, force: true });
    }
...
function runMigrationV140(fakeHome: string): {
  code: number;
  stdout: string;
  stderr: string;
} {
...
describe("v1.40.0.0 migration", () => {
  test("adds eng-review-test-plan pattern to allowlist on top of an installed v1.38.1.0 state", () => {
    ...
    try {
      ...
      const engRevIdx = content.indexOf(
      ...
      expect(
        existsSync(join(home, ".gstack", ".migrations", "v1.40.0.0.done")),
      ).toBe(true);
    } finally {
      rmSync(home, { recursive: true, force: true });
    }
  ...
  test("adds eng-review-test-plan entry to privacy-map.json via jq", () => {
    ...
    try {
      ...
      expect(
        parsed.find(
          (e: any) => e.pattern === "projects/*/*-eng-review-test-plan-*.md",
        ).class,
      ).toBe("artifact");
    } finally {
      rmSync(home, { recursive: true, force: true });
  ...
  test("adds union-merge rule to gitattributes", () => {
    ...
    try {
      ...
      expect(content).toContain(
        "projects/*/*-eng-review-test-plan-*.md merge=union",
      );
    } finally {
      rmSync(home, { recursive: true, force: true });
    }
  ...
  test("is idempotent: re-running is a no-op", () => {
    ...
    try {
      ...
      const occurrences =
        content.match(/projects\/\*\/\*-eng-review-test-plan-\*\.md/g) || [];
