...
describe("question-registry schema", () => {
  test("every entry has required fields", () => {
    for (const [key, q] of Object.entries(
      QUESTIONS as Record<string, QuestionDef>,
    )) {
  ...
  test("no duplicate ids (keys and id fields are 1:1 by construction)", () => {
    const ids = Object.values(QUESTIONS as Record<string, QuestionDef>).map(
      (q) => q.id,
    );
...
describe("question-registry helpers", () => {
  ...
  test("getRegistryStats totals are consistent", () => {
    ...
    const byCategorySum = Object.values(stats.by_category).reduce(
      (a, b) => a + b,
      0,
    );
...
describe("psychographic signal map", () => {
  ...
  test("applySignal returns [] for unknown user_choice", () => {
    const dims = newDimensionTotals();
    const applied = applySignal(
      dims,
      "scope-appetite",
      "definitely-not-a-real-choice",
    );
    expect(applied).toEqual([]);
  });
...
describe("one-way-doors classifier", () => {
  test("registry lookup wins when question_id is known", () => {
    ...
    const safeResult = classifyQuestion({
  ...
  test("skill-category fallback fires for cso:approval and land-and-deploy:approval", () => {
    ...
    expect(
      isOneWayDoor({ skill: "land-and-deploy", category: "approval" }),
    ).toBe(true);
  });
  ...
  test("skill-category set covers security + deploy", () => {
    ...
    expect(ONE_WAY_SKILL_CATEGORY_SET.has("land-and-deploy:approval")).toBe(
      true,
    );
  });
});
...
describe("preamble — QUESTION_TUNING injection", () => {
  ...
  test("codex host produces different paths", async () => {
    const { generateQuestionTuning } =
      await import("../scripts/resolvers/question-tuning");
    const codexCtx = {
      skillName: "test",
      tmplPath: "x",
      host: "codex" as const,
...
describe("end-to-end pipeline (binaries working together)", () => {
  test("log many expand choices → derive pushes scope_appetite up", () => {
    const tmpHome = fs.mkdtempSync(
      path.join(require("os").tmpdir(), "gstack-e2e-"),
    );
    try {
      ...
      const derive = spawnSync(devBin, ["--derive"], {
        env,
        cwd: ROOT,
        encoding: "utf-8",
      });
      ...
      const profileOut = spawnSync(devBin, ["--profile"], {
        env,
        cwd: ROOT,
        encoding: "utf-8",
      });
  ...
  test("preference blocks tune: write from inline-tool-output in full pipeline", () => {
    ...
    try {
      ...
      const read = spawnSync(prefBin, ["--read"], {
        env,
        cwd: ROOT,
        encoding: "utf-8",
      });
  ...
  test("migration preserves sessions, builder-profile shim still works", () => {
    ...
    try {
      ...
      // Migrate
      const m = spawnSync(devBin, ["--migrate"], {
        env,
        cwd: ROOT,
        encoding: "utf-8",
      });
      ...
      const shimOut = spawnSync(shimBin, [], {
        env,
        cwd: ROOT,
        encoding: "utf-8",
      });
