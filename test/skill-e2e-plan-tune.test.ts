...
- Show them the logged questions with counts and the follow/override ratio.`,
      ...
      const mentionsCEO =
        output.includes("plan-ceo-review-mode") ||
        output.includes("review mode");
      const mentionsShipTest =
        output.includes("ship-test-failure-triage") ||
        output.includes("test failed");
      const mentionsChangelog =
        output.includes("changelog") ||
        output.includes("ship-changelog-voice-polish");
      const foundCount = [
        mentionsCEO,
        mentionsShipTest,
        mentionsChangelog,
      ...
      recordE2E(
        evalCollector,
        "/plan-tune",
        "Plan-tune inspection flow (plain English)",
        result,
        {
      ...
      if (!noticedOverride) {
        console.warn(
          "Agent did not surface override/skip behavior from the log",
        );
      }
    },
    180_000,
