...
describe("resolve-user-slug fallback chain", () => {
  test("uses $USER when set (layer 2)", () => {
    const result = runConfig(["resolve-user-slug"], {
      GSTACK_HOME: TMP_HOME,
      USER: "alice-test",
    });
    expect(result.status).toBe(0);
  ...
  test("subsequent calls return same slug (stable across sessions)", () => {
    const first = runConfig(["resolve-user-slug"], {
      GSTACK_HOME: TMP_HOME,
      USER: "stabletest",
    });
    const second = runConfig(["resolve-user-slug"], {
      GSTACK_HOME: TMP_HOME,
      USER: "changed-after",
    });
...
describe("brain_trust_policy@<hash> namespace", () => {
  test('default value is "unset"', () => {
    const result = runConfig(["get", "brain_trust_policy@deadbeef"], {
      GSTACK_HOME: TMP_HOME,
    });
    expect(result.status).toBe(0);
  ...
  test("set + get roundtrip works", () => {
    const setResult = runConfig(
      ["set", "brain_trust_policy@deadbeef", "personal"],
      { GSTACK_HOME: TMP_HOME },
    );
    expect(setResult.status).toBe(0);
    const getResult = runConfig(["get", "brain_trust_policy@deadbeef"], {
      GSTACK_HOME: TMP_HOME,
    });
  ...
  test("invalid value falls back to unset with warning", () => {
    const result = runConfig(
      ["set", "brain_trust_policy@deadbeef", "invalid-value"],
      { GSTACK_HOME: TMP_HOME },
    );
    expect(result.status).toBe(0);
  ...
  test("shared value accepted", () => {
    runConfig(["set", "brain_trust_policy@deadbeef", "shared"], {
      GSTACK_HOME: TMP_HOME,
    });
    const getResult = runConfig(["get", "brain_trust_policy@deadbeef"], {
      GSTACK_HOME: TMP_HOME,
  ...
  test("per-endpoint policies dont collide", () => {
    runConfig(["set", "brain_trust_policy@aaaaaaaa", "personal"], {
      GSTACK_HOME: TMP_HOME,
    });
    runConfig(["set", "brain_trust_policy@bbbbbbbb", "shared"], {
      GSTACK_HOME: TMP_HOME,
    });
    const a = runConfig(["get", "brain_trust_policy@aaaaaaaa"], {
      GSTACK_HOME: TMP_HOME,
    });
    const b = runConfig(["get", "brain_trust_policy@bbbbbbbb"], {
      GSTACK_HOME: TMP_HOME,
    });
...
describe("key validation", () => {
  ...
  test("accepts @<hex-hash> suffix on key", () => {
    const result = runConfig(["get", "brain_trust_policy@abc123ff"], {
      GSTACK_HOME: TMP_HOME,
    });
    expect(result.status).toBe(0);
  });
