...
describeE2E("AskUserQuestion format compliance (gate)", () => {
  test("/plan-ceo-review's first AskUserQuestion is a compliant decision brief (7/7 + substance)", async () => {
    ...
    if (!text.trim()) {
      throw new Error(
        "No AskUserQuestion captured — the skill never reached its mode-selection question.",
      );
    }
