...
describeIfSelected(
  ...
  () => {
    beforeAll(() => {
      ...
      spawnSync(browseBin, ["goto", testServer.url], {
        cwd: tmpDir,
        timeout: 30000,
        stdio: "pipe",
      });
...
Report whether it worked.`,
        ...
        recordE2E(
          evalCollector,
          "SKILL.md setup block discovery",
          "Skill E2E tests",
          result,
        );
        expect(result.browseErrors).toHaveLength(0);
...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
    ...
    testConcurrentIfSelected(
      "operational-learning",
      async () => {
        const opDir = fs.mkdtempSync(
          path.join(os.tmpdir(), "skill-e2e-oplearn-"),
        );
        ...
        fs.copyFileSync(
          path.join(ROOT, "lib", "jsonl-store.ts"),
          path.join(libDir, "jsonl-store.ts"),
        );
            ...
            if (fs.existsSync(lPath)) {
              ...
              if (jsonl) {
                ...
                if (opEntry) {
                  hasOperational = true;
                  console.log(
                    `Operational learning logged: key="${opEntry.key}" insight="${opEntry.insight}" (slug: ${slug})`,
                  );
                  break;
                }
              }
