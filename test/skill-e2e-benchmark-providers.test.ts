...
describeIfEvals("multi-provider benchmark adapters (live)", () => {
  ...
  test("claude: trivial prompt produces parseable output", async () => {
    ...
    if (result.error) {
      throw new Error(
        `claude errored: ${result.error.code} — ${result.error.reason}`,
      );
    }
  ...
  test("gpt: trivial prompt produces parseable output", async () => {
    ...
    if (result.error) {
      throw new Error(
        `gpt errored: ${result.error.code} — ${result.error.reason}`,
      );
    }
    expect(result.output.toLowerCase()).toContain("ok");
    expect(result.tokens.input).toBeGreaterThan(0);
  ...
  test("gemini: trivial prompt produces parseable output", async () => {
    ...
    if (result.error) {
      throw new Error(
        `gemini errored: ${result.error.code} — ${result.error.reason}`,
      );
    }
  ...
  test("timeout error surfaces as error.code=timeout (no exception)", async () => {
    ...
    const adapter = (await claude.available()).ok
      ? claude
      : (await gpt.available()).ok
        ? gpt
        : (await gemini.available()).ok
          ? gemini
          : null;
    if (!adapter) {
      process.stderr.write(
    ...
    expect(["timeout", "unknown", "binary_missing"]).toContain(
      result.error!.code,
    );
    expect(result.durationMs).toBeGreaterThan(0);
  }, 30_000);
  ...
  test("runBenchmark: Promise.allSettled means one unavailable provider does not block others", async () => {
    ...
    const hadSuccess = report.entries.some(
      (e) => e.available && e.result && !e.result.error,
    );
