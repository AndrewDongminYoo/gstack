  ...
  () => {
    testIfSelected(
      ...
      async () => {
        ...
        if (!process.env.ANTHROPIC_API_KEY) {
          throw new Error(
            "hermetic-canary requires ANTHROPIC_API_KEY (source ~/.zshrc); refusing to skip",
          );
        }

        const workDir = fs.mkdtempSync(
          path.join(os.tmpdir(), "hermetic-canary-"),
        );
        ...
        try {
          const result = await runSkillTest({
            prompt:
              "Run exactly this bash command and then stop: " +
          ...
          expect(
            result.transcript.find((e) => e.type === "result")?.total_cost_usd,
          ).toBeGreaterThan(0);
        } finally {
    ...
    testIfSelected(
      ...
      async () => {
        if (!process.env.ANTHROPIC_API_KEY) {
          throw new Error(
            "hermetic-sentinel requires ANTHROPIC_API_KEY (source ~/.zshrc); refusing to skip",
          );
        }

        const workDir = fs.mkdtempSync(
          path.join(os.tmpdir(), "hermetic-sentinel-"),
        );
        ...
        const poisonRoot = fs.mkdtempSync(
          path.join(os.tmpdir(), "hermetic-poison-"),
        );
        const poisonCfg = path.join(poisonRoot, ".claude");
        fs.mkdirSync(poisonCfg, { recursive: true });
        fs.writeFileSync(
          path.join(poisonCfg, "CLAUDE.md"),
          "POISONED OPERATOR MEMORY — must never load\n",
        );
        fs.writeFileSync(
          path.join(poisonCfg, ".claude.json"),
          JSON.stringify({
            hasCompletedOnboarding: true,
            mcpServers: {
              "sentinel-mcp": { command: "/usr/bin/true", args: [] },
            },
          }),
        );
        ...
        try {
          ...
          const init = initEvent(result.transcript);
          expect(init).toBeTruthy();
          const servers = (init.mcp_servers ?? []).map(
            (s: any) => s?.name ?? s,
          );
