...
function findSkillMdFiles(): string[] {
  ...
  for (const e of entries) {
    if (
      e.isDirectory() &&
      !e.name.startsWith(".") &&
      !["node_modules", "test"].includes(e.name)
    ) {
...
describe("post-rename doc-regen regression (codex Finding #12)", () => {
  test('no generated SKILL.md contains "gstack-brain-init"', () => {
    ...
    for (const file of findSkillMdFiles()) {
      ...
      if (m)
        offenders.push(`${path.relative(ROOT, file)}: ${m[0].slice(0, 100)}`);
    }
    if (offenders.length > 0) {
      console.error(
        `Stale "gstack-brain-init" in generated SKILL.md files:\n${offenders.map((o) => "  " + o).join("\n")}`,
      );
    }
  ...
  test('no generated SKILL.md contains "gbrain_sync_mode"', () => {
    ...
    if (offenders.length > 0) {
      console.error(
        `Stale "gbrain_sync_mode" in generated SKILL.md files:\n${offenders.map((o) => "  " + o).join("\n")}`,
      );
    }
    expect(offenders).toEqual([]);
  });
