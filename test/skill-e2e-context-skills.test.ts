...
function setupWorkdir(suffix: string): {
  workDir: string;
  gstackHome: string;
  slug: string;
} {
  const workDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `skill-e2e-ctx-${suffix}-`),
  );
  ...
  () => {
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        recordE2E(
          evalCollector,
          "context-save routes via Skill tool",
          "Context Skills E2E",
          result,
          {
...
Do NOT use AskUserQuestion.`,
        ...
        const invokedSkills = skillCalls(result);
        const bothRouted =
          invokedSkills.includes("context-save") &&
          invokedSkills.includes("context-restore");
        const checkpointDir = path.join(
          gstackHome,
          "projects",
          slug,
        ...
        const restoreMentionsTitle = fullOutputSurface(result)
          .toLowerCase()
          .includes(magicMarker.toLowerCase());
        const exitOk = ["success", "error_max_turns"].includes(
          result.exitReason,
        );
        ...
        recordE2E(
          ...
          {
            passed:
              exitOk && bothRouted && files.length > 0 && restoreMentionsTitle,
          },
        );
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        seedSave(
          gstackHome,
          slug,
          "20260101-120000-alpha-feature.md",
          {
            status: "in-progress",
            branch: "feat/alpha",
        ...
        seedSave(
          gstackHome,
          slug,
          "20260202-120000-middle-payments.md",
          {
            status: "in-progress",
        ...
        seedSave(
          gstackHome,
          slug,
          "20260303-120000-omega-release.md",
          {
            status: "in-progress",
            branch: "feat/omega",
        ...
        const didNotLoadOthers =
          !out.includes("FRAGMATCH_ALPHA_BUILD") &&
          !out.includes("FRAGMATCH_OMEGA_BUILD");
        ...
        recordE2E(
          ...
          {
            passed:
              exitOk && routedToRestore && loadedPayments && didNotLoadOthers,
          },
        );
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        const gracefulMessage =
          /no saved context|no contexts? yet|nothing to restore|NO_CHECKPOINTS/i.test(
            out,
          );
        const noCrash =
          !/error|exception|undefined/i.test(out) || gracefulMessage; // mention of "error" in the graceful message is fine
        const routedToRestore = skillCalls(result).includes("context-restore");
        const exitOk = ["success", "error_max_turns"].includes(
          result.exitReason,
        );
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        seedSave(
          gstackHome,
          slug,
          "20260301-120000-legacy-pre-rename-work.md",
          {
            status: "in-progress",
        ...
        recordE2E(
          evalCollector,
          "legacy /checkpoint file loads via /context-restore",
          "Context Skills E2E",
          result,
          {
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        recordE2E(
          evalCollector,
          "context-save list (current branch default)",
          "Context Skills E2E",
          result,
          {
