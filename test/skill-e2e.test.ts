 ...
 */
function dumpOutcomeDiagnostic(
  dir: string,
  label: string,
  report: string,
  judgeResult: any,
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
        ...
        recordE2E(
          "SKILL.md setup block (no local binary)",
          "Skill E2E tests",
          result,
        );
    ...
    testIfSelected(
      ...
      async () => {
        const sessionDir = fs.mkdtempSync(
          path.join(os.tmpdir(), "skill-e2e-session-"),
        );
        ...
        run("git", [
          ...
          "add",
          "origin",
          "https://github.com/acme/billing-app.git",
        ]);
...
Remember: _SESSIONS=4, so ELI16 mode is active. The user is juggling multiple windows and may not remember what this conversation is about. Re-ground them.`,
        ...
        if (fs.existsSync(outputPath)) {
          ...
          expect(lower.includes("billing") || lower.includes("acme")).toBe(
            true,
          );
          // Must mention branch
          expect(lower.includes("payment") || lower.includes("feature")).toBe(
            true,
          );
...
describeIfSelected("Review skill E2E", ["review-sql-injection"], () => {
  ...
  beforeAll(() => {
    ...
    const vulnContent = fs.readFileSync(
      path.join(ROOT, "test", "fixtures", "review-eval-vuln.rb"),
      "utf-8",
    );
    ...
    fs.copyFileSync(
      path.join(ROOT, "review", "checklist.md"),
      path.join(reviewDir, "review-checklist.md"),
    );
    fs.copyFileSync(
      path.join(ROOT, "review", "greptile-triage.md"),
      path.join(reviewDir, "review-greptile-triage.md"),
    );
  });
  ...
  () => {
    ...
    beforeAll(() => {
      ...
      const baseContent = fs.readFileSync(
        path.join(ROOT, "test", "fixtures", "review-eval-enum.rb"),
        "utf-8",
      );
      ...
      const diffContent = fs.readFileSync(
        path.join(ROOT, "test", "fixtures", "review-eval-enum-diff.rb"),
        "utf-8",
      );
      ...
      fs.copyFileSync(
        path.join(ROOT, "review", "checklist.md"),
        path.join(enumDir, "review-checklist.md"),
      );
      fs.copyFileSync(
        path.join(ROOT, "review", "greptile-triage.md"),
        path.join(enumDir, "review-greptile-triage.md"),
      );
    });

    afterAll(() => {
      try {
        fs.rmSync(enumDir, { recursive: true, force: true });
      } catch {}
    });
...
The diff adds a new "returned" status to the Order model. Your job is to check if all consumers handle it.`,
      ...
      if (fs.existsSync(reviewPath)) {
        ...
        const mentionsEnum =
          review.toLowerCase().includes("enum") ||
          review.toLowerCase().includes("status");
...
describeE2E("Review design lite E2E", () => {
  ...
  beforeAll(() => {
    designDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "skill-e2e-design-lite-"),
    );
    ...
    fs.writeFileSync(
      path.join(designDir, "styles.css"),
      "body { font-size: 16px; }\n",
    );
    run("git", ["add", "."]);
    ...
    const slopCss = fs.readFileSync(
      path.join(ROOT, "test", "fixtures", "review-eval-design-slop.css"),
      "utf-8",
    );
    const slopHtml = fs.readFileSync(
      path.join(ROOT, "test", "fixtures", "review-eval-design-slop.html"),
      "utf-8",
    );
    ...
    fs.copyFileSync(
      path.join(ROOT, "review", "checklist.md"),
      path.join(designDir, "review-checklist.md"),
    );
    fs.copyFileSync(
    ...
    fs.copyFileSync(
      path.join(ROOT, "review", "greptile-triage.md"),
      path.join(designDir, "review-greptile-triage.md"),
    );
  });
...
Important: The design checklist should catch issues like blacklisted fonts, small font sizes, outline:none, !important, AI slop patterns (purple gradients, generic hero copy, 3-column feature grid), etc.`,
    ...
    if (fs.existsSync(reviewPath)) {
      ...
      if (
        review.includes("papyrus") ||
        review.includes("blacklisted font") ||
        review.includes("font family")
      )
        detected++;
      // Issue 2: Body text < 16px — HIGH
      if (
        review.includes("14px") ||
        review.includes("font-size") ||
        review.includes("font size") ||
        review.includes("body text")
      )
        detected++;
      // Issue 3: outline: none — HIGH
      if (review.includes("outline") || review.includes("focus")) detected++;
      // Issue 4: !important — HIGH
      if (review.includes("!important") || review.includes("important"))
        detected++;
      // Issue 5: Purple gradient — MEDIUM
      if (
        review.includes("gradient") ||
        review.includes("purple") ||
        review.includes("violet") ||
        review.includes("#6366f1") ||
        review.includes("#8b5cf6")
      )
        detected++;
      // Issue 6: Generic hero copy — MEDIUM
      if (
        review.includes("welcome to") ||
        review.includes("all-in-one") ||
        review.includes("generic") ||
        review.includes("hero copy") ||
        review.includes("ai slop")
      )
        detected++;
      // Issue 7: 3-column feature grid — LOW
      if (
        review.includes("3-column") ||
        review.includes("three-column") ||
        review.includes("feature grid") ||
        review.includes("icon") ||
        review.includes("circle")
      )
        detected++;
...
(anyOutcomeSelected ? describeOutcome : describe.skip)(
  ...
  () => {
     ...
     */
    async function runPlantedBugEval(
      fixture: string,
      ...
      const testWorkDir = fs.mkdtempSync(
        path.join(os.tmpdir(), `skill-e2e-${label}-`),
      );
...
- The report MUST exist at ${reportPath} when you finish`,
      ...
      if (
        ...
        throw new Error(
          `${label}: unexpected exit reason: ${result.exitReason}`,
        );
      }
      ...
      const groundTruth = JSON.parse(
        fs.readFileSync(
          path.join(ROOT, "test", "fixtures", groundTruthFile),
          "utf-8",
        ),
      );
          ...
          try {
            ...
            if (mdFiles.length > 0) {
              report = fs.readFileSync(
                path.join(searchDir, mdFiles[0]),
                "utf-8",
              );
              break;
            }
          } catch {
            /* dir may not exist if agent hit max_turns early */
          }
        }
      ...
      if (!report) {
        dumpOutcomeDiagnostic(testWorkDir, label, "(no report file found)", {
      ...
      // Phase 2 assertions
      expect(judgeResult.detection_rate).toBeGreaterThanOrEqual(
        groundTruth.minimum_detection,
      );
      expect(judgeResult.false_positives).toBeLessThanOrEqual(
        groundTruth.max_false_positives,
      );
...
`,
    ...
    {
      const _sec = path.join(ROOT, "plan-ceo-review", "sections");
      if (fs.existsSync(_sec))
        fs.cpSync(_sec, path.join(planDir, "plan-ceo-review", "sections"), {
          recursive: true,
        });
    }
  });
...
`,
    ...
    {
      const _sec = path.join(ROOT, "plan-eng-review", "sections");
      if (fs.existsSync(_sec))
        fs.cpSync(_sec, path.join(planDir, "plan-eng-review", "sections"), {
          recursive: true,
        });
    }
  });
...
describeIfSelected("Retro E2E", ["retro"], () => {
  ...
  beforeAll(() => {
    ...
    run("git", [
      "commit",
      "-m",
      "feat: initial app setup",
      "--date",
      "2026-03-10T09:00:00",
    ]);

    fs.writeFileSync(
      path.join(retroDir, "auth.ts"),
    ...
    run("git", [
      "commit",
      "-m",
      "fix: wire up auth to app",
      "--date",
      "2026-03-11T10:00:00",
    ]);

    fs.writeFileSync(
      path.join(retroDir, "test.ts"),
  ...
  afterAll(() => {
    try {
      fs.rmSync(retroDir, { recursive: true, force: true });
    } catch {}
  });
...
describeIfSelected("QA-Only skill E2E", ["qa-only-no-fix"], () => {
  ...
  afterAll(() => {
    try {
      fs.rmSync(qaOnlyDir, { recursive: true, force: true });
    } catch {}
  });
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout
      .toString()
      .trim()
      .split("\n")
      .filter(
        (l: string) =>
          l.trim() &&
          !l.includes(".prompt-tmp") &&
          !l.includes(".gstack/") &&
          !l.includes("qa-reports/"),
      );
    expect(
      statusLines.filter(
        (l: string) => l.startsWith(" M") || l.startsWith("M "),
      ),
    ).toHaveLength(0);
  }, 240_000);
});
...
This is a test+fix loop: find bugs, fix them in the source code, commit each fix, and re-verify.`,
    ...
    console.log(
      `/qa fix loop: ${commits.length} commits total (1 initial + ${commits.length - 1} fixes)`,
    );
...
describeIfSelected(
  ...
  ["review-base-branch", "ship-base-branch", "retro-base-branch"],
  () => {
    ...
    beforeAll(() => {
      baseBranchDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "skill-e2e-basebranch-"),
      );
    });

    afterAll(() => {
      try {
        fs.rmSync(baseBranchDir, { recursive: true, force: true });
      } catch {}
    });
    ...
    testIfSelected(
      ...
      async () => {
        ...
        fs.copyFileSync(
          path.join(ROOT, "review", "checklist.md"),
          path.join(dir, "review-checklist.md"),
        );
        fs.copyFileSync(
          path.join(ROOT, "review", "greptile-triage.md"),
          path.join(dir, "review-greptile-triage.md"),
        );
...
- The diff stat against the base branch`,
        ...
        const destructiveTools = result.toolCalls.filter(
          (tc) =>
            tc.tool === "Bash" &&
            typeof tc.input === "string" &&
            (tc.input.includes("git push") ||
              tc.input.includes("gh pr create")),
        );
    ...
    testIfSelected(
      ...
      async () => {
        ...
        run(
          "git",
          ["commit", "-m", "feat: add auth", "--date", "2026-03-15T10:00:00"],
          dir,
        );
        ...
        run(
          "git",
          ["commit", "-m", "test: add tests", "--date", "2026-03-16T11:00:00"],
          dir,
        );
...
Write your retrospective to ${dir}/retro-output.md`,
        ...
        recordE2E(
          "/retro default branch detection",
          "Base branch detection",
          result,
          {
            passed: ["success", "error_max_turns"].includes(result.exitReason),
...
describeIfSelected("Document-Release skill E2E", ["document-release"], () => {
  ...
  beforeAll(() => {
    docReleaseDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "skill-e2e-doc-release-"),
    );

    // Copy document-release skill files
    copyDirSync(
      path.join(ROOT, "document-release"),
      path.join(docReleaseDir, "document-release"),
    );
    ...
    fs.writeFileSync(
      path.join(docReleaseDir, "README.md"),
      "# Test Project\n\n## Features\n\n- Feature A\n- Feature B\n\n## Install\n\n```bash\nnpm install\n```\n",
    );

    // Create initial CHANGELOG that must NOT be clobbered
    fs.writeFileSync(
      path.join(docReleaseDir, "CHANGELOG.md"),
    ...
    fs.writeFileSync(
      path.join(docReleaseDir, "feature-c.ts"),
  ...
  afterAll(() => {
    try {
      fs.rmSync(docReleaseDir, { recursive: true, force: true });
    } catch {}
  });
...
- After editing, just commit the changes locally.`,
    ...
    if (readmeUpdated) {
      ...
      console.warn(
        "README was NOT updated — agent may not have found the feature",
      );
    }
  }, 240_000);
});
...
describeIfSelected("gstack-upgrade E2E", ["gstack-upgrade-happy-path"], () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      path.join(mockGstack, "CHANGELOG.md"),
      "# Changelog\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n",
    );
    fs.writeFileSync(
      path.join(mockGstack, "setup"),
...
IMPORTANT: The install directory is at ./.claude/skills/gstack — use that exact path.`,
      ...
      const versionAfter = fs
        .readFileSync(path.join(mockGstack, "VERSION"), "utf-8")
        .trim();
      const output = result.output || "";
 ...
 */
async function designQualityJudge(
  designMd: string,
): Promise<{ passed: boolean; reasoning: string }> {
  return callJudge<{
    passed: boolean;
    reasoning: string;
  }>(`You are evaluating a generated DESIGN.md file for quality.
...
`,
      ...
      fs.writeFileSync(
        path.join(designDir, "package.json"),
        JSON.stringify(
          {
            name: "civicpulse",
            version: "0.1.0",
      ...
      fs.mkdirSync(path.join(designDir, "design-consultation"), {
        recursive: true,
      });
      fs.copyFileSync(
...
Write DESIGN.md and CLAUDE.md (or update it) in the working directory.`,
        ...
        const missingSections = requiredSections.filter(
          (s) => !designContent.toLowerCase().includes(s.toLowerCase()),
        );
        ...
        if (designExists && designContent.length > 100) {
          try {
            ...
            judgeResult = {
              passed: true,
              reasoning: "judge error — defaulting to pass",
            };
          }
        }

        const structuralPass =
          designExists && claudeExists && missingSections.length === 0;
        recordE2E(
          "/design-consultation core",
          "Design Consultation E2E",
          result,
    ...
    testIfSelected(
      ...
      async () => {
        ...
        try {
          fs.unlinkSync(path.join(designDir, "DESIGN.md"));
        } catch {}
        try {
          fs.unlinkSync(path.join(designDir, "CLAUDE.md"));
        } catch {}
...
Write DESIGN.md to the working directory.`,
        ...
        const webSearchCalls = result.toolCalls.filter(
          (tc) => tc.tool === "WebSearch",
        );
...
Skip research. Skip any AskUserQuestion calls — this is non-interactive. Generate the font and color preview page but write it to ./design-preview.html instead of /tmp/ (do NOT run the open command). Then write DESIGN.md.`,
        ...
        const hasHtml =
          ...
          previewContent.includes("<!DOCTYPE");
        const hasFontRef =
...
describeIfSelected(
  ...
  ["plan-design-review-plan-mode", "plan-design-review-no-ui-scope"],
  () => {
    let reviewDir: string;

    beforeAll(() => {
      reviewDir = fs.mkdtempSync(
      ...
      fs.mkdirSync(path.join(reviewDir, "plan-design-review"), {
        recursive: true,
      });
      fs.copyFileSync(
      ...
      {
        const _sec = path.join(ROOT, "plan-design-review", "sections");
        if (fs.existsSync(_sec))
          fs.cpSync(
            _sec,
            path.join(reviewDir, "plan-design-review", "sections"),
            { recursive: true },
          );
      }
...
IMPORTANT: Do NOT try to browse any URLs or use a browse binary. This is a plan review, not a live site audit. Just read the plan file, review it, and edit it to fix the gaps.`,
        ...
        const planAfter = fs.readFileSync(
          path.join(reviewDir, "plan.md"),
          "utf-8",
        );
    ...
    testIfSelected(
      ...
      async () => {
        ...
        fs.writeFileSync(
          path.join(reviewDir, "backend-plan.md"),
...
IMPORTANT: Do NOT try to browse any URLs or use a browse binary. This is a plan review, not a live site audit.`,
        ...
        const output = result.output || "";
        const detectsNoUI =
          output.toLowerCase().includes("no ui") ||
...
describeIfSelected("Design Review E2E", ["design-review-fix"], () => {
  ...
  beforeAll(() => {
    qaDesignDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "skill-e2e-qa-design-"),
    );
    ...
    fs.writeFileSync(
      path.join(qaDesignDir, "index.html"),
      `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <main>
    ...
    fs.writeFileSync(
      path.join(qaDesignDir, "style.css"),
      `body {
...
`,
  ...
  afterAll(() => {
    qaDesignServer?.stop();
    try {
      fs.rmSync(qaDesignDir, { recursive: true, force: true });
    } catch {}
  });
...
Review the site at ${serverUrl}. Use --quick mode. Skip any AskUserQuestion calls — this is non-interactive. Fix up to 3 issues max. Write your report to ./design-audit.md.`,
    ...
    const commits = gitLog.stdout.toString().trim().split("\n");
    const designFixCommits = commits.filter((c: string) =>
      c.includes("style(design)"),
    );
...
describeIfSelected("Test Bootstrap E2E", ["qa-bootstrap"], () => {
  ...
  beforeAll(() => {
    bootstrapDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "skill-e2e-bootstrap-"),
    );
...
`,
    ...
    fs.writeFileSync(
      path.join(bootstrapDir, "index.html"),
      `<!DOCTYPE html>
<html lang="en">
...
This is a test+fix loop: find bugs, fix them, write regression tests, commit each fix.`,
    ...
    const hasTestConfig =
      fs.existsSync(path.join(bootstrapDir, "vitest.config.ts")) ||
    ...
    const regressionLog = spawnSync(
      "git",
      ["log", "--oneline", "--grep=test(qa)"],
      {
        cwd: bootstrapDir,
        stdio: "pipe",
      },
...
describeIfSelected("Test Coverage Audit E2E", ["ship-coverage-audit"], () => {
  ...
  beforeAll(() => {
    ...
    const {
      createCoverageAuditFixture,
    } = require("./fixtures/coverage-audit-fixture");
...
Output the diagram directly.`,
    ...
    const hasGap =
      outputLower.includes("gap") || outputLower.includes("no test");
    const hasTested =
      outputLower.includes("tested") ||
      output.includes("✓") ||
      output.includes("★");
    const hasCoverage =
      outputLower.includes("coverage") || outputLower.includes("paths tested");
...
describeIfSelected(
  ...
  () => {
    ...
    beforeAll(() => {
      reviewCoverageDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "skill-e2e-review-coverage-"),
      );

      // Copy review skill files
      copyDirSync(
        path.join(ROOT, "review"),
        path.join(reviewCoverageDir, "review"),
      );
    ...
    afterAll(() => {
      try {
        fs.rmSync(reviewCoverageDir, { recursive: true, force: true });
      } catch {}
    });
...
Output the diagram directly.`,
      ...
      recordE2E(
        "/review Step 4.75 coverage audit",
        "Review Coverage Audit E2E",
        result,
        {
          passed: result.exitReason === "success",
      ...
      const hasTested =
        ...
        output.includes("★");
      const hasCoverage =
        outputLower.includes("coverage") ||
        outputLower.includes("paths tested");
...
describeIfSelected(
  ...
  () => {
    ...
    beforeAll(() => {
      planCoverageDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "skill-e2e-plan-coverage-"),
      );

      // Copy plan-eng-review skill files
      copyDirSync(
        path.join(ROOT, "plan-eng-review"),
        path.join(planCoverageDir, "plan-eng-review"),
      );
    ...
    afterAll(() => {
      try {
        fs.rmSync(planCoverageDir, { recursive: true, force: true });
      } catch {}
    });
...
Output the diagram directly.`,
      ...
      recordE2E(
        "/plan-eng-review coverage audit",
        "Plan Eng Review Coverage Audit E2E",
        result,
        {
          passed: result.exitReason === "success",
...
This is a solo repo (REPO_MODE=solo). For pre-existing failures, recommend fixing now.`,
    ...
    const hasInBranch =
      outputLower.includes("in-branch") ||
      outputLower.includes("in branch") ||
      outputLower.includes("introduced");
    // The triage should identify the math/divide failure as pre-existing
    const hasPreExisting =
      outputLower.includes("pre-existing") ||
      outputLower.includes("pre existing") ||
      outputLower.includes("existed before");
    ...
    const mentionsTruncate =
      outputLower.includes("truncate") || outputLower.includes("string");
    const mentionsDivide =
      outputLower.includes("divide") || outputLower.includes("math");

    console.log(
      `Mentions truncate/string (in-branch bug): ${mentionsTruncate}`,
    );
    ...
    const ranMathTest =
      output.includes("math.test") || output.includes("FAIL: divide");
    const ranStringTest =
      output.includes("string.test") || output.includes("FAIL: truncate");
...
describeIfSelected("Codex skill E2E", ["codex-review"], () => {
  ...
  beforeAll(() => {
    ...
    fs.copyFileSync(
      ...
      path.join(codexDir, "codex-SKILL.md"),
    );
  });
  ...
  test("/codex review produces findings and GATE verdict", async () => {
    ...
    const codexCheck = spawnSync("which", ["codex"], {
      stdio: "pipe",
      timeout: 3000,
    });
...
describeIfSelected(
  ...
  () => {
    ...
    beforeAll(() => {
      ...
      {
        const _sec = path.join(ROOT, "office-hours", "sections");
        if (fs.existsSync(_sec))
          fs.cpSync(_sec, path.join(ohDir, "office-hours", "sections"), {
            recursive: true,
          });
      }
    });
    ...
    beforeAll(() => {
      ...
      );
      {
        ...
        if (fs.existsSync(_sec))
          fs.cpSync(
            _sec,
            path.join(benefitsDir, "plan-ceo-review", "sections"),
            { recursive: true },
          );
      }
    });
...
describeIfSelected("Ship idempotency", ["ship-idempotency"], () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      path.join(idempDir, "CHANGELOG.md"),
      "# Changelog\n\n## [0.2.0.0] — 2026-03-30\n\n- Updated app to v2\n",
    );
    ...
    const extracted = [
      full.slice(
        step4Start,
        step4End > step4Start ? step4End : step4Start + 500,
      ),
      full.slice(
        step7Start,
        step8End > step7Start ? step8End : step7Start + 500,
      ),
    ].join("\n\n---\n\n");
...
Do NOT modify VERSION or CHANGELOG. Only run the detection checks and report.`,
      ...
      const version = fs
        .readFileSync(path.join(idempDir, "VERSION"), "utf-8")
        .trim();
      ...
      const versionEntries = (changelog.match(/## \[0\.2\.0\.0\]/g) || [])
        .length;
