...
const gbrainPath = spawnSync("which", ["gbrain"], {
  encoding: "utf-8",
}).stdout.trim();
...
describe.skipIf(!shouldRun)(
  ...
  () => {
    test("init with voyage-code-3 produces a 1024-dim-aligned PGLite config", () => {
      const s = makeSandbox();
      try {
    ...
    test("sync --strategy code generates Voyage embeddings and registers pages + chunks", () => {
      const s = makeSandbox();
      try {
    ...
    test("code-def finds symbols defined in the embedded fixture", () => {
      const s = makeSandbox();
      try {
        runGbrain(s, [
          "init",
