...
function runLog(
  ...
  try {
    const stdout = execSync(
      `${BIN}/gstack-timeline-log '${input.replace(/'/g, "'\\''")}'`,
      execOpts,
    ).trim();
...
function runReadArgs(args: string[] = []): string {
  ...
  try {
    return execFileSync(
      path.join(BIN, "gstack-timeline-read"),
      args,
      execOpts,
    ).trim();
  } catch {
...
describe("gstack-timeline-log", () => {
  ...
  test("preserves timestamp when ts field is present", () => {
    const input =
      '{"skill":"review","event":"completed","branch":"main","ts":"2025-06-15T10:00:00Z"}';
    runLog(input);
...
describe("gstack-timeline-read", () => {
  ...
  test("limits output with --limit", () => {
    ...
    const unlimitedEvents = unlimited
      .split("\n")
