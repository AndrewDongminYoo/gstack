...
Write your review findings to ${reviewDir}/review-output.md`,
      ...
      if (fs.existsSync(reviewOutputPath)) {
        const reviewContent = fs
          .readFileSync(reviewOutputPath, "utf-8")
          .toLowerCase();
        const hasSqlContent =
  ...
  () => {
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        const step0Start = full.indexOf(
          "## Step 0: Detect platform and base branch",
        );
        const step1Start = full.indexOf("## Step 1: Check branch");
        const step1End = full.indexOf("---", step1Start + 10);
        const extracted = full.slice(
          step0Start,
          step1End > step1Start ? step1End : step1Start + 500,
        );
...
Write your findings to ${dir}/review-output.md`,
        ...
        const usedGitDiff = result.toolCalls.some((tc) => {
          ...
          const cmd =
            typeof tc.input === "string"
              ? tc.input
              : tc.input?.command || JSON.stringify(tc.input);
    ...
    testConcurrentIfSelected(
      ...
      async () => {
        ...
        const fullShipSkill = fs.readFileSync(
          path.join(ROOT, "ship", "SKILL.md"),
          "utf-8",
        );
        const step0Start = fullShipSkill.indexOf(
          "## Step 0: Detect platform and base branch",
        );
        const step0End = fullShipSkill.indexOf("## Step 1: Pre-flight");
        const shipSection = fullShipSkill.slice(
          step0Start,
          step0End > step0Start ? step0End : undefined,
        );
...
describeIfSelected(
  "Review Dashboard Via Attribution",
  ["review-dashboard-via"],
  () => {
    let dashDir: string;

    beforeAll(() => {
      dashDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "skill-e2e-dashboard-via-"),
      );
      ...
      fs.writeFileSync(
        path.join(mockBinDir, "gstack-review-read"),
        [
          "#!/usr/bin/env bash",
      ...
      const dashSection = fullSkill.slice(
        dashStart,
        dashEnd > dashStart ? dashEnd : undefined,
      );
...
Write the dashboard output to ${dashDir}/dashboard-output.md`,
        ...
        recordE2E(
          evalCollector,
          "/ship review dashboard via attribution",
          "Dashboard via field",
          result,
        );
        expect(result.exitReason).toBe("success");
        ...
        const gateQuestions = result.toolCalls.filter(
          (tc) =>
