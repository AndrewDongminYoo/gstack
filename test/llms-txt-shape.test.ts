...
describe("gen-llms-txt — strict mode", () => {
  test("does NOT throw on the live skill set (every gstack skill has name + description)", async () => {
    ...
    await expect(
      generateLlmsTxt({ root: ROOT, strict: true }),
  ...
  test("throws on a synthesized skill missing description", async () => {
    ...
    const tmp = fs.mkdtempSync(
      path.join(require("os").tmpdir(), "llms-txt-strict-"),
    );
    try {
      ...
      await expect(
        generateLlmsTxt({ root: tmp, strict: true }),
      ).rejects.toThrow(/missing name or description/);
    } finally {
